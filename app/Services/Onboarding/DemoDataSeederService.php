...
class DemoDataSeederService
{
    ...
    public function reset(Restaurant $restaurant): void
    {
        DB::transaction(function () use ($restaurant) {
            ...
            // Xóa employees mẫu và user accounts của họ
            $employees = Employee::withTrashed()->where('restaurant_id', $rid)
                ->where('status', 'demo')
                ->get();
            $userIds = $employees->pluck('user_id')->filter();
            Employee::withTrashed()->where('restaurant_id', $rid)->where('status', 'demo')->forceDelete();
    ...
    private function seedEmployees(Restaurant $restaurant): Collection
    {
        ...
        $demoStaff = [
            ['Nguyễn Văn Minh',  'minh.demo',   'manager', $managerRoleId, 85000,  'M'],
            ['Trần Thị Hoa',     'hoa.demo',    'staff',   $staffRoleId,   65000,  'F'],
            ['Lê Quang Hưng',    'hung.demo',   'staff',   $staffRoleId,   65000,  'M'],
            ['Phạm Thu Thảo',    'thao.demo',   'staff',   $staffRoleId,   65000,  'F'],
            ['Đỗ Bảo Long',      'long.demo',   'staff',   $staffRoleId,   65000,  'M'],
        ];
        ...
        foreach ($demoStaff as [$name, $emailPrefix, $roleName, $roleId, $payRate, $gender]) {
            ...
            $emp = Employee::updateOrCreate(
                ...
                [
                    ...
                    'pay_type'    => 'hourly',
                    'pay_rate'    => $payRate,
                    'status'      => 'demo', // dùng để nhận diện khi reset
                ]
            );
    ...
    {
        return [
            'g'    => Unit::updateOrCreate(['restaurant_id' => $restaurant->id, 'symbol' => 'g'],    ['name' => 'Gram',       'type' => 'mass']),
            'kg'   => Unit::updateOrCreate(['restaurant_id' => $restaurant->id, 'symbol' => 'kg'],   ['name' => 'Kilogram',   'type' => 'mass']),
            'ml'   => Unit::updateOrCreate(['restaurant_id' => $restaurant->id, 'symbol' => 'ml'],   ['name' => 'Milliliter', 'type' => 'volume']),
            'l'    => Unit::updateOrCreate(['restaurant_id' => $restaurant->id, 'symbol' => 'l'],    ['name' => 'Liter',      'type' => 'volume']),
            'cai'  => Unit::updateOrCreate(['restaurant_id' => $restaurant->id, 'symbol' => 'cai'],  ['name' => 'Cái',        'type' => 'count']),
            'chai' => Unit::updateOrCreate(['restaurant_id' => $restaurant->id, 'symbol' => 'chai'], ['name' => 'Chai',       'type' => 'count']),
        ];
    }
    ...
    private function ingredient(int $rid, ?int $branchId, Unit $unit, string $sku, string $name, string $catName, int $minStock, int $reorder, float $cost): Ingredient
    {
        return Ingredient::updateOrCreate(
            ['restaurant_id' => $rid, 'sku' => $sku],
            ['branch_id' => $branchId, 'unit_id' => $unit->id, 'name' => $name, 'category_name' => $catName, 'min_stock_level' => $minStock, 'reorder_level' => $reorder, 'average_cost' => $cost, 'status' => 'active']
        );
    }
