...
use App\Jobs\RefreshBranchInventoryJob;
...
class InventoryAvailabilityService
{
    ...
    {
        RefreshBranchInventoryJob::dispatch($restaurantId, $branchId, $notify);
    }
    ...
    public function refreshBranch(int $restaurantId, int $branchId, bool $notify = true): void
    {
        ...
        if ($notify && $newlySoldOut !== []) {
            DB::afterCommit(function () use ($newlySoldOut, $branchId, $restaurantId): void {
                $branchName = (string) (RestaurantBranch::withoutGlobalScopes()->whereKey($branchId)->value('name') ?? 'hiện tại');
