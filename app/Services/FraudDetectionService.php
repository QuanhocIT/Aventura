...
class FraudDetectionService
{
    ...
    public function detectAiFraudAlerts(int $restaurantId, string $periodStart, string $periodEnd): array
    {
        ...
        // 3. Fallback PHP: derive alerts from real audit_logs only (no synthetic/canned
        // incidents). This used to always inject two fictional "ai-1"/"ai-2" alerts with
        // invented order numbers and amounts regardless of actual activity — that fabricated
        // data even when nothing suspicious had happened. Now this list starts empty and is
        // only ever populated from genuine audit log rows below.
        $alerts = [];
