...
class EmailMicroserviceClient
{
    ...
    {
        if (empty($this->baseUrl) || app(ServiceMonitorService::class)->isMaintenance('email_service')) {
    ...
    private function sendViaBrevoDirectly(string $endpoint, array $payload): bool
    {
        ...
        // Không có Brevo key, hoặc dev dùng mail driver 'log' → đi thẳng Mail (SMTP/log).
        // KHÔNG trả false ở đây (sẽ làm hỏng cả chuỗi fallback dù SMTP vẫn gửi được).
        if (empty($apiKey) || config('mail.default') === 'log') {
