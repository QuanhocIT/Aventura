...
class PolicyEnforcementService
{
    ...
    public function isWithinShiftHours(User $user): bool
    {
        ...
        $now = now();
        $hasActiveShift = ScheduleAssignment::where('employee_id', function ($q) use ($user) {
    ...
    {
        // event là ENUM created/updated/deleted — 'policy_check' không hợp lệ, luôn vi
        // phạm ràng buộc và khiến bản ghi audit của mọi hành động nhạy cảm (đổi chính
        // sách vận hành...) không bao giờ được lưu. Bản chất các lời gọi hiện tại đều
        // là cập nhật cấu hình nên 'updated' là phân loại đúng, không chỉ là giá trị an toàn.
        AuditLog::create([
