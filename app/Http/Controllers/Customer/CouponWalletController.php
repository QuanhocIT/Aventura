...
{
    public function showWallet(int $restaurantId, string $phone)
    {
    ...
    public function claimCoupon(int $restaurantId, string $phone, Request $request)
    {
        $customer = Customer::withoutGlobalScopes()
            ->where('restaurant_id', $restaurantId)
            ->where('phone', $phone)
        ...
        $promotion = Promotion::withoutGlobalScopes()->findOrFail($data['promotion_id']);
