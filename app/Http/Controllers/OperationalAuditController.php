...
class OperationalAuditController extends Controller
{
    ...
    public function page(Request $request): Response
    {
        ...
        $stats = [
            ...
            'overdue' => $activeReports->filter(fn (OperationalInfringementReport $report) =>
                $report->remediation_deadline?->lt($today) === true
            )->count(),
        ...
        $planStats = [
            ...
            'overdue' => $plans->filter(fn (OperationalInspectionPlan $plan) =>
                $plan->due_date?->lt($today) === true && ! in_array($plan->status, ['completed', 'cancelled'], true)
            )->count(),
        ];
        ...
        $trend = collect(range(5, 0))->map(function (int $monthsAgo) use ($reports): array {
            ...
            $monthReports = $reports->filter(fn (OperationalInfringementReport $report) =>
                $report->infringement_date?->isSameMonth($month) === true
            );
