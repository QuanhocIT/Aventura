    ...
    public function create(Request $request): Response
    {
     ...
     public function store(Request $request): RedirectResponse
     {
         ...
         $data = $request->validate([
             ...
             // Delivery address fields — required only when channel = delivery
             'delivery_customer_name' => [($request->input('channel') === 'delivery' ? 'required' : 'nullable'), 'string', 'max:255'],
             'delivery_phone'         => [($request->input('channel') === 'delivery' ? 'required' : 'nullable'), 'string', 'max:20'],
             'delivery_address'       => [($request->input('channel') === 'delivery' ? 'required' : 'nullable'), 'string', 'max:500'],
        ...
        if (isset($data['items'])) {
            foreach ($data['items'] as $itemData) {
                if (!empty($itemData['product_id'])) {
                    $prod = \App\Models\Product::where('restaurant_id', $user->restaurant_id)->find($itemData['product_id']);
