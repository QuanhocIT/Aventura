...
class CouponController extends Controller
{
    public function index(Request $request): Response
    {
        ...
        $stats = [
            ...
            'expired' => Coupon::where('status', 'inactive')
                ...
                ->count(),
            'total_uses' => BillingAdjustment::whereNotNull('coupon_code')->count(),
            'total_saved' => number_format(
                (float) BillingAdjustment::whereNotNull('coupon_code')->sum('discount_amount'),
                0, ',', '.'
            ),
        ];
    ...
    public function destroy(Coupon $coupon): RedirectResponse
    {
        ...
        $used = BillingAdjustment::where('coupon_code', $coupon->code)->exists();
