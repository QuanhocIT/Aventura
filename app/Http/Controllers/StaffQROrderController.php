...
class StaffQROrderController extends Controller
{
    ...
    public function confirm(Request $request, TemporaryOrder $temporaryOrder): JsonResponse
    {
        ...
        try {
            ...
            Log::warning('Broadcast temporary order updated failed: '.$e->getMessage());
        }

        // Gợi ý Upselling AI chỉ là phần bổ trợ: nhân viên không có quyền xem
        // gợi ý, hoặc dịch vụ gợi ý lỗi, đều không được làm hỏng việc xác nhận
        // đơn đã thực hiện xong ở trên.
        $upsell = null;
        ...
        try {
            ...
            $upsell = $promotionController->getUpsellSuggestion($upsellRequest)->getData();
        } catch (\Throwable $e) {
            Log::info(
                'Bỏ qua gợi ý upsell khi xác nhận đơn tạm: '.$e->getMessage(),
            );
        }
    ...
    public function requestRevision(Request $request, TemporaryOrder $temporaryOrder): JsonResponse
    {
        ...
        try {
            ...
            Log::warning('Broadcast temporary order revision failed: '.$e->getMessage());
        }
