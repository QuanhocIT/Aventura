...
class EmployeeManagementController extends Controller
{
    ...
    public function employeesPage(Request $request): Response
    {
        ...
        $centralWarehouse = $isWarehouseManager
            ? app(CentralWarehouseService::class)->getCentralWarehouse((int) $user->restaurant_id)
            : null;
        ...
        // Tự động đồng bộ tài khoản User chưa có hồ sơ Employee.
        ...
        foreach ($usersWithoutEmployee as $u) {
            ...
            Employee::create([
                ...
                // Hồ sơ đồng bộ tự động chưa có mức lương do Nhân sự duyệt.
                // Trước đây gán cứng 8.000.000đ, khiến quỹ lương chi nhánh bị
                // trừ bằng một con số không ai nhập — để 0 cho tới khi Nhân sự
                // đặt bậc lương thật.
                'base_salary' => 0,
        ...
        $employees = Employee::where('restaurant_id', $user->restaurant_id)
            ...
            })->whereHas('user.roles', fn ($rq) => $rq->whereIn('name', ['warehouse_staff', 'warehouse_manager'])))
        ...
        return Inertia::render('employees/Index', [
            ...
            'payrollBudget' => ($isBranchManager || $isWarehouseManager || ($user->isOwner() && $viewBranchId)) && $viewBranchId
                ? app(PayrollBudgetService::class)->summary((int) $user->restaurant_id, (int) $viewBranchId)
                : null,
            ...
            'wageTiers' => WageTier::where('restaurant_id', $restaurant->id)
    ...
    public function storeEmployee(Request $request): RedirectResponse
    {
        ...
        $data = $request->validate([
            ...
            'salary_calculation_method' => ['sometimes', 'string', 'in:standard_days,fixed_package'],
            'standard_working_days' => ['sometimes', 'nullable', 'integer', 'min:1', 'max:31'],
            'allowance_meal' => ['sometimes', 'nullable', 'numeric', 'min:0'],
            'allowance_transport' => ['sometimes', 'nullable', 'numeric', 'min:0'],
            'allowance_phone' => ['sometimes', 'nullable', 'numeric', 'min:0'],
            'allowance_responsibility' => ['sometimes', 'nullable', 'numeric', 'min:0'],
            'allowance_other' => ['sometimes', 'nullable', 'numeric', 'min:0'],
        ...
        if ($isInspectorRole) {
            ...
            if ($centralBranch && $branchId === (int) $centralBranch->id && ! $isWarehouseRole) {
                throw ValidationException::withMessages([
                    'role' => 'Khi tạo nhân sự ở chi nhánh Kho Tổng, chỉ có thể tạo Trưởng kho Tổng hoặc Nhân viên Kho Tổng.',
                ]);
            }
            ...
            if ($isWarehouseRole) {
                ...
                if ($tenantContext->isBranchScoped() && $activeBranchId !== null && $activeBranchId !== 0 && $activeBranchId !== (int) $centralBranch->id) {
                    throw ValidationException::withMessages([
                        'role' => 'Khi chọn tạo nhân sự ở chi nhánh kinh doanh thì không thể tạo nhân viên Kho Tổng. Vui lòng chuyển sang Phạm vi toàn chuỗi hoặc Kho Tổng.',
                    ]);
                }

                if ($branchId !== 0 && $branchId !== (int) $centralBranch->id) {
                    throw ValidationException::withMessages([
                        'branch_id' => 'Nhân viên Kho Tổng chỉ được phép xếp làm việc tại chi nhánh Tổng kho, không được xếp vào chi nhánh kinh doanh.',
                    ]);
                }
        ...
        if (! empty($data['wage_tier_id'])) {
            $wageTierQuery = WageTier::where('restaurant_id', $user->restaurant_id)
        ...
        $isOwner = $user->isOwner() || $user->isSuperAdmin();
        $hasPublishedWageTiers = $branchId !== null && WageTier::where('restaurant_id', $user->restaurant_id)
            ->forBranch($branchId)
            ->active()
            ->exists();
        ...
        $budgets = app(PayrollBudgetService::class);
        ...
        try {
            [$newUser, $newEmployee] = DB::transaction(function () use ($data, $user, $branchId, $frontUrl, $backUrl, $budgets, $monthlyWage, $isWarehouseRole): array {
                ...
                if ($branchId !== null) {
                    $budgetLocked = BranchPayrollBudget::where('restaurant_id', $user->restaurant_id)
                ...
                $centralBranch = app(CentralWarehouseService::class)->getCentralWarehouse($user->restaurant_id);
                ...
                $newEmployee = Employee::create([
                    ...
                    'salary_calculation_method' => $data['salary_calculation_method'] ?? 'standard_days',
                    ...
                    'allowance_responsibility' => $data['allowance_responsibility'] ?? 0,
        ...
        try {
            app(TrainingService::class)->autoAssignRequiredCourses($newEmployee);
...
";
    ...
    public function updateEmployee(Request $request, Employee $employee): RedirectResponse
    {
        ...
        if ($isTargetInspectorRole) {
            ...
            if ($tenantContext->isBranchScoped() && $tenantContext->activeBranchId() !== null && $tenantContext->activeBranchId() !== 0 && $tenantContext->activeBranchId() !== (int) $centralBranch->id) {
                throw ValidationException::withMessages([
                    'role' => 'Khi ở chi nhánh kinh doanh không thể gán nhân viên Kho Tổng.',
                ]);
            }
            ...
            if ($centralBranch && $newBranchId === (int) $centralBranch->id && ! $isTargetWarehouseRole) {
                throw ValidationException::withMessages([
                    'role' => 'Khi gán nhân viên làm việc tại chi nhánh Kho Tổng, chỉ được phép chọn Trưởng kho Tổng hoặc Nhân viên Kho Tổng.',
                ]);
            }
    ...
    public function storeBonus(Request $request): RedirectResponse
    {
        ...
        // Nếu là Chủ doanh nghiệp hoặc Super Admin: Thưởng trực tiếp
        ...
        // Nếu là Quản lý hoặc nhân sự khác: Yêu cầu Chủ doanh nghiệp phê duyệt
        $hasPendingRequest = ApprovalRequest::forRestaurant((int) $employee->restaurant_id)
            ->where('operation_type', 'employee_bonus')
            ->where('status', 'pending')
            ->get()
            ->contains(function (ApprovalRequest $approval) use ($employee, $data) {
                $opData = $approval->operation_data ?? [];

                return (int) ($opData['employee_id'] ?? 0) === (int) $employee->id
                    && abs((float) ($opData['amount'] ?? 0) - (float) $data['amount']) < 0.01;
            });

        if ($hasPendingRequest) {
            return back()->withErrors(['amount' => 'Nhân viên này đã có đề xuất thưởng đang chờ Chủ doanh nghiệp phê duyệt.']);
        }

        $approval = app(ApprovalService::class)->submitRequest('employee_bonus', [
            'employee_id' => $employee->id,
        ...
        AuditLog::log('bonus_requested', 'updated', $employee, null, [
            ...
            'is_sensitive' => true,
            'alert_level' => 'warning',
            'warning_message' => "Quản lý {$user->name} vừa gửi đề xuất thưởng ".number_format((float) $data['amount'], 0, ',', '.')." ₫ cho nhân viên {$employee->full_name} ({$employee->employee_code}) — Cần Chủ doanh nghiệp phê duyệt.",
        ]);

        return back()->with('success', 'Đã gửi đề xuất thưởng nhân viên đến Chủ doanh nghiệp để phê duyệt.');
    }
    ...
    public function syncShifts(Request $request): RedirectResponse
    {
        ...
        foreach ($data['shifts'] as $index => $s) {
            ...
            if ($shift) {
                ...
                if ($timeChanged) {
                    $hasAttendance = ScheduleAssignment::where('work_shift_id', $shift->id)
                        ->where('status', 'completed')
                        ->exists();
                    if ($hasAttendance) {
                        throw ValidationException::withMessages([
    ...
    private function assertRoleAssignmentAllowed(User $user, string $role, ?string $existingRole = null): void
    {
        ...
        if (! $isOwnerOrAdmin) {
            if ($user->hasRole('warehouse_manager')) {
                $allowedWarehouseRoles = ['warehouse_staff'];
                if (! in_array($role, $allowedWarehouseRoles, true) && $role !== $existingRole) {
                    throw ValidationException::withMessages([
                        'role' => 'Tài khoản Trưởng kho tổng chỉ được phép tạo hoặc quản lý Nhân viên Kho Tổng. Không được phép tạo tài khoản Quản lý hoặc nhân viên chi nhánh khác.',
                    ]);
                }
