...
Route::middleware(['auth', 'verified', 'tenant.subscription', 'tenant.ratelimit'])->group(function () {
    ...
    // Quản lý & Điều phối Giao hàng (Smart Routing & Dispatch)
    Route::prefix('delivery')->name('delivery.')->group(function () {
        Route::get('/', [\App\Http\Controllers\Delivery\DeliveryManagementController::class, 'index'])->name('index');
        Route::get('api/unassigned-orders', [\App\Http\Controllers\Delivery\DeliveryManagementController::class, 'unassignedOrders'])->name('unassigned-orders');
        Route::get('api/active-shippers', [\App\Http\Controllers\Delivery\DeliveryManagementController::class, 'activeShippers'])->name('active-shippers');
        Route::post('api/optimize-route', [\App\Http\Controllers\Delivery\DeliveryManagementController::class, 'optimizeRoute'])->name('optimize-route');
        Route::post('api/suggest-shippers', [\App\Http\Controllers\Delivery\DeliveryManagementController::class, 'suggestShippers'])->name('suggest-shippers');
        Route::post('api/batches', [\App\Http\Controllers\Delivery\DeliveryManagementController::class, 'createBatch'])->name('batches.store');
        Route::patch('api/batches/{batch}/dispatch', [\App\Http\Controllers\Delivery\DeliveryManagementController::class, 'dispatchBatch'])->name('batches.dispatch');
        Route::patch('api/batches/{batch}/complete', [\App\Http\Controllers\Delivery\DeliveryManagementController::class, 'completeBatch'])->name('batches.complete');
        Route::patch('api/batches/{batch}/cancel', [\App\Http\Controllers\Delivery\DeliveryManagementController::class, 'cancelBatch'])->name('batches.cancel');
        Route::get('api/stats', [\App\Http\Controllers\Delivery\DeliveryManagementController::class, 'stats'])->name('stats');
        Route::get('api/suggest-batches', [\App\Http\Controllers\Delivery\DeliveryManagementController::class, 'suggestBatches'])->name('suggest-batches');
    ...
    // Ứng dụng PWA cho Shipper
    Route::prefix('shipper')->name('shipper.')->group(function () {
        Route::get('app', [\App\Http\Controllers\Delivery\ShipperPwaController::class, 'app'])->name('app');
        Route::post('location', [\App\Http\Controllers\Delivery\ShipperPwaController::class, 'updateLocation'])
            ->middleware('throttle:60,1')
            ->name('location');
        Route::post('location-batch', [\App\Http\Controllers\Delivery\ShipperPwaController::class, 'updateLocationBatch'])
            ->middleware('throttle:30,1')
            ->name('location-batch');
        Route::patch('batch-items/{item}/status', [\App\Http\Controllers\Delivery\ShipperPwaController::class, 'updateItemStatus'])->name('item-status');
    });
});
