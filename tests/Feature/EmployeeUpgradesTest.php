...
class EmployeeUpgradesTest extends TestCase
{
    ...
    public function test_vietqr_webhook_queues_post_payment_processing()
    {
        ...
        // (no signature verification — anyone could mark any order as paid). Order-level
        // bank-transfer confirmation now goes through the same signed webhook as
        // subscription billing; see Billing\PaymentWebhookController.
        $secret = 'test-billing-webhook-secret';
        config(['billing.webhook_secret' => $secret]);
        ...
        $body = json_encode(['content' => "AVTORD{$order->id} chuyen khoan"]);
