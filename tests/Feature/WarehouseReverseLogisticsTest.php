...
class WarehouseReverseLogisticsTest extends TestCase
{
    ...
    public function test_damaged_transfer_only_posts_good_quantity_and_can_be_returned(): void
    {
        ...
        $this->actingAs($warehouseManager)->post("/inventory/transfers/{$transfer->id}/dispatch", ['quantity_dispatched' => 30])->assertRedirect();

        $this->actingAs($owner)->post("/inventory/transfers/{$transfer->id}/receive", [
    ...
    public function test_central_supply_damage_is_quarantined_and_only_good_quantity_enters_branch_stock(): void
    {
        ...
        $quarantine = InventoryQuarantine::where('source_type', 'supply_request')
            ->where('source_id', $dispatched->id)
            ->firstOrFail();
        $return = $this->actingAs($branchManager)->postJson(route('warehouse.reverse-logistics.quarantines.return', $quarantine->id), [
            'reason' => 'HoÃ n hÃ ng há»ng vá» Kho Tá»•ng.',
        ])->assertCreated()->json('return');
        $this->assertEquals($central->id, $return['to_branch_id']);

        $this->actingAs($owner)->postJson(route('warehouse.reverse-logistics.returns.approve', $return['id']))->assertOk();
        $this->actingAs($owner)->postJson(route('warehouse.reverse-logistics.returns.complete', $return['id']), [
            'disposition' => 'central_quarantine',
            'notes' => 'ÄÃ£ nháº­n láº¡i táº¡i Kho Tá»•ng vÃ  chuyá»ƒn vÃ o cÃ¡ch ly.',
        ])->assertOk();

        $this->assertSame('returned', $dispatched->refresh()->status);
        $this->assertSame('returned', $quarantine->refresh()->status);
        $this->assertDatabaseHas('inventory_quarantines', [
            'source_type' => 'inventory_return',
            'source_id' => $return['id'],
    ...
    public function test_destroy_quarantine_only_consumes_the_quarantined_quantity(): void
    {
        Storage::fake('local');
        Role::firstOrCreate(['name' => 'owner', 'guard_name' => 'web']);
        ...
        $ingredient = Ingredient::create([
            'restaurant_id' => $restaurant->id,
            'unit_id' => $unit->id,
            'name' => 'Nguyên liệu tiêu hủy theo phần',
            'sku' => 'REV-DISPOSAL',
            'average_cost' => 100,
            'status' => 'active',
        ]);
        ...
        $quarantine = InventoryQuarantine::create([
            ...
            'condition' => 'damaged',
            'status' => 'open',
            'reason' => 'Hàng hỏng cần tiêu hủy theo biên bản.',
            'created_by' => $owner->id,
        ]);

        $this->actingAs($owner)->postJson(route('warehouse.reverse-logistics.quarantines.destroy', $quarantine->id), [
            'reason' => 'Đã tiêu hủy 10 kg theo biên bản.',
            'evidence' => UploadedFile::fake()->image('disposal.jpg'),
        ])->assertOk();

        $this->assertSame('destroyed', $quarantine->refresh()->status);
        $this->assertEqualsWithDelta(4, (float) $batch->refresh()->quantity_remaining, 0.001);
        $this->assertSame('locked', $batch->status);
    }
