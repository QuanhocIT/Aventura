...
class InventoryBatchFefoTest extends TestCase
{
    ...
    public function test_orders_consume_batches_by_expiry_and_refunds_restore_the_same_lots(): void
    {
        ...
        // POS cố ý KHÔNG chặn bán khi thiếu tồn: doanh số vẫn được ghi nhận,
        // tồn kho được để âm và đẩy sang quy trình xử lý âm nguyên liệu
        // (NegativeInventoryService). Xem chú thích trong
        ...
        // Cần 8, chỉ còn 7: vét sạch cả hai lô rồi âm 1.
        $this->assertEquals(-1.0, (float) $inventory->quantity_on_hand);
        $this->assertEquals(0.0, (float) $oldBatch->quantity_remaining);
        $this->assertEquals(0.0, (float) $newBatch->quantity_remaining);
        $this->assertEquals('depleted', $newBatch->status);

        // Phần thiếu phải được ghi vết trên giao dịch xuất kho...
        $usageTransaction = InventoryTransaction::where('order_id', $insufficientOrder->id)
            ->where('type', 'usage')
            ->firstOrFail();
        $this->assertStringContainsString('Thiếu ghi nhận', $usageTransaction->notes);

        // ...và mở hồ sơ âm kho để người phụ trách xử lý.
        $case = InventoryNegativeCase::where('inventory_id', $inventory->id)->first();
        $this->assertNotNull($case, 'Tồn kho âm phải sinh hồ sơ xử lý âm nguyên liệu.');
        $this->assertEquals(1.0, (float) $case->negative_quantity);
    }
}
