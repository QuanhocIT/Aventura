...
const selectedBranchId = ref<number | null>(
    props.branches[0]?.branch_id ?? null,
);
...
const selectedBranch = computed(
    () =>
        props.branches.find(
            (branch) => branch.branch_id === selectedBranchId.value,
        ) ?? null,
);
...
function branchStatus(branch: BranchRow): {
    label: string;
    className: string;
    iconClass: string;
} {
...
const stats = computed(() => {
    const configured = props.branches.filter(
        (branch) => branch.budget_amount !== null,
    );
    const budgetTotal = configured.reduce(
        (sum, branch) => sum + (branch.budget_amount ?? 0),
        0,
    );
    const committedTotal = props.branches.reduce(
        (sum, branch) => sum + branch.committed,
        0,
    );
    const remainingTotal = configured.reduce(
        (sum, branch) => sum + (branch.remaining ?? 0),
        0,
    );
    const overCount = props.branches.filter(
        (branch) => branch.over_budget,
    ).length;
    const nearLimitCount = configured.filter(
        (branch) =>
            !branch.over_budget &&
            (branch.budget_amount ?? 0) > 0 &&
            usagePercent(branch) >= 90,
    ).length;
...
const filteredBranches = computed(() => {
    ...
    return props.branches.filter((branch) => {
        ...
        const matchesStatus =
            ...
            branchStatusKey(branch) === statusFilter.value;
...
const recommendedBudget = computed(() => {
    ...
    return (
        Math.ceil((selectedBranch.value.committed * 1.1) / 100_000) * 100_000
    );
});
...
function saveBudget() {
    ...
    budgetForm.post('/payroll-budget/budget', {
        ...
        onError: (errors: Record<string, string>) =>
            toast.error(
                String(Object.values(errors)[0] ?? 'Không thể lưu quỹ lương.'),
            ),
    });
}

function monthlyTierAmount(
    tier: Pick<WageTier, 'compensation_type' | 'rate'>,
): number {
...
function branchName(branchId: number | null): string {
    return branchId
        ? (props.branches.find((branch) => branch.branch_id === branchId)
...
function submitTier() {
    const options = {
        ...
        onError: (errors: Record<string, string>) =>
            toast.error(
                String(Object.values(errors)[0] ?? 'Không thể lưu bậc lương.'),
            ),
    };

    if (editingTierId.value) {
        tierForm.put(
            `/payroll-budget/wage-tiers/${editingTierId.value}`,
            options,
        );
    } else {
...
function toggleTier(tier: WageTier) {
    router.patch(
        `/payroll-budget/wage-tiers/${tier.id}/toggle`,
        {},
        {
            preserveScroll: true,
            onSuccess: () =>
                toast.success(
                    tier.is_active
                        ? 'Đã tạm dừng bậc lương.'
                        : 'Đã kích hoạt bậc lương.',
                ),
        },
    );
}
...
async function removeTier(tier: WageTier) {
    if (
        await confirmDialog({
            title: 'Xóa bậc lương',
            description: `Bạn chắc chắn muốn xóa “${tier.name}”?`,
            variant: 'destructive',
        })
    ) {
...
<template>
    ...
    <div class="mx-auto w-full max-w-7xl space-y-6 p-4 sm:p-6 lg:space-y-8">
        <header
            class="flex flex-col gap-4 border-b border-border/70 pb-6 lg:flex-row lg:items-end lg:justify-between"
        >
            <div>
                <div
                    class="mb-3 flex flex-wrap items-center gap-2 text-xs font-semibold tracking-[0.18em] text-primary uppercase"
                >
                    <span
                        class="rounded-full border border-primary/30 bg-primary/10 px-2.5 py-1"
                        >Chủ doanh nghiệp</span
                    >
                    <span class="text-muted-foreground"
                        >Kiểm soát tài chính nhân sự</span
                    >
                </div>
                ...
                >
                    Cấp hạn mức, kiểm soát tổng lương nhân viên và quy định bậc
                    lương trước khi quản lý tuyển người. Kỳ áp dụng:
                ...
                <div class="flex items-start justify-between gap-3">
                    <div>
                        ...
                        <p class="mt-3 text-2xl font-bold tabular-nums">
                            {{ compactVnd(stats.budgetTotal) }}
                        </p>
                        <p class="mt-1 text-xs text-muted-foreground">
                            {{ stats.configuredCount }}/{{ branches.length }}
                            chi nhánh
                        </p>
                    </div>
            ...
            >
                <div class="flex items-start justify-between gap-3">
                    <div>
                        ...
                        <p class="mt-3 text-2xl font-bold tabular-nums">
                            {{ compactVnd(stats.committedTotal) }}
                        </p>
                        <p class="mt-1 text-xs text-muted-foreground">
                ...
                <div class="flex items-start justify-between gap-3">
                    <div>
                        ...
                        <p
                            ...
                            "
                        >
                            {{ compactVnd(stats.remainingTotal) }}
                        </p>
                        <p class="mt-1 text-xs text-muted-foreground">
                            Chỉ tính chi nhánh đã cấp quỹ
                        </p>
                    </div>
                    <div
                        class="rounded-xl bg-emerald-500/10 p-2.5 text-emerald-400"
                    >
            ...
            >
                <div class="flex items-start justify-between gap-3">
                    <div>
                        ...
                        <p class="mt-1 text-xs text-muted-foreground">
                            {{ stats.overCount }} vượt ·
                            {{ stats.nearLimitCount }} sắp chạm ·
                            {{ stats.unconfiguredCount }} chưa cấp
                        </p>
                    </div>
                    <div
                        class="rounded-xl bg-amber-500/10 p-2.5 text-amber-400"
                    >
        ...
        >
            ...
            >
                <div>
                    ...
                    <p class="mt-1 text-xs text-muted-foreground">
                        “Tổng lương/tháng” là chi phí lương dự kiến của nhân
                        viên đang hoạt động.
                    </p>
                </div>
                <div class="flex flex-col gap-2 sm:flex-row">
                    <label class="relative block">
                        ...
                        <input
                            ...
                            class="h-9 w-full rounded-lg border border-border bg-background pr-3 pl-9 text-sm transition outline-none focus:border-primary sm:w-56"
                        />
                    </label>
                    <label class="relative block">
                        <Filter
                        ...
                        <select
                            ...
                            class="h-9 w-full appearance-none rounded-lg border border-border bg-background pr-8 pl-9 text-sm transition outline-none focus:border-primary sm:w-44"
                        >
                ...
                <table class="w-full min-w-[1080px] text-sm">
                    ...
                    >
                        <tr class="border-b border-border/70">
                            ...
                            <th class="w-56 px-5 py-3 font-semibold">
                                Mức sử dụng
                            </th>
                            <th class="px-5 py-3 text-right font-semibold">
                                Còn lại
                    ...
                    <tbody>
                        ...
                        >
                            <td class="px-5 py-4">
                                ...
                                <div
                                    v-if="branch.branch_code"
                                    class="mt-1 text-xs text-muted-foreground"
                                >
                                    Mã {{ branch.branch_code }}
                                </div>
                            </td>
                            <td
                            ...
                            >
                                {{
                                    branch.budget_amount === null
                                        ? 'Chưa cấp'
                                        : vnd(branch.budget_amount)
                                }}
                            </td>
                            <td class="px-5 py-4 text-right tabular-nums">
                                {{ vnd(branch.committed) }}
                            </td>
                            <td class="px-5 py-4">
                                <div
                                    v-if="branch.budget_amount !== null"
                                    class="space-y-1.5"
                                >
                                    <div
                                    ...
                                    >
                                        <span class="text-muted-foreground"
                                            >{{ usagePercent(branch) }}%</span
                                        >
                                        <span class="text-muted-foreground">{{
                                            compactVnd(branch.budget_amount)
                                        }}</span>
                                    </div>
                                    <div
                                    ...
                                    >
                                        <div
                                            ...
                                            "
                                            :style="{
                                                width: `${Math.min(100, usagePercent(branch))}%`,
                                            }"
                                        />
                                    </div>
                                </div>
                            ...
                            >
                                {{
                                    branch.remaining === null
                                        ? '—'
                                        : branch.over_budget
                                          ? `Vượt ${vnd(Math.abs(branch.remaining))}`
                                          : vnd(branch.remaining)
                                }}
                            </td>
                            <td class="px-5 py-4">
                                <span
                                    class="inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-semibold"
                                    :class="branchStatus(branch).className"
                                >
                                    <span
                                        class="size-1.5 rounded-full bg-current"
                                        :class="branchStatus(branch).iconClass"
                                    />
                            ...
                            <td class="px-5 py-4 text-right">
                                <button
                                    type="button"
                                    class="inline-flex cursor-pointer items-center gap-1 text-xs font-semibold text-primary hover:underline"
                                    @click.stop="openAdjustBudgetModal(branch)"
                                >
                    ...
                    <div class="mt-6 grid grid-cols-2 gap-3">
                        ...
                        <div class="rounded-xl bg-muted/35 p-3">
                            ...
                            <p
                                ...
                                :class="
                                    selectedBranch.over_budget
                                        ? 'text-rose-400'
                                        : 'text-emerald-400'
                                "
                            >
                                {{
                                    selectedBranch.budget_amount === null
                                        ? 'Chưa cấp'
                                        : `${usagePercent(selectedBranch)}%`
                                }}
                            </p>
                        </div>
                    </div>
                    <div
                        class="mt-5 flex items-start gap-2 rounded-xl border border-blue-500/20 bg-blue-500/5 p-3 text-xs leading-5 text-muted-foreground"
                    >
                        <CircleHelp
                            class="mt-0.5 size-4 shrink-0 text-blue-400"
                        />
                        <span
                            >Quỹ cấp phải luôn lớn hơn hoặc bằng tổng lương hiện
                            tại. Khi quỹ đã được cấp, hệ thống sẽ chặn việc thêm
                            nhân sự làm vượt hạn mức.</span
                        >
                    </div>
                </template>
                <p v-else class="mt-8 text-sm text-muted-foreground">
                    Hãy chọn một chi nhánh trong bảng để xem chi tiết.
                </p>
            </div>
                ...
                <form class="mt-5 space-y-4" @submit.prevent="saveBudget">
                    <div class="grid gap-4 sm:grid-cols-2">
                        <label class="block text-sm font-medium">
                            ...
                            <select
                                ...
                                class="mt-1.5 block h-10 w-full rounded-lg border border-border bg-background px-3 text-sm outline-none focus:border-primary"
                                @change="
                                    selectBranch(
                                        branches.find(
                                            (branch) =>
                                                branch.branch_id ===
                                                budgetForm.branch_id,
                                        )!,
                                    )
                                "
                            >
                        ...
                        <label class="block text-sm font-medium">
                            ...
                            <input
                                ...
                                min="0"
                                step="100000"
                                class="mt-1.5 block h-10 w-full rounded-lg border border-border bg-background px-3 text-sm tabular-nums outline-none focus:border-primary"
                            />
                        </label>
                    </div>
                    <div
                        class="flex flex-wrap items-center justify-between gap-2 rounded-lg bg-muted/30 px-3 py-2.5 text-xs"
                    >
                        <span class="text-muted-foreground"
                            >Gợi ý an toàn: tổng lương + 10% dự phòng</span
                        >
                        <button
                            type="button"
                    ...
                    <label class="block text-sm font-medium">
                        ...
                        <textarea
                            ...
                            rows="2"
                            maxlength="500"
                            placeholder="Ví dụ: đã duyệt theo kế hoạch vận hành tháng này"
                            class="mt-1.5 block w-full resize-none rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary"
                        />
                    </label>
                    <p
                    ...
                    <div
                        class="flex items-center justify-end gap-3 border-t border-border/70 pt-4"
                    >
                        <span
                            v-if="selectedBranch?.budget_amount !== null"
                            class="mr-auto text-xs text-muted-foreground"
                            >Đang cập nhật quỹ {{ month }}</span
                        >
                        <button
                            type="submit"
                            :disabled="
                                budgetForm.processing || !budgetForm.branch_id
                            "
                            class="inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50"
                        >
        ...
        >
            <div
                class="flex flex-col gap-3 border-b border-border/70 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6"
            >
                <div>
                    <div class="flex items-center gap-2">
                    ...
                    <p class="mt-1 text-xs text-muted-foreground">
                        Quản lý chỉ được chọn bậc đang hoạt động khi tạo hoặc
                ...
                >
                    <span
                        class="rounded-full bg-emerald-500/10 px-2.5 py-1 font-semibold text-emerald-400"
                        >{{ activeTierCount }} đang hoạt động</span
                    >
            ...
            <div class="grid lg:grid-cols-[1.15fr_0.85fr]">
                        ...
                        <tbody>
                            ...
                            >
                                <td class="px-5 py-4">
                                    ...
                                    >
                                        ...
                                        >
                                        <span
                                            v-if="tier.revenue_percent"
                                            class="inline-flex items-center rounded-full bg-emerald-500/10 px-2 py-0.5 font-semibold text-emerald-600 dark:text-emerald-400"
                                        >
                                            + {{ tier.revenue_percent }}% doanh
                                            thu
                                        </span>
                                    </div>
                                </td>
                                ...
                                <td class="px-5 py-4">
                                    <div class="flex justify-end gap-1">
                                        ...
                                        <button
                                            type="button"
                                            class="rounded-md p-1.5 text-rose-400 transition hover:bg-rose-500/10"
                                            title="Xóa"
                            ...
                            <tr v-if="!wageTiers.length">
                                <td
                                    ...
                                    class="px-5 py-12 text-center text-sm text-muted-foreground"
                                >
                                    Chưa có bậc lương nào được thiết lập.
                                </td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <div class="p-5 sm:p-6">
                    ...
                    <p class="mt-1 text-xs leading-5 text-muted-foreground">
                        Mức theo giờ quy đổi
                        {{ payrollRules.hours_per_month }}h/tháng; theo ca quy
                        đổi {{ payrollRules.shifts_per_month }} ca/tháng.
                    </p>
                    <form class="mt-5 space-y-3" @submit.prevent="submitTier">
                        <label class="block text-sm font-medium">
                            ...
                            <input
                                ...
                                type="text"
                                maxlength="120"
                                placeholder="Ví dụ: Phục vụ ca ngày"
                                class="mt-1.5 block h-10 w-full rounded-lg border border-border bg-background px-3 text-sm outline-none focus:border-primary"
                            />
                        </label>
                        <div class="grid gap-3 sm:grid-cols-3">
                            <label class="block text-sm font-medium">
                                Hình thức
                                <select
                                    v-model="tierForm.compensation_type"
                                    class="mt-1.5 block h-10 w-full rounded-lg border border-border bg-background px-3 text-sm outline-none focus:border-primary"
                                >
                            ...
                            <label class="block text-sm font-medium">
                                Mức (đ)
                                <input
                                    v-model.number="tierForm.rate"
                                    type="number"
                                    min="0"
                                    step="1000"
                                    class="mt-1.5 block h-10 w-full rounded-lg border border-border bg-background px-3 py-2 text-sm tabular-nums outline-none focus:border-primary"
                                />
                            </label>
                            <label class="block text-sm font-medium">
                                % Doanh thu
                                ...
                                <div class="relative mt-1.5">
                                    ...
                                    />
                                    <span
                                        class="absolute top-2.5 right-2.5 text-xs font-semibold text-muted-foreground"
                                        >%</span
                                    >
                                </div>
                            </label>
                        ...
                        >
                            ...
                            <span class="font-bold text-foreground"
                                >{{
                                    vnd(monthlyTierAmount(tierForm))
                                }}/tháng</span
                            >
                        </div>
                        <div class="flex justify-end gap-2 pt-2">
                            <button
                                v-if="editingTierId"
                                type="button"
                                class="rounded-lg border border-border px-3 py-2 text-sm font-semibold hover:bg-accent"
                                @click="resetTierForm"
                            >
                                Hủy
                            </button>
                            <button
                                ...
                                class="inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground transition hover:opacity-90 disabled:opacity-50"
                            >
        ...
        <div
            class="flex items-start gap-3 rounded-xl border border-amber-500/20 bg-amber-500/5 p-4 text-xs leading-5 text-muted-foreground"
        >
            <ArrowUpRight class="mt-0.5 size-4 shrink-0 text-amber-400" />
            <p>
                <span class="font-semibold text-foreground"
                    >Nguyên tắc kiểm soát:</span
                >
                Quỹ do chủ doanh nghiệp cấp. Quản lý chỉ được chọn bậc lương đã
                phê duyệt và không thể thêm nhân sự nếu tổng lương mới vượt phần
                quỹ còn lại.
            </p>
        </div>
            ...
            <DialogContent class="sm:max-w-[500px]">
                <DialogHeader>
                    ...
                    <DialogDescription class="text-xs text-muted-foreground">
                        Kỳ áp dụng tháng
                        <strong class="text-foreground">{{ month }}</strong
                        >. Cấp hạn mức ngân sách chi trả lương cho chi nhánh.
                    </DialogDescription>
                </DialogHeader>
                ...
                <form class="mt-2 space-y-4" @submit.prevent="saveBudget">
                    <div class="space-y-1.5 rounded-xl bg-muted/40 p-3 text-xs">
                        <div class="flex justify-between">
                            <span class="text-muted-foreground"
                                >Chi phí lương hiện tại (dự kiến):</span
                            >
                            <span class="font-bold text-foreground">{{
                                selectedBranch
                                    ? vnd(selectedBranch.committed)
                        ...
                        <div class="flex justify-between">
                            <span class="text-muted-foreground"
                                >Hạn mức hiện tại:</span
                            >
                            <span class="font-semibold">{{
                                selectedBranch &&
                                selectedBranch.budget_amount !== null
                                    ? vnd(selectedBranch.budget_amount)
                                    : 'Chưa cấp'
                            }}</span>
                        </div>
                    </div>
                    ...
                    <div
                        class="flex items-center justify-between gap-2 rounded-lg border border-primary/20 bg-primary/5 px-3 py-2 text-xs"
                    >
                        <span class="text-muted-foreground"
                            >Gợi ý an toàn (+10% dự phòng):</span
                        >
                        <button
                            type="button"
