...
const initialInspectionId =
    Number(initialQuery?.get('inspection_id') || 0) || null;
const initialBranchId = Number(initialQuery?.get('branch_id') || 0) || null;
const initialPlanId =
    Number(initialQuery?.get('inspection_plan_id') || 0) || null;
...
const submitReport = async () => {
    ...
    try {
        ...
        if (reportForm.value.inspection_plan_id) {
            payload.append(
                'inspection_plan_id',
                String(reportForm.value.inspection_plan_id),
            );
        }
        ...
        if (reportForm.value.operational_inspection_id) {
            payload.append(
                ...
                String(reportForm.value.operational_inspection_id),
            );
        }
        ...
        if (reportForm.value.remediation_plan.trim()) {
            payload.append(
                'remediation_plan',
                reportForm.value.remediation_plan,
            );
        }
...
const submitPlan = async () => {
    if (
        !planForm.value.title.trim() ||
        planForm.value.scope.trim().length < 10
    ) {
    ...
    try {
        const res = await axios.post(
            '/api/operational-audit/inspection-plans',
            planForm.value,
        );
        ...
        toast.error(
            e.response?.data?.message || 'Không thể tạo kế hoạch kiểm tra.',
        );
    } finally {
        isProcessing.value = false;
    }
...
const updatePlanStatus = async (
    ...
    if (action === 'complete') {
        const notes = window.prompt(
            'Tóm tắt kết quả và các tồn tại cần theo dõi:',
            plan.notes || '',
        );
    ...
    try {
        const res = await axios.post(
            `/api/operational-audit/inspection-plans/${plan.id}/${action}`,
            body,
        );
        ...
        toast.error(
            e.response?.data?.message || 'Không thể cập nhật kế hoạch.',
        );
    } finally {
        isProcessing.value = false;
    }
    ...
    try {
        const res = await axios.post(
            `/api/operational-audit/reports/${selectedReport.value.id}/assign`,
            {
                ...
                remediation_deadline:
                    remediationForm.value.remediation_deadline,
...
const acceptAssignment = async () => {
    ...
    try {
        const res = await axios.post(
            `/api/operational-audit/reports/${selectedReport.value.id}/assignment/accept`,
        );
        toast.success(res.data.message || 'Đã nhận việc.');
        router.reload();
    } catch (e: any) {
        toast.error(
            e.response?.data?.message || 'Không thể xác nhận nhận việc.',
        );
    }
};
...
const rejectAssignment = async () => {
    ...
    const reason = window.prompt(
        'Lý do từ chối nhận việc (bắt buộc):',
        'Không thuộc phạm vi phụ trách hoặc thiếu nguồn lực.',
    );
    ...
    try {
        const res = await axios.post(
            `/api/operational-audit/reports/${selectedReport.value.id}/assignment/reject`,
            { reason },
        );
        ...
        toast.error(
            e.response?.data?.message || 'Không thể từ chối nhận việc.',
        );
    }
};
...
const acknowledgeReport = async () => {
    ...
    const responseText = window.prompt(
        'Phản hồi của chi nhánh và cam kết xử lý:',
        'Đã tiếp nhận, sẽ xử lý theo hạn SLA và cập nhật bằng chứng.',
    );
    ...
    try {
        const res = await axios.post(
            `/api/operational-audit/reports/${selectedReport.value.id}/acknowledge`,
...
const submitRemediation = async () => {
    if (
        ...
        !remediationForm.value.remediation_notes.trim()
    ) {
    ...
    try {
        ...
        payload.append(
            'remediation_notes',
            remediationForm.value.remediation_notes,
        );

        if (remediationForm.value.remediation_proof) {
            payload.append(
                'remediation_proof',
                remediationForm.value.remediation_proof,
            );
        }
        ...
        if (res.data.success) {
            toast.success(
                'Đã nộp khắc phục và chuyển hồ sơ sang chờ tái kiểm.',
            );
            router.reload();
        }
    } catch (e: any) {
        toast.error(
            e.response?.data?.message || 'Không thể nộp kết quả khắc phục.',
        );
    } finally {
        isProcessing.value = false;
    }
...
const submitReinspection = async () => {
    ...
    try {
        ...
        const res = await axios.post(
            `/api/operational-audit/reports/${selectedReport.value.id}/reinspect`,
            payload,
            { headers: { 'Content-Type': 'multipart/form-data' } },
        );
        ...
        toast.error(
            e.response?.data?.message || 'Không thể ghi nhận kết quả tái kiểm.',
        );
    } finally {
        isProcessing.value = false;
    }
...
const filteredReports = computed(() => {
    ...
    return props.reports.filter((report) => {
        ...
        const matchesSeverity =
            ...
            report.severity_level === severityFilter.value;
        const matchesBranch =
            branchFilter.value === 'all' ||
            String(report.branch?.id) === branchFilter.value;

        return (
            matchesQuery && matchesStatus && matchesSeverity && matchesBranch
        );
    });
});
...
const eligibleAssignees = computed(() => {
    ...
    return props.employees.filter(
        (employee) =>
            !branchId || Number(employee.branch_id) === Number(branchId),
    );
});
...
const availablePlansForReport = computed(() => {
    ...
    return (props.inspectionPlans || []).filter(
        (plan) =>
...
const activityForSelected = computed(() => {
    ...
    return (
        props.auditTrail?.[String(selectedReport.value.id)] ||
...
const totalPenaltyApproved = computed(() => {
    return Number(
        props.reportStats?.approved_penalty ??
            props.reports
                .filter((r) =>
                    [
                        'approved',
...
const trendMax = computed(() =>
    Math.max(1, ...(props.trend || []).map((item) => Number(item.total || 0))),
);
    ...
    <div class="mx-auto flex w-full max-w-7xl flex-col gap-6 p-4 md:p-6">
                    ...
                    <div>
                        ...
                        >
                            <template v-if="isOverview">
                                Tổng hợp rủi ro, xu hướng vi phạm và tiến độ
                                kiểm tra tại toàn bộ chi nhánh.
                            </template>
                            <template v-else>
                                Theo dõi vi phạm, SLA khắc phục, bằng chứng và
                                kết quả tái kiểm tại toàn bộ chi nhánh.
                            </template>
                        </p>
                    </div>
                </div>
        ...
        <section class="grid gap-4 lg:grid-cols-[1.35fr_1fr]">
            <Card class="overflow-hidden border-border/70">
                <CardHeader
                    class="flex flex-row items-start justify-between gap-4 border-b border-border/60 bg-muted/15"
                >
                    <div>
                        <p
                            class="text-[10px] font-bold tracking-[0.16em] text-rose-600 uppercase dark:text-rose-300"
                        >
                            Điểm nóng theo chi nhánh
                        </p>
                        <h2 class="mt-1 text-lg font-bold text-foreground">
                            Bảng điều hành rủi ro
                        </h2>
                        <CardDescription class="mt-1 text-xs"
                            >Ưu tiên xử lý dựa trên hồ sơ mở, quá SLA và mức độ
                            nghiêm trọng.</CardDescription
                        >
                    </div>
                    <BarChart3
                        class="mt-1 size-5 text-rose-600 dark:text-rose-300"
                    />
                </CardHeader>
                <CardContent class="p-0">
                    <div
                        v-if="!branchInsights?.length"
                        class="p-6 text-center text-xs text-muted-foreground"
                    >
                        Chưa có dữ liệu chi nhánh.
                    </div>
                    <div
                        v-for="branch in branchInsights"
                        :key="branch.id"
                        class="flex items-center gap-3 border-b border-border/50 px-5 py-3 last:border-0"
                    >
                        <div
                            class="flex size-9 shrink-0 items-center justify-center rounded-xl bg-muted text-xs font-black text-foreground"
                        >
                            {{ branch.risk_score }}
                        </div>
                        <div class="min-w-0 flex-1">
                            ...
                            >
                                ...
                                <span
                                    :class="[
                                        'text-[10px] font-bold',
                                        branch.risk_level === 'critical'
                                            ? 'text-rose-600 dark:text-rose-300'
                                            : branch.risk_level === 'warning'
                            ...
                            >
                                <div
                                    ...
                                    "
                                    :style="{
                                        width: `${Math.max(branch.risk_score, 4)}%`,
                                    }"
                                />
                            </div>
                            <p class="mt-1 text-[10px] text-muted-foreground">
                                {{ branch.open_reports }} hồ sơ mở ·
                                {{ branch.overdue_reports }} quá SLA ·
                                {{ branch.active_plans }} kế hoạch đang theo dõi
                            </p>
                        </div>
                    </div>
                </CardContent>
            ...
            <Card class="overflow-hidden border-border/70">
                <CardHeader class="border-b border-border/60 bg-muted/15">
                    <div class="flex items-center justify-between gap-3">
                        <div>
                            <p
                                class="text-[10px] font-bold tracking-[0.16em] text-indigo-600 uppercase dark:text-indigo-300"
                            >
                                Xu hướng
                            </p>
                            <h2 class="mt-1 text-lg font-bold text-foreground">
                        ...
                        <span
                            class="rounded-full bg-indigo-500/10 px-2.5 py-1 text-[10px] font-bold text-indigo-700 dark:text-indigo-300"
                            >{{ reportStats.total }} hồ sơ</span
                        >
                    </div>
                </CardHeader>
                <CardContent class="space-y-3 p-5">
                    <div
                        ...
                        :key="month.label"
                        class="grid grid-cols-[42px_1fr_28px] items-center gap-2 text-[10px]"
                    >
                        ...
                        <div class="h-2 overflow-hidden rounded-full bg-muted">
                            <div
                                ...
                                :style="{
                                    width: `${Math.max((month.total / trendMax) * 100, month.total ? 5 : 0)}%`,
                                }"
                            />
                        </div>
                    ...
                    <div
                        class="flex items-center gap-4 border-t border-border/60 pt-3 text-[10px] text-muted-foreground"
                    >
                        <span
                            ><i
                                class="mr-1 inline-block size-2 rounded-full bg-indigo-500"
                            />Tổng hồ sơ</span
                        ><span
                            ><i
                                class="mr-1 inline-block size-2 rounded-full bg-rose-500"
                            />Nghiêm trọng:
                            {{
                                trend.reduce(
                                    (sum, item) =>
                                        sum + Number(item.critical || 0),
                                    0,
                                )
                            }}</span
                        >
        ...
        <section
            v-if="canManagePlans || inspectionPlans?.length"
            class="overflow-hidden rounded-3xl border border-border/70 bg-card/90 shadow-sm"
        >
            <div
                class="flex flex-col gap-3 border-b border-border/60 bg-muted/15 px-5 py-4 sm:flex-row sm:items-center sm:justify-between md:px-6"
            >
                <div>
                    <div class="flex items-center gap-2">
                        <CalendarPlus
                            class="size-5 text-indigo-600 dark:text-indigo-300"
                        />
                    ...
                    <p class="mt-1 text-xs text-muted-foreground">
                        Chủ động lập phạm vi, đầu mối và thời hạn; biên bản phát
                        sinh sẽ được gom vào đúng kế hoạch.
                    </p>
                </div>
                <div class="flex flex-wrap items-center gap-2">
                    <span
                        class="rounded-full bg-amber-500/10 px-2.5 py-1 text-[10px] font-bold text-amber-700 dark:text-amber-300"
                        >{{ planStats.planned || 0 }} chờ thực hiện</span
                    >
                    <span
                        class="rounded-full bg-indigo-500/10 px-2.5 py-1 text-[10px] font-bold text-indigo-700 dark:text-indigo-300"
                        >{{ planStats.in_progress || 0 }} đang thực hiện</span
                    >
                    <Button
                        v-if="canManagePlans"
                        @click="openPlanModal"
                        size="sm"
                        class="h-8 gap-1.5 rounded-lg bg-indigo-600 text-xs font-bold text-white hover:bg-indigo-700"
                        ><CalendarPlus class="size-3.5" /> Tạo kế hoạch</Button
                    >
                </div>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full min-w-[980px] text-left text-xs">
                    ...
                    >
                        <tr>
                            <th class="px-5 py-3">Kế hoạch</th>
                            <th class="px-3 py-3">Phạm vi</th>
                            <th class="px-3 py-3">Lịch & đầu mối</th>
                            <th class="px-3 py-3">Hồ sơ</th>
                            <th class="px-3 py-3">Trạng thái</th>
                            <th class="px-5 py-3 text-right">Xử lý</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-border/50">
                        <tr v-if="!inspectionPlans?.length">
                            <td
                                colspan="6"
                                class="px-5 py-10 text-center text-muted-foreground"
                            >
                                Chưa có kế hoạch kiểm tra. Hãy lập kế hoạch định
                                kỳ đầu tiên.
                            </td>
                        </tr>
                        <tr
                        ...
                        >
                            <td class="px-5 py-3">
                                <p
                                    class="font-mono font-bold text-indigo-600 dark:text-indigo-300"
                                >
                                    {{ plan.plan_code }}
                                </p>
                                <p
                                    class="mt-1 max-w-[230px] truncate font-semibold text-foreground"
                                >
                                    {{ plan.title }}
                                </p>
                                <p
                                ...
                                >
                                    {{ getPlanTypeLabel(plan.inspection_type) }}
                                </p>
                            </td>
                            <td class="max-w-[220px] px-3 py-3">
                                <p class="font-semibold text-foreground">
                                    {{ plan.branch?.name || 'Toàn chuỗi' }}
                                </p>
                                <p
                            ...
                            <td class="px-3 py-3 text-muted-foreground">
                                <p>
                                    ...
                                    }}<span v-if="plan.due_date">
                                        → {{ formatDate(plan.due_date) }}</span
                                    >
                                </p>
                                <p class="mt-1 text-[10px]">
                                    {{
                                        plan.lead_inspector?.name ||
                                        'Chưa phân công'
                                    }}
                                </p>
                            </td>
                            <td class="px-3 py-3">
                                <p class="font-bold text-foreground">
                                    {{ plan.reports_count }} biên bản
                                </p>
                                <p
                                    v-if="plan.open_reports_count"
                                    class="mt-1 text-[10px] text-amber-600 dark:text-amber-300"
                                >
                                    {{ plan.open_reports_count }} đang mở
                                </p>
                            </td>
                            <td class="px-3 py-3">
                                <span
                                    :class="[
                                        'rounded-full border border-border bg-muted/30 px-2.5 py-1 text-[10px] font-bold',
                                        getPlanStatusMeta(plan.status)
                                            .className,
                                    ]"
                                    >{{
                                ...
                                >
                                <p
                                    v-if="plan.is_overdue"
                                    class="mt-1 text-[10px] font-bold text-rose-600 dark:text-rose-300"
                                >
                                    Quá hạn kế hoạch
                                </p>
                            </td>
                            <td class="px-5 py-3 text-right">
                                ...
                                >
                                    <Button
                                        ...
                                        @click="updatePlanStatus(plan, 'start')"
                                        size="sm"
                                        variant="outline"
                                        class="h-7 gap-1 rounded-lg px-2 text-[10px]"
                                        ><Play class="size-3" /> Bắt đầu</Button
                                    ><Button
                                        v-if="plan.status === 'in_progress'"
                                        @click="
                                            updatePlanStatus(plan, 'complete')
                                        "
                                        size="sm"
                                        class="h-7 gap-1 rounded-lg bg-emerald-600 px-2 text-[10px] font-bold text-white hover:bg-emerald-700"
                                        ><CheckCheck class="size-3" /> Hoàn
                                        tất</Button
                                    ><Button
                                        @click="
                                            updatePlanStatus(plan, 'cancel')
                                        "
                                        size="sm"
                                        variant="ghost"
                                        class="h-7 gap-1 rounded-lg px-2 text-[10px] text-rose-600 hover:bg-rose-500/10 dark:text-rose-300"
                                        ><Ban class="size-3" /> Hủy</Button
                                    >
                                </div>
                                <span
                                    v-else
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-7">
            ...
            <Card class="border-indigo-200/60 dark:border-indigo-500/20">
                ...
                >
                    <CardDescription
                        class="text-[11px] font-bold tracking-wider text-indigo-600 uppercase dark:text-indigo-300"
                        >Đang khắc phục</CardDescription
                    >
                    <UserCog
            ...
            <Card class="border-sky-200/60 dark:border-sky-500/20">
                ...
                >
                    <CardDescription
                        ...
                        >Chờ tái kiểm</CardDescription
                    >
                    <ClipboardCheck
                        class="size-4 text-sky-600 dark:text-sky-300"
                    />
                </CardHeader>
                <CardContent class="pb-5">
                    <p class="text-3xl font-black text-foreground">
                        {{ reportStats.reinspection_pending ?? 0 }}
                    </p>
                    <p class="mt-1 text-xs text-muted-foreground">
                        cần kết luận đạt / không đạt
                    </p>
                </CardContent>
            </Card>
            <Card class="border-red-200/70 dark:border-red-500/20">
                ...
                >
                    <CardDescription
                        class="text-[11px] font-bold tracking-wider text-red-600 uppercase dark:text-red-300"
                        >Quá SLA</CardDescription
                    >
                    <AlertOctagon
                        class="size-4 text-red-600 dark:text-red-300"
                    />
                </CardHeader>
                <CardContent class="pb-5">
                    <p class="text-3xl font-black text-foreground">
        ...
        >
            ...
            >
                <div>
                    ...
                    <p class="mt-1 text-xs text-muted-foreground">
                        Theo dõi xuyên suốt từ lập biên bản, phê duyệt, giao
                        khắc phục, nộp bằng chứng đến tái kiểm và đóng hồ sơ.
                    </p>
                </div>
                <div class="flex items-center gap-2">
                    <Button
                        ...
                        ><Download class="size-3.5" /> Xuất CSV</Button
                    >
            ...
            <div
                class="grid gap-3 border-b border-border/60 bg-muted/10 px-5 py-4 md:grid-cols-[1.5fr_1fr_1fr_1fr] md:px-6"
            >
                <div class="relative">
                    <Search
                    ...
                    <Input
                        ...
                        placeholder="Tìm mã biên bản, chi nhánh, người xử lý..."
                    />
                </div>
                <div class="flex items-center gap-2">
                    ...
                    <select
                        ...
                        class="h-9 w-full rounded-lg border border-input bg-background px-3 text-xs text-foreground"
                    >
                        <option value="all">Tất cả trạng thái</option>
                        <option value="pending_owner_approval">
                            Chờ Chủ duyệt
                        </option>
                        <option value="approved">Đã duyệt</option>
                        <option value="remediation_in_progress">
                            Đang khắc phục
                        </option>
                        <option value="reinspection_pending">
                            Chờ tái kiểm
                        </option>
                ...
                <select
                    ...
                    class="h-9 rounded-lg border border-input bg-background px-3 text-xs text-foreground"
                >
                    ...
                    <tbody class="divide-y divide-border/50">
                        ...
                        >
                            ...
                            >
                                ...
                                <p
                                    :class="[
                                        ...
                                        getSeverityMeta(r.severity_level)
                                            .className,
                                    ]"
                                >
                                    {{
                            ...
                            <td class="p-3">
                                ...
                                >
                                    {{ r.is_overdue ? 'Quá SLA · ' : 'Hạn '
                                    }}{{ formatDate(r.remediation_deadline) }}
                                </p>
                            </td>
                ...
                >
                    ...
                    >
                        <div class="flex items-center gap-2">
                            ...
                            <div>
                                ...
                                >
                                    Xác định rõ phạm vi, thời hạn và người chịu
                                    trách nhiệm.
                                </p>
                            </div>
                        </div>
                    ...
                    <div class="space-y-4 overflow-y-auto p-6 text-xs">
                        <div>
                            ...
                            ><Input
                                ...
                                placeholder="Ví dụ: Kiểm tra an toàn thực phẩm tháng 9"
                                class="text-xs"
                            />
                        </div>
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div>
                                <label
                                    ...
                                    >Loại kiểm tra</label
                                ><select
                                    v-model="planForm.inspection_type"
                                    class="w-full rounded-xl border border-input bg-background p-2.5 text-foreground"
                                >
                                    ...
                                    <option value="follow_up">Tái kiểm</option>
                                </select>
                            </div>
                            <div>
                                <label
                        ...
                        <div>
                            <label
                                class="mb-1.5 block font-semibold text-foreground"
                                >Đầu mối thanh tra</label
                            ><select
                                v-model="planForm.lead_inspector_id"
                                class="w-full rounded-xl border border-input bg-background p-2.5 text-foreground"
                            >
                                <option
                        ...
                        <div>
                            <label
                                class="mb-1.5 block font-semibold text-foreground"
                                >Phạm vi & tiêu chí kiểm tra (*)</label
                            ><textarea
                                v-model="planForm.scope"
                                rows="5"
                                placeholder="Nêu khu vực, ca làm, quy trình cần đối chiếu, hồ sơ cần thu thập và tiêu chí kết luận..."
                                class="min-h-28 w-full rounded-xl border border-input bg-background p-3 text-xs text-foreground outline-none placeholder:text-muted-foreground focus:ring-2 focus:ring-indigo-500/30"
                            ></textarea>
                        </div>
                    </div>
                    <div
                    ...
                    >
                        ...
                        ><Button
                            ...
                            ><CalendarPlus class="size-4" /> Lưu kế
                            hoạch</Button
                        >
                    </div>
                </div>
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        <div
                            v-if="reportForm.operational_inspection_id"
                            class="rounded-xl border border-indigo-200/70 bg-indigo-500/5 p-3 text-[11px] text-indigo-700 dark:border-indigo-500/30 dark:text-indigo-300"
                        >
                            Biên bản này được lập từ phiên kiểm tra hiện trường
                            <span class="font-mono font-bold"
                                >#{{
                                    reportForm.operational_inspection_id
                                }}</span
                            >; hệ thống sẽ liên kết hai hồ sơ để truy nguyên.
                        </div>
                        ...
                        <div>
                            ...
                            <div v-if="availablePlansForReport.length">
                                <label
                                    class="mb-1.5 block font-semibold text-foreground"
                                    >Gắn vào kế hoạch kiểm tra</label
                                >
                                <select
                                    v-model="reportForm.inspection_plan_id"
                                ...
                                >
                                    ...
                                    <option
                                        v-for="plan in availablePlansForReport"
                                        :key="plan.id"
                                        :value="plan.id"
                                    >
                                ...
                                <p
                                    class="mt-1 text-[10px] text-muted-foreground"
                                >
                                    Kế hoạch sẽ tự chuyển sang “Đang thực hiện”
                                    khi biên bản đầu tiên được gửi.
                                </p>
                            </div>
                        ...
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div>
                                <label
                                    class="mb-1.5 block font-semibold text-foreground"
                                    >Nhóm phát hiện</label
                                ><Input
                                    v-model="reportForm.finding_category"
                                    placeholder="ATTP, PCCC, tài sản, quy trình..."
                                    class="text-xs"
                                />
                            </div>
                            <div>
                                <label
                                    class="mb-1.5 block font-semibold text-foreground"
                                    >Điều khoản tham chiếu</label
                                ><Input
                                    v-model="reportForm.requirement_reference"
                                    placeholder="Mã quy định / mục checklist..."
                                    class="text-xs"
                                />
                            </div>
                            <div class="sm:col-span-2">
                                <label
                                    class="mb-1.5 block font-semibold text-foreground"
                                    >Hiện trạng quan sát</label
                                ><textarea
                                    v-model="reportForm.observed_condition"
                                    rows="2"
                                    placeholder="Ghi nhận khách quan tại hiện trường..."
                                    class="w-full rounded-xl border border-input bg-background p-3 text-xs text-foreground outline-none placeholder:text-muted-foreground focus:ring-2 focus:ring-rose-500/30"
                                ></textarea>
                            </div>
                            <div>
                                <label
                                    class="mb-1.5 block font-semibold text-foreground"
                                    >Nguyên nhân gốc</label
                                ><textarea
                                    v-model="reportForm.root_cause"
                                    rows="2"
                                    placeholder="Vì sao sai lệch xảy ra?"
                                    class="w-full rounded-xl border border-input bg-background p-3 text-xs text-foreground outline-none placeholder:text-muted-foreground focus:ring-2 focus:ring-rose-500/30"
                                ></textarea>
                            </div>
                            <div>
                                <label
                                    class="mb-1.5 block font-semibold text-foreground"
                                    >Hành động khắc phục</label
                                ><textarea
                                    v-model="reportForm.corrective_action"
                                    rows="2"
                                    placeholder="Việc cần làm để sửa lỗi..."
                                    class="w-full rounded-xl border border-input bg-background p-3 text-xs text-foreground outline-none placeholder:text-muted-foreground focus:ring-2 focus:ring-rose-500/30"
                                ></textarea>
                            </div>
                            <div class="sm:col-span-2">
                                <label
                                    class="mb-1.5 block font-semibold text-foreground"
                                    >Phòng ngừa tái diễn</label
                                ><textarea
                                    v-model="reportForm.preventive_action"
                                    rows="2"
                                    placeholder="Cập nhật quy trình, đào tạo, kiểm soát..."
                                    class="w-full rounded-xl border border-input bg-background p-3 text-xs text-foreground outline-none placeholder:text-muted-foreground focus:ring-2 focus:ring-rose-500/30"
                                ></textarea>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label
                                    class="mb-1.5 block font-semibold text-foreground"
                                    >Hạn khắc phục (SLA)</label
                                >
                                <Input
                                    v-model="reportForm.remediation_deadline"
                                    type="date"
                                    class="text-xs"
                                />
                            </div>
                            <div
                                class="flex items-end pb-2 text-[11px] text-muted-foreground"
                            >
                                Vi phạm nghiêm trọng bắt buộc có kế hoạch và
                        ...
                        <div>
                            <label
                                class="mb-1.5 block font-semibold text-foreground"
                                >Kế hoạch khắc phục ban đầu</label
                            >
                            <textarea
                    ...
                    >
                        ...
                        >
                            ...
                            <div v-if="selectedReport.inspection_plan">
                                ...
                                <span
                                    class="ml-1 font-mono font-bold text-indigo-600 dark:text-indigo-300"
                                    >{{
                                        selectedReport.inspection_plan.plan_code
                                    }}</span
                                >
                                <span class="ml-1"
                                    >·
                                    {{
                                        selectedReport.inspection_plan.title
                                    }}</span
                                >
                            </div>
                            <div>
                            ...
                            <div class="flex flex-wrap items-center gap-2 pt-1">
                                <span
                                    class="rounded-full border border-border bg-background px-2.5 py-1 text-[10px] font-bold"
                                    :class="
                                        getSeverityMeta(
                                ...
                                <span
                                    :class="[
                                        ...
                                        getStatusMeta(selectedReport.status)
                                            .className,
                                    ]"
                                >
                                    {{
                            ...
                            <div
                                v-if="
                                    ...
                                    selectedReport.remediation_deadline
                                "
                            >
                                <strong>Kiểm soát khắc phục:</strong>
                                {{
                                    selectedReport.assignee?.name ||
                                    'Chưa giao người phụ trách'
                                }}
                                <span
                                    v-if="selectedReport.assignment_status"
                                    class="ml-1 rounded-full bg-indigo-500/10 px-2 py-0.5 text-[10px] font-bold text-indigo-700 dark:text-indigo-300"
                                    >{{
                                        selectedReport.assignment_status
                                    }}</span
                                >
                                <span
                                    v-if="selectedReport.remediation_deadline"
                                    :class="
                        ...
                        <div
                            v-if="
                                selectedReport.assignment_status ===
                                    'assigned' &&
                                selectedReport.assignee?.id === currentUserId
                            "
                            class="flex flex-wrap items-center gap-2 rounded-2xl border border-amber-200/70 bg-amber-500/5 p-4 dark:border-amber-500/20"
                        >
                            <div
                                class="mr-auto text-xs text-amber-800 dark:text-amber-200"
                            >
                                Hồ sơ đang chờ bạn xác nhận nhận việc.
                            </div>
                            <Button
                                size="sm"
                        ...
                        <div
                            v-if="
                                canAcknowledge &&
                                !selectedReport.branch_acknowledged_at &&
                                selectedReport.status !== 'rejected'
                            "
                            class="flex flex-wrap items-center gap-3 rounded-2xl border border-amber-200/70 bg-amber-500/5 p-4 dark:border-amber-500/20"
                        >
                            <div
                                class="mr-auto text-xs text-amber-800 dark:text-amber-200"
                            >
                                Chi nhánh cần xác nhận đã tiếp nhận phát hiện và
                                nêu cam kết xử lý.
                            </div>
                            <Button
                                size="sm"
                                class="bg-amber-600 text-xs text-white hover:bg-amber-700"
                        ...
                        <div
                            v-if="selectedReport.remediation_plan"
                            class="rounded-2xl border border-indigo-200/70 bg-indigo-500/5 p-4 dark:border-indigo-500/20"
                        >
                            <div
                                class="flex items-center gap-2 font-semibold text-indigo-700 dark:text-indigo-300"
                            >
                                <CalendarClock class="size-4" /> Kế hoạch khắc
                                phục
                            </div>
                            <p
                                class="mt-2 leading-relaxed whitespace-pre-line text-foreground"
                            >
                                {{ selectedReport.remediation_plan }}
                            </p>
                        </div>

                        <div
                            v-if="selectedReport.remediation_notes"
                            class="rounded-2xl border border-emerald-200/70 bg-emerald-500/5 p-4 dark:border-emerald-500/20"
                        >
                            <div
                            ...
                            <p
                                class="mt-2 leading-relaxed whitespace-pre-line text-foreground"
                            >
                                {{ selectedReport.remediation_notes }}
                            </p>
                            <a
                                v-if="selectedReport.remediation_proof_url"
                                :href="selectedReport.remediation_proof_url"
                                target="_blank"
                                rel="noopener"
                                class="mt-2 inline-flex items-center gap-1 text-[11px] font-bold text-emerald-700 hover:underline dark:text-emerald-300"
                            >
                                <Upload class="size-3" /> Xem bằng chứng khắc
                                phục
                            </a>
                            <p
                                v-if="selectedReport.remediation_submitted_at"
                                class="mt-1 text-[10px] text-muted-foreground"
                            >
                                Nộp lúc
                                {{ selectedReport.remediation_submitted_at }}
                            </p>
                        </div>
                        ...
                        <div
                            ...
                            "
                            class="space-y-3 rounded-2xl border border-indigo-200/70 bg-indigo-500/5 p-4 dark:border-indigo-500/20"
                        >
                            <div
                                class="flex items-center gap-2 font-semibold text-indigo-700 dark:text-indigo-300"
                            >
                                <UserCog class="size-4" /> Giao khắc phục &
                                thiết lập SLA
                            </div>
                            <div class="grid gap-3 sm:grid-cols-2">
                                <div>
                                    ...
                                    >
                                    <select
                                        v-model="remediationForm.assigned_to"
                                        class="w-full rounded-xl border border-input bg-background p-2.5 text-xs text-foreground"
                                    >
                                        <option :value="null">
                                            -- Chưa chỉ định --
                                        </option>
                                        <option
                                            v-for="employee in eligibleAssignees"
                                            :key="employee.id"
                                            :value="employee.id"
                                        >
                                            {{ employee.name }} ·
                                ...
                                <div>
                                    ...
                                    <Input
                                        v-model="
                                            remediationForm.remediation_deadline
                                        "
                                        type="date"
                                        class="text-xs"
                                    />
                            ...
                            <textarea
                                v-model="remediationForm.remediation_plan"
                                rows="3"
                                placeholder="Kế hoạch, tiêu chí đạt và cách xác minh..."
                                class="min-h-20 w-full rounded-xl border border-input bg-background p-3 text-xs text-foreground outline-none placeholder:text-muted-foreground focus:ring-2 focus:ring-indigo-500/30"
                            ></textarea>
                            <Button
                        ...
                        <div
                            v-if="
                                selectedReport.can_submit_remediation &&
                                [
                                    'approved',
                                    'remediation_in_progress',
                                ].includes(selectedReport.status)
                            "
                            class="space-y-3 rounded-2xl border border-emerald-200/70 bg-emerald-500/5 p-4 dark:border-emerald-500/20"
                        >
                            <div
                                class="flex items-center gap-2 font-semibold text-emerald-700 dark:text-emerald-300"
                            >
                                <CheckCircle2 class="size-4" /> Nộp kết quả khắc
                                phục
                            </div>
                            <textarea
                                v-model="remediationForm.remediation_notes"
                                rows="3"
                                placeholder="Mô tả việc đã làm, kết quả đo được và phần còn tồn..."
                                class="min-h-20 w-full rounded-xl border border-input bg-background p-3 text-xs text-foreground outline-none placeholder:text-muted-foreground focus:ring-2 focus:ring-emerald-500/30"
                            ></textarea>
                            <input
                                type="file"
                                accept="image/*"
                                @change="onRemediationProofChange"
                                class="block w-full cursor-pointer rounded-xl border border-input bg-background p-2 text-xs text-foreground file:mr-3 file:rounded-lg file:border-0 file:bg-emerald-600 file:px-3 file:py-1.5 file:text-xs file:font-semibold file:text-white"
                            />
                            <p
                                v-if="remediationFileName"
                                class="text-[11px] text-muted-foreground"
                            >
                                Đã chọn: {{ remediationFileName }}
                            </p>
                            <Button
                                @click="submitRemediation"
                                size="sm"
                                :disabled="isProcessing"
                        ...
                        <div
                            v-if="
                                canReinspect &&
                                selectedReport.status === 'reinspection_pending'
                            "
                            class="space-y-3 rounded-2xl border border-sky-200/70 bg-sky-500/5 p-4 dark:border-sky-500/20"
                        >
                            <div
                                class="flex items-center gap-2 font-semibold text-sky-700 dark:text-sky-300"
                            >
                                <ClipboardCheck class="size-4" /> Kết luận tái
                                kiểm
                            </div>
                            ...
                            <textarea
                                ...
                                rows="3"
                                placeholder="Nêu tiêu chí đã kiểm, bằng chứng đối chiếu và kết luận..."
                                class="min-h-20 w-full rounded-xl border border-input bg-background p-3 text-xs text-foreground outline-none placeholder:text-muted-foreground focus:ring-2 focus:ring-sky-500/30"
                            ></textarea>
                            <input
                                type="file"
                                accept="image/*"
                                @change="onReinspectionProofChange"
                                class="block w-full cursor-pointer rounded-xl border border-input bg-background p-2 text-xs text-foreground file:mr-3 file:rounded-lg file:border-0 file:bg-sky-600 file:px-3 file:py-1.5 file:text-xs file:font-semibold file:text-white"
                            />
                            <p
                                v-if="reinspectionFileName"
                                class="text-[11px] text-muted-foreground"
                            >
                                Đã chọn: {{ reinspectionFileName }}
                            </p>
                            <Button
                                @click="submitReinspection"
                                size="sm"
                                :disabled="isProcessing"
                                class="gap-1.5 rounded-xl bg-sky-600 text-xs font-semibold text-white hover:bg-sky-700"
                            >
                                <ClipboardCheck class="size-4" /> Ghi nhận tái
                                kiểm
                            </Button>
                        </div>

                        <div
                            v-if="selectedReport.reinspection_notes"
                            class="rounded-2xl border border-sky-200/70 bg-sky-500/5 p-4 dark:border-sky-500/20"
                        >
                            <div
                                class="flex items-center gap-2 font-semibold text-sky-700 dark:text-sky-300"
                            >
                                <RotateCcw class="size-4" /> Kết quả tái kiểm:
                                {{
                                    selectedReport.reinspection_result ===
                                    'pass'
                                        ? 'Đạt'
                                        : 'Chưa đạt'
                                }}
                            ...
                            <p
                                class="mt-2 leading-relaxed whitespace-pre-line text-foreground"
                            >
                                {{ selectedReport.reinspection_notes }}
                            </p>
                            <a
                                v-if="selectedReport.reinspection_proof_url"
                                :href="selectedReport.reinspection_proof_url"
                                target="_blank"
                                rel="noopener"
                                class="mt-2 inline-flex text-[11px] font-bold text-sky-700 hover:underline dark:text-sky-300"
                                >Xem bằng chứng tái kiểm</a
                            >
                            <p
                                v-if="selectedReport.reinspected_at"
                                class="mt-1 text-[10px] text-muted-foreground"
                            >
                                Thực hiện lúc
                                {{ selectedReport.reinspected_at }} ·
                                {{ selectedReport.reinspector?.name }}
                            </p>
                        </div>

                        <div
                            v-if="selectedReport.actions?.length"
                            class="space-y-2 rounded-2xl border border-violet-200/70 bg-violet-500/5 p-4 dark:border-violet-500/20"
                        >
                            <div
                                class="flex items-center gap-2 font-semibold text-violet-700 dark:text-violet-300"
                            >
                                <ClipboardList class="size-4" /> CAPA liên quan
                            </div>
                            <div
                                v-for="action in selectedReport.actions"
                                :key="action.id"
                                class="rounded-xl border border-border/70 bg-background/60 p-3 text-xs"
                            >
                                <div
                                ...
                                >
                                    ...
                                    ><span
                                        class="rounded-full bg-muted px-2 py-1 text-[10px] font-bold"
                                        >{{ action.status }}</span
                                    >
                                </div>
                        ...
                        <div
                            v-if="activityForSelected.length"
                            class="space-y-2"
                        >
                            <div
                            ...
                            <div
                                v-for="activity in activityForSelected"
                                :key="activity.id"
                                class="flex items-start gap-3 border-l-2 border-border pl-3 text-[11px]"
                            >
                                <div class="min-w-0 flex-1">
                                    <p class="font-semibold text-foreground">
                                        {{ activity.user_name }} ·
