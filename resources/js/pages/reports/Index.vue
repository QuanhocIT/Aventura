...
// Pie chart data (top 5 products + others)
const pieSlices = computed(() => {
    const top5 = props.topProducts.slice(0, 5);
    const top5Revenue = top5.reduce((sum, p) => sum + p.total_revenue, 0);
    const otherRevenue = props.totals.net_revenue - top5Revenue;
    const slices = top5.map((p, idx) => ({
        name: p.name,
        value: p.total_revenue,
        color: ['#4f46e5', '#f97316', '#10b981', '#f59e0b', '#ef4444'][idx % 5],
        percentage: (p.total_revenue / props.totals.net_revenue) * 100,
    }));
    if (otherRevenue > 0) {
        slices.push({
            name: 'Khác',
            value: otherRevenue,
            color: '#94a3b8',
            percentage: (otherRevenue / props.totals.net_revenue) * 100,
        });
    }
    return slices;
});
...
const getArcPath = (startAngle: number, endAngle: number, radius: number) => {
    ...
    return `M ${start.x} ${start.y} A ${radius} ${radius} 0 ${largeArc} 1 ${end.x} ${end.y} L ${radius} ${radius} Z`;
};

const piePaths = computed(() => {
    let currentAngle = -Math.PI / 2; // start at top
    const total = pieSlices.value.reduce((sum, s) => sum + s.value, 0);
    if (total === 0) return [];
...
<template>
    ...
    <div class="flex flex-col gap-6 p-6 max-w-7xl mx-auto w-full">
                    ...
                    <div v-if="topProducts.length" class="divide-y divide-slate-100 dark:divide-slate-800">
                        ...
                        >
                            <span
                                ...
                                :class="idx === 0 ? 'bg-amber-100 text-amber-700' : idx === 1 ? 'bg-slate-100 text-slate-600' : idx === 2 ? 'bg-orange-100 text-orange-700' : 'bg-muted text-muted-forerence'"
                            >
                                {{ idx + 1 }}
                            </span>
        ...
        <div v-if="pieSlices.length" class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
                <CardHeader class="pb-3 border-b">
                    <CardTitle class="text-base">Cơ cấu doanh thu theo món</CardTitle>
                    <CardDescription class="text-xs">Top 5 sản phẩm đóng góp</CardDescription>
                </CardHeader>
                <CardContent>
                    <div class="flex flex-col items-center">
                        <svg viewBox="0 0 220 220" class="w-48 h-48">
                            <g v-for="slice in piePaths" :key="slice.name">
                                <path :d="slice.path" :fill="slice.color" class="stroke-white dark:stroke-slate-900 stroke-2" />
                                <title>{{ slice.name }}: {{ slice.percentage.toFixed(1) }}%</title>
                            </g>
                        </svg>
                        <div class="mt-4 grid grid-cols-2 gap-2 text-xs w-full">
                            <div v-for="slice in pieSlices" :key="slice.name" class="flex items-center gap-2">
                                <div class="w-3 h-3 rounded-full" :style="{ backgroundColor: slice.color }"></div>
                                <span class="truncate">{{ slice.name }}</span>
                                <span class="ml-auto font-mono">{{ slice.percentage.toFixed(1) }}%</span>
                            </div>
                        </div>
                    </div>
                </CardContent>
