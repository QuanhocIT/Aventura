...
const props = defineProps<{
    ...
    stats: {
        ...
        total_location_logs: number;
        old_location_logs_count: number;
        location_log_retention_days: number;
...
const selectedOptimizeTasks = ref<string[]>(['cleanup_queues', 'clear_sessions', 'archive_audit_logs', 'prune_shipper_location_logs']);
    ...
    return actionsString.split(', ')
        .map(act => {
            switch(act.trim()) {
                ...
                case 'prune_shipper_location_logs': return 'Xóa ping vị trí Shipper cũ';
                default: return act;
            }
        })
        .join(', ');
...
return '';
    ...
    if (details.prune_shipper_location_logs) {
        const p = details.prune_shipper_location_logs;
        parts.push(`Vị trí Shipper: Đã xóa ${p.deleted_count} bản ghi cũ hơn ${p.retention_days} ngày`);
    }
                    ...
                    <CardContent class="p-5 space-y-4">
                        <div class="space-y-3">
                            ...
                            <label class="flex items-start gap-3 p-3.5 rounded-xl border border-border/30 bg-muted/10 hover:bg-muted/20 transition-all cursor-pointer">
                                <input
                                    type="checkbox"
                                    :checked="selectedOptimizeTasks.includes('prune_shipper_location_logs')"
                                    @change="toggleTask('prune_shipper_location_logs')"
                                    class="size-4.5 rounded text-orange-500 border-border mt-0.5 accent-orange-500 focus:ring-orange-500/20"
                                />
                                <div>
                                    <h4 class="font-bold text-xs text-slate-800 dark:text-slate-200 flex items-center gap-1.5 flex-wrap">
                                        Xóa ping vị trí Shipper cũ hơn {{ stats.location_log_retention_days }} ngày
                                        <span class="inline-flex rounded bg-rose-500/10 px-1.5 py-0.2 text-[9px] font-bold text-rose-600 dark:text-rose-450 border border-rose-500/10">Tối ưu dung lượng</span>
                                    </h4>
                                    <p class="text-[10px] text-muted-foreground mt-0.5 leading-normal font-semibold">
                                        Xóa vĩnh viễn {{ stats.old_location_logs_count }} bản ghi GPS shipper cũ hơn {{ stats.location_log_retention_days }} ngày (trên tổng số {{ stats.total_location_logs }}). Dữ liệu vị trí thô tần suất cao không lưu trữ vì giá trị nghiệp vụ thấp sau thời gian này.
                                    </p>
                                </div>
                            </label>
                        </div>
