        ...
        <div class="grid gap-6 md:grid-cols-3">
            ...
            <div class="md:col-span-2 rounded-3xl border bg-zinc-50/50 dark:bg-zinc-900/10 p-6 flex flex-col justify-center">
                ...
                <div class="mt-4 flex items-center gap-4 text-xs font-semibold text-foreground">
                    <span class="flex items-center gap-1.5"><CheckCircle2 class="h-4 w-4 text-emerald-500 dark:text-emerald-400" /> Tự động cộng số dư</span>
                    <span class="flex items-center gap-1.5"><CheckCircle2 class="h-4 w-4 text-emerald-500 dark:text-emerald-400" /> Lưu vết log chi tiết</span>
                    <span class="flex items-center gap-1.5"><CheckCircle2 class="h-4 w-4 text-emerald-500 dark:text-emerald-400" /> Bảo mật giao dịch</span>
                </div>
            </div>
        </div>
                    ...
                    <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                        <tr v-for="req in withdrawalRequests.data" :key="req.id" class="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/30">
                            <td class="p-4">
                                ...
                                <div class="text-[10px] text-teal-600 dark:text-teal-400 font-semibold">Số dư hiện tại: {{ formatCurrency(req.user_balance) }}</div>
                            </td>
                            ...
                            <td class="p-4 text-right">
                                <div v-if="req.status === 'pending'" class="flex items-center justify-end gap-2">
                                    ...
                                    <Button type="button" variant="outline" size="sm" class="border-rose-200 text-rose-600 hover:bg-rose-50 dark:border-rose-900/50 dark:text-rose-400 dark:hover:bg-rose-950/20 rounded-xl h-8 px-3.5 cursor-pointer font-bold" @click="openActionModal(req, 'reject')">
                                        Từ chối
                                    </Button>
                                </div>
        ...
        <DialogContent class="sm:max-w-md rounded-3xl">
            <DialogHeader>
                <DialogTitle class="flex items-center gap-2 text-foreground">
                    <AlertTriangle v-if="actionType === 'reject'" class="h-5 w-5 text-rose-500 dark:text-rose-400" />
                    <CheckCircle2 v-else class="h-5 w-5 text-emerald-500 dark:text-emerald-400" />
