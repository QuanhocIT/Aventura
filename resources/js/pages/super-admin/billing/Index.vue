...
// Mock Webhook Payload for detailed view
const mockWebhookPayload = computed(() => {
    if (!selectedWebhook.value) return {};
    return {
        event_id: `evt_` + selectedWebhook.value.id + `_` + Math.random().toString(36).substr(2, 6),
        event_type: selectedWebhook.value.event_type || 'payment.success',
        provider: selectedWebhook.value.provider || 'payos',
        status: selectedWebhook.value.status || 'processed',
        processed_at: selectedWebhook.value.processed_at || new Date().toISOString(),
        gateway_details: {
            transaction_code: selectedWebhook.value.transaction_code || 'TXN_UNDEFINED',
            currency: 'VND',
            fee_amount: 1650,
            ip_address: '113.161.44.' + Math.floor(Math.random() * 254 + 1),
            signature: 'sha256_mock_sig_' + Math.random().toString(16).substr(2, 16) + '8e20f17b'
        },
        system_meta: {
            app_id: 'aventura_booking_system',
            tenant_id: 'rest_id_' + Math.floor(Math.random() * 1000 + 1),
            api_version: 'v1.4'
        }
    };
});
                ...
                <div class="space-y-2">
                    <div class="flex items-center justify-between text-xs font-bold text-muted-foreground">
                        <span class="flex items-center gap-1.5">
                            ...
                            <span>MÔ PHỎNG PAYLOAD DATA (JSON)</span>
                        </span>
                        <Button 
                            variant="ghost" 
                            size="sm" 
                            @click="copyToClipboard(JSON.stringify(mockWebhookPayload, null, 2))"
                    ...
                    <div class="relative bg-slate-950 rounded-2xl p-4 border border-white/[0.08] shadow-inner">
                        <pre class="text-[11px] font-mono text-emerald-400 overflow-x-auto max-h-[350px] leading-relaxed pt-1"><code>{{ JSON.stringify(mockWebhookPayload, null, 2) }}</code></pre>
                    </div>
                </div>
