        ...
        },
        {
            ...
            onSuccess: () => {
                ...
                if (
                    ...
                    selectedInvoice.value.id === writeOffInvoiceId.value
                ) {
...
const hasRealData = computed(
    () => props.invoices.total > 0 || props.webhooks.total > 0,
);
...
const metrics = computed(() => {
    ...
    const paidInvoices = invData.filter(
        (i) => i.status === 'paid' || i.status === 'processed',
    ).length;
    const paymentRate = Math.round((paidInvoices / totalInvoices) * 100);

    const totalWebhooks = webData.length || 1;
    const successWebhooks = webData.filter(
        (w) => w.status === 'processed' || w.status === 'success',
    ).length;
    ...
    const failedWebhooks = webData.filter(
...
const aiInsights = computed(() => {
    ...
    if (data.mostCommonType !== 'N/A') {
        ...
        const commonLabel =
            typeLabelMap[data.mostCommonType] || data.mostCommonType;
        list.push({
            type: 'info',
...
<template>
    ...
    <div class="flex flex-col gap-5 px-6 py-5">
        ...
        <div class="grid gap-4 md:grid-cols-3">
            <StatCard
                ...
                :sparkline="[
                    ...
                    28,
                    35,
                    invoices.total > 0 ? invoices.total : 42,
                ]"
            ...
            <StatCard
                ...
                :sparkline="[
                    ...
                    48,
                    50,
                    webhooks.total > 0 ? webhooks.total : 55,
                ]"
                class="group cursor-default transition-all duration-300 hover:scale-[1.02] hover:shadow-lg hover:shadow-violet-500/10"
            />
            <StatCard
                label="Điều chỉnh"
                ...
                :sparkline="[
                    ...
                    6,
                    7,
                    adjustments.total > 0 ? adjustments.total : 9,
                ]"
                class="group cursor-default transition-all duration-300 hover:scale-[1.02] hover:shadow-lg hover:shadow-emerald-500/10"
            />
        </div>
        ...
        <FilterBar>
            <div class="grid flex-1 gap-1.5">
                ...
                >
                    <SelectTrigger
                        class="border-border/60 bg-background/50 backdrop-blur-sm transition-all hover:border-border"
                    >
            ...
            <div v-if="activeTab === 'invoices'" class="grid gap-1.5">
                ...
                <Select v-model="status" @update:modelValue="applyFilters">
                    <SelectTrigger
                        class="w-[150px] border-border/60 bg-background/50 backdrop-blur-sm transition-all hover:border-border"
                    >
                        <SelectValue placeholder="Tất cả" />
                    </SelectTrigger>
                    <SelectContent>
            ...
            <div class="grid gap-1.5">
                ...
                <Select v-model="type" @update:modelValue="applyFilters">
                    <SelectTrigger
                        class="w-[170px] border-border/60 bg-background/50 backdrop-blur-sm transition-all hover:border-border"
                    >
                        <SelectValue placeholder="Tất cả" />
                    </SelectTrigger>
                    <SelectContent>
            ...
            <div class="grid flex-1 gap-1.5">
                ...
                <div class="relative">
                    <Search
                        class="absolute top-2.5 left-3 size-4 text-muted-foreground transition-colors"
                    />
                    <Input
                        v-model="search"
        ...
        <div class="grid gap-6 xl:grid-cols-3">
            ...
            <div class="space-y-5 xl:col-span-2">
                ...
                <Card
                    class="group relative overflow-hidden rounded-3xl border border-border/40 bg-card/45 p-5 shadow-[var(--shadow-premium)] backdrop-blur-md"
                >
                    <!-- Subtle background radial glows -->
                    <div
                        class="pointer-events-none absolute -top-24 -left-24 size-48 rounded-full bg-primary/10 blur-3xl transition-all duration-700 group-hover:scale-125"
                    />
                    <div
                        class="pointer-events-none absolute -right-24 -bottom-24 size-48 rounded-full bg-emerald-500/10 blur-3xl transition-all duration-700 group-hover:scale-125"
                    />

                    <div
                        class="relative flex flex-col items-stretch gap-6 lg:flex-row"
                    >
                        ...
                        <div class="flex-1 space-y-4">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-2">
                                    <Sparkles
                                        class="size-5 animate-pulse text-primary"
                                    />
                                    <h3
                                ...
                                <span
                                    ...
                                    >Chế độ Mô phỏng</span
                                >
                                <span
                                    v-else
                                    class="rounded-full border border-emerald-500/20 bg-emerald-500/10 px-2 py-0.5 text-[9px] font-bold text-emerald-600 dark:text-emerald-400"
                                    >Dữ liệu thời gian thực</span
                                >
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-3.5">
                                ...
                                <div
                                    class="flex items-center gap-3 rounded-2xl border border-border/30 bg-muted/20 p-3 shadow-inner"
                                >
                                    <div class="relative flex shrink-0">
                                        ...
                                        >
                                            ...
                                            <text
                                                ...
                                                y="21.5"
                                                text-anchor="middle"
                                                class="fill-foreground font-mono text-[9px] font-bold"
                                            >
                                                {{
                                                    metrics.paymentSuccessRate
                                                }}%
                            ...
                            <div
                                class="rounded-2xl border border-border/20 bg-muted/10 p-3"
                            >
                                <div
                                    class="mb-2 flex items-center justify-between text-[9px] font-bold tracking-wider text-muted-foreground uppercase"
                                >
                                ...
                                <svg
                                    class="mt-1 h-24 w-full text-muted-foreground"
                                    viewBox="0 0 300 100"
                                ...
                                >
                                    ...
                                    <g
                                        v-for="(item, idx) in metrics.chartData"
                                        :key="idx"
                                    >
                                        <rect
                                            :x="
                                        ...
                                        <text
                                            ...
                                            y="94"
                                            text-anchor="middle"
                                            class="fill-muted-foreground font-mono text-[7px] opacity-60"
                                        >
                                            {{ item.label }}
                                        </text>
                                    </g>
                        ...
                        <div class="hidden w-px bg-border/40 lg:block" />
                        ...
                        <div class="flex flex-1 flex-col gap-3">
                            ...
                            <div
                                class="max-h-[225px] space-y-2.5 overflow-y-auto pr-1"
                            >
                                <div
                                    v-for="(insight, idx) in aiInsights"
                                    :key="idx"
                                    class="flex items-start gap-2.5 rounded-2xl border border-border/30 bg-muted/15 p-3 transition-all hover:bg-muted/30"
                                >
                                    <span
                                        class="mt-0.5 shrink-0 rounded-lg border border-border/30 bg-background p-1"
                                    >
                                        <CheckCircle2
                                        ...
                                        <ShieldAlert
                                            v-else-if="
                                                insight.type === 'danger'
                                            "
                                            class="size-4 animate-bounce text-rose-500"
                                        />
                                        <AlertTriangle
                                            v-else-if="
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <p
                                            class="text-xs leading-tight font-bold text-foreground"
                                        >
                                            {{ insight.title }}
                                        </p>
                                        <p
                                            class="mt-1 text-[11px] leading-normal font-medium text-muted-foreground"
                                        >
                                            {{ insight.description }}
                                        </p>
                ...
                <!-- Custom Tab Switcher -->
                <div
                    class="flex items-center justify-between rounded-2xl border border-border/40 bg-card/45 p-1.5 shadow-[var(--shadow-premium)] backdrop-blur-md"
                >
                    <div class="flex gap-1.5">
                        <button
                            ...
                            :class="[
                                ...
                                activeTab === 'invoices'
                                    ? 'border border-sky-500/20 bg-sky-500/10 text-sky-600 shadow-[0_2px_8px_rgba(14,165,233,0.12)] dark:text-sky-400'
                        ...
                        <button
                            ...
                            :class="[
                                ...
                                activeTab === 'adjustments'
                                    ? 'border border-emerald-500/20 bg-emerald-500/10 text-emerald-600 shadow-[0_2px_8px_rgba(16,185,129,0.12)] dark:text-emerald-400'
                                    : 'border border-transparent text-muted-foreground hover:bg-muted/40 hover:text-foreground',
                            ]"
                        >
                    ...
                    <span
                        class="hidden px-4 text-[10px] font-bold tracking-wider text-muted-foreground uppercase sm:inline"
                        >Tài chính hệ thống</span
                    >
                </div>
                ...
                <Card
                    ...
                    class="border-border/40 bg-card/45 shadow-[var(--shadow-premium)] backdrop-blur-md transition-all duration-300 hover:border-border/60"
                >
                    ...
                    <CardContent class="space-y-3.5">
                        <div
                            ...
                            class="group/item relative flex cursor-pointer flex-col gap-3 overflow-hidden rounded-2xl border border-border/40 bg-gradient-to-br from-transparent to-transparent p-4 transition-all duration-300 hover:-translate-y-0.5 hover:border-sky-500/20 hover:from-sky-500/[0.01] hover:shadow-md"
                        ...
                        >
                            ...
                            <div
                                class="flex items-start justify-between gap-3 pl-2"
                            >
                                <div>
                                    <div class="flex items-center gap-2">
                                        <p
                                            class="font-mono text-sm font-bold text-foreground transition-colors group-hover/item:text-sky-500"
                                        >
                                    ...
                                    >
                                        ...
                                        />
                                        <p
                                            class="font-semibold text-muted-foreground/90"
                                        >
                                            {{ invoice.restaurant }}
                                        </p>
                                    </div>
                                ...
                                >
                                    ...
                                    <DropdownMenu>
                                        ...
                                        >
                                            ...
                                            <a
                                                :href="`/super-admin/billing/invoices/${invoice.id}/download`"
                                                target="_blank"
                                                @click.stop
                                            >
                                                <DropdownMenuItem
                            ...
                            <div
                                class="my-1 ml-2 border-t border-dashed border-border/40"
                            ></div>

                            <div
                                class="mt-0.5 flex flex-wrap items-center justify-between gap-3 pl-2 text-xs text-muted-foreground"
                            >
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="flex items-center gap-1">
                                        <Clock
                                            class="size-3.5 animate-pulse text-violet-500"
                                        />
                                        <span
                                            >Gửi:
                                            <span
                                ...
                                <span
                                    class="rounded-lg border border-border/40 bg-muted px-2.5 py-0.5 font-mono text-sm font-bold text-foreground shadow-inner"
                                    >{{ invoice.total }}
                ...
                >
                    <CardHeader class="pb-3">
                        ...
                        >
                            ...
                            <span
                                class="rounded-md bg-muted/60 px-2 py-0.5 font-mono text-xs font-normal text-muted-foreground"
                                >trang {{ adjustments.current_page }}/{{
                    ...
                    <CardContent class="space-y-3.5">
                        <div
                            ...
                            class="group/adj relative flex cursor-pointer flex-col gap-3 overflow-hidden rounded-2xl border border-border/40 bg-gradient-to-br from-transparent to-transparent p-4 transition-all duration-300 hover:-translate-y-0.5 hover:border-emerald-500/20 hover:from-emerald-500/[0.01] hover:shadow-md"
                            :style="{ animationDelay: `${idx * 40}ms` }"
                        >
                            <div
                                class="absolute top-0 bottom-0 left-0 w-1.5 bg-emerald-500"
                            />
                            ...
                            >
                                <div>
                                    <p
                                        class="text-sm font-bold text-foreground transition-colors group-hover/adj:text-emerald-500"
                                    >
                                    ...
                                    >
                                        ...
                                        />
                                        <p
                                            class="max-w-[280px] truncate font-medium text-muted-foreground/80 italic sm:max-w-[450px]"
                                        >
                                            {{
                                                adjustment.reason ||
                            ...
                            >
                                <div class="flex items-center gap-3">
                                    <span
                                        class="rounded border border-emerald-500/20 bg-emerald-500/10 px-2 py-0.5 font-mono font-extrabold text-emerald-600 dark:text-emerald-400"
                                        >+{{ adjustment.days }} ngày</span
                                    >
                                    <span
                                        >Gốc:
                                        <span
                                ...
                                <div
                                    class="flex items-center gap-3 font-medium text-muted-foreground/80"
                                >
                                    <span class="flex items-center gap-1">
                                        <User
            ...
            <div class="xl:col-span-1">
                <Card
                    class="h-fit border-border/40 bg-card/45 shadow-[var(--shadow-premium)] backdrop-blur-md transition-all duration-300 hover:border-border/60"
                >
                    <CardHeader class="pb-3">
                        <CardTitle
                            class="flex items-center justify-between text-base"
                    ...
                    <CardContent class="space-y-3.5">
                        <div
                            ...
                            class="group/webhook relative flex cursor-pointer flex-col gap-2 overflow-hidden rounded-2xl border border-border/40 bg-gradient-to-br from-transparent to-transparent p-4 text-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-violet-500/20 hover:from-violet-500/[0.01] hover:shadow-md"
                            :style="{ animationDelay: `${idx * 40}ms` }"
                        >
                            <div
                            ...
                            <p
                                class="truncate rounded border border-border/40 bg-muted/60 p-1.5 font-mono text-[11px] text-muted-foreground select-all"
                            >
                                {{
                            ...
                            >
                                ...
                                >
                                <span
                                    class="flex items-center gap-1 font-mono text-[10px] text-muted-foreground/80"
                                >
                                    <Clock class="size-3" />
                                    <span>{{
                        ...
                        <Pagination
                            v-if="webhooks.last_page > 1"
                            :links="webhooks.links"
                        />
                    </CardContent>
                </Card>
            </div>
    ...
    <Dialog v-model:open="showWriteOff">
        <DialogContent class="max-w-md border-border/40 bg-card">
            <DialogHeader>
            ...
            <div class="space-y-3.5 py-2">
                <p class="text-sm leading-relaxed text-muted-foreground">
                    Hóa đơn
                    <strong
                        class="rounded border border-border/60 bg-muted px-1.5 py-0.5 font-mono text-foreground"
                        >{{ writeOffInvoiceNum }}</strong
                    >
                    sẽ bị đánh dấu là không thể thu hồi.
                </p>
                <div class="space-y-2">
                    ...
                    <textarea
                        ...
                        rows="3"
                        class="w-full rounded-xl border border-border/60 bg-background/50 px-3 py-2 text-sm transition-all placeholder:text-muted-foreground/50 focus:border-rose-400 focus:ring-2 focus:ring-rose-400 focus:outline-none"
            ...
            <DialogFooter>
                ...
                >
                <Button
                    class="rounded-xl bg-rose-600 text-white hover:bg-rose-700"
    ...
    <Sheet
        ...
        @update:open="
            (val) => {
                if (!val) selectedInvoice = null;
            }
        "
    >
        <SheetContent
            side="right"
            class="w-full overflow-y-auto border-l border-border/40 bg-card/95 p-0 backdrop-blur-md sm:max-w-lg"
        >
            <div class="border-b border-border/40 bg-muted/20 px-6 py-5">
                <SheetHeader>
                    <div class="flex items-center justify-between">
            ...
            <div v-if="selectedInvoice" class="space-y-6 p-6">
                <!-- Mockup Paper Receipt -->
                <div
                    class="relative flex flex-col gap-4 overflow-hidden rounded-3xl border border-border/80 bg-gradient-to-br from-white to-slate-50 p-6 shadow-lg dark:from-slate-900 dark:to-slate-950/70"
                >
                    <!-- Top jagged styling mockup border using CSS gradient -->
                    <div
                        class="absolute top-0 right-0 left-0 h-1 bg-gradient-to-r from-sky-500 via-indigo-500 to-emerald-500"
                    />
                    ...
                    <div class="flex items-start justify-between">
                        <div>
                            <div class="flex items-center gap-1.5">
                                <Sparkles
                                    class="size-4 animate-pulse text-sky-500"
                                />
                                <span
                        ...
                        <div class="text-right">
                            ...
                            <p class="mt-0.5 text-[10px] text-muted-foreground">
                                Ngày:
                                {{ selectedInvoice.sent_at || 'Chưa gửi' }}
                            </p>
                        </div>
                    </div>

                    <div
                        class="my-1 border-t border-dashed border-border/80"
                    ></div>
                    ...
                    <div class="mt-2 space-y-2">
                        ...
                        <div
                            class="flex items-center justify-between rounded-xl border border-border/40 bg-muted/40 p-3 text-xs"
                        >
                    ...
                    <div class="mt-1 space-y-2">
                        ...
                        >
                            ...
                            <span class="font-mono"
                                >0 {{ selectedInvoice.currency }}</span
                            >
                        </div>
                        <div
                            class="my-1 border-t border-dashed border-border/60"
                        ></div>
                        <div
                            class="flex justify-between text-sm font-black text-foreground"
                        >
                            <span>TỔNG TIỀN THANH TOÁN</span>
                            <span
                                class="font-mono text-sky-600 dark:text-sky-400"
                                >{{ selectedInvoice.total }}
                                {{ selectedInvoice.currency }}</span
                            >
                        </div>
                    ...
                    <div
                        class="mt-4 border-t border-dashed border-border/80 pt-2"
                    >
                        <div
                            class="flex flex-col items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-border/80 bg-muted/30 p-4 shadow-inner"
                        >
                            <div
                                class="relative flex size-36 items-center justify-center rounded-2xl border border-border/60 bg-white p-2.5 shadow-md"
                            >
                                <svg
                                    class="size-30 text-slate-800"
                                    viewBox="0 0 100 100"
                                    fill="currentColor"
                                >
                                    <path
                                    ...
                                    />
                                    <path
                                        d="M30,30 h10 v10 h-10 z M60,30 h10 v10 h-10 z M30,60 h10 v10 h-10 z M55,55 h10 v10 h-10 z"
                                        opacity="0.8"
                                    />
                                </svg>
                                <div
                                    class="absolute flex size-8 items-center justify-center rounded-lg border-2 border-white bg-sky-500 p-1.5 text-white shadow-md"
                                >
                                    <Wallet class="size-4" />
                                </div>
                            </div>
                            <p
                                class="mt-1 text-[9px] font-black tracking-widest text-muted-foreground uppercase"
                            >
                    ...
                    <div
                        class="mt-1 space-y-1 rounded-xl border border-border/20 bg-muted/20 p-2 text-center text-[10px] text-muted-foreground/80"
                    >
                        <p
                            class="flex items-center justify-center gap-1 font-semibold text-foreground"
                        >
                            <Calendar class="size-3 text-sky-500" /> Hạn thanh
                            toán:
                            {{ selectedInvoice.due_on || 'Chưa xác định' }}
                        </p>
                        <p>
                            Trạng thái hóa đơn:
                ...
                <div class="space-y-2 border-t border-border/40 pt-2">
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <Button
                            variant="outline"
                            size="sm"
                            @click="resendInvoice(selectedInvoice.id)"
                        ...
                        <Button
                            variant="outline"
                            size="sm"
                            @click="regenerateInvoice(selectedInvoice.id)"
                            class="flex items-center justify-center gap-1.5 rounded-xl border-border/60 py-4 transition-all hover:bg-violet-500/10 hover:text-violet-600"
                        >
                            <RefreshCcw class="size-4" />
                            <span>Tạo lại hóa đơn</span>
                        </Button>
                        <a
                            :href="`/super-admin/billing/invoices/${selectedInvoice.id}/download`"
                            target="_blank"
                        ...
                        >
                            <Button
                                variant="outline"
                                size="sm"
                                class="flex w-full items-center justify-center gap-1.5 rounded-xl border-border/60 py-4 transition-all hover:bg-emerald-500/10 hover:text-emerald-600"
                            >
    ...
    >
        ...
        >
            ...
            <div v-if="selectedWebhook" class="space-y-6 p-6">
                <!-- Metadata Grid -->
                <div
                    class="space-y-3 rounded-2xl border border-border/40 bg-muted/40 p-4"
                >
                    <div class="grid grid-cols-2 gap-y-3 text-xs">
                        ...
                        <div>
                            ...
                            <p
                                class="mt-0.5 flex items-center gap-1 font-mono font-semibold text-foreground"
                            >
                            ...
                            <p class="mt-1">
                                <LedIndicator
                                    :status="
                                        ...
                                        selectedWebhook.status === 'resolved'
                                            ? 'online'
                                            : selectedWebhook.status ===
                                                    'failed' ||
                                                selectedWebhook.status ===
                ...
                <div class="space-y-2">
                    ...
                    >
                        ...
                        <Button
                            ...
                            "
                            class="h-7 gap-1 rounded-lg px-2 text-xs text-sky-500 hover:bg-sky-500/10 hover:text-sky-600"
                        >
                            <Copy class="size-3" />
                            <span>{{
                    ...
                    <div
                        class="relative rounded-2xl border border-white/[0.08] bg-slate-950 p-4 shadow-inner"
                    >
                        <pre
                            class="max-h-[350px] overflow-x-auto pt-1 font-mono text-[11px] leading-relaxed text-emerald-400"
                        ><code>{{ JSON.stringify(webhookPayloadPreview, null, 2) }}</code></pre>
                    </div>
                </div>
                ...
                <div class="space-y-2 border-t border-border/40 pt-2">
                    ...
                    <Button
                        variant="outline"
                        size="sm"
                        @click="retryWebhook(selectedWebhook.id)"
    ...
    <Sheet
        ...
        @update:open="
            (val) => {
                if (!val) selectedAdjustment = null;
            }
        "
    >
        <SheetContent
        ...
        >
            ...
            <div v-if="selectedAdjustment" class="space-y-6 p-6">
                <!-- Metadata Card -->
                <div
                    class="space-y-4 rounded-2xl border border-border/40 bg-muted/40 p-5"
                >
                    <div
                    ...
                    <div class="grid grid-cols-2 gap-4 pt-1 text-xs">
                        <div>
                            ...
                            <p
                                class="mt-0.5 inline-block rounded-md border border-emerald-500/20 bg-emerald-500/10 px-2 py-0.5 font-mono font-extrabold text-emerald-600 dark:text-emerald-400"
                            >
                                +{{ selectedAdjustment.days }} ngày
                            </p>
                        </div>
                        <div>
                            ...
                            <p class="mt-1 font-mono font-bold text-foreground">
                                {{ selectedAdjustment.discount_amount }} VND
                            </p>
                        </div>
                        <div>
                            <p class="text-muted-foreground">Người thực hiện</p>
                            <p
                                class="mt-0.5 flex items-center gap-1 font-semibold text-foreground"
                            >
                ...
                <!-- Notes / Reason -->
                <div
                    class="space-y-2 rounded-2xl border border-border/40 bg-gradient-to-br from-card to-muted/20 p-4"
                >
                    <p
                    ...
                    <p
                        class="rounded-xl border border-dashed border-border/80 bg-muted/30 p-2 text-sm leading-relaxed font-medium text-foreground italic"
                    >
                        "{{
                            selectedAdjustment.reason ||
                            'Không có ghi chú được nhập'
                        }}"
                    </p>
                </div>
                ...
                <!-- Logs Info -->
                <div
                    class="space-y-1 rounded-xl border border-border/20 bg-muted/20 p-3 text-[10px] text-muted-foreground"
                >
                    <p class="font-bold text-foreground">
                        💡 Lưu ý quan trọng:
                    </p>
