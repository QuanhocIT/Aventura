...
async function deleteItem(item: KnowledgeItem) {
    if (
        !(await confirmDialog({
            ...
            description: `Xóa câu hỏi: "${item.question}"?`,
        }))
    ) {
        return;
    }
...
const totalUnhelpful = computed(() => {
    return props.items.reduce(
        (sum, item) => sum + (item.unhelpful_count || 0),
        0,
    );
});
    ...
    <div class="flex flex-col gap-5 px-6 py-5">
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                <div
                    class="flex size-11 items-center justify-center rounded-xl border border-orange-500/20 bg-gradient-to-br from-orange-500/10 to-amber-500/10"
                >
        ...
        <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
            ...
            >
                <div>
                    ...
                    <h3
                        class="mt-1 font-mono text-2xl font-black tracking-tight text-blue-500"
                    >
                        {{ stats.total_helpful.toLocaleString() }}
                    </h3>
                </div>
                <div
                    class="flex size-9 items-center justify-center rounded-xl border border-blue-500/20 bg-blue-500/10 text-blue-500"
                >
            ...
            <div class="space-y-5 lg:col-span-3">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    <Input
                        ...
                        class="h-9 w-full rounded-xl border-border bg-background text-xs sm:w-64"
                ...
                >
                    <CardContent class="p-0">
                        <div class="overflow-x-auto">
                            <table class="w-full text-sm">
                                <thead>
                                    ...
                                    >
                                        <th
                                            class="w-32 px-4 py-3 text-left text-xs font-black tracking-wider text-muted-foreground uppercase"
                                        >
                                            Danh mục
                                        </th>
                                        <th
                                        ...
                                        <th
                                            class="w-16 px-4 py-3 text-center text-xs font-black tracking-wider text-muted-foreground uppercase"
                                        >
                                        ...
                                        <th
                                            class="w-24 px-4 py-3 text-right text-xs font-black tracking-wider text-muted-foreground uppercase"
                                        >
                                            Thao tác
                                        </th>
                                    </tr>
                                ...
                                <tbody>
                                    ...
                                    >
                                        <!-- Main row -->
                                        <tr
                                            class="cursor-pointer border-b border-border/30 transition hover:bg-muted/20"
                                        ...
                                        >
                                            ...
                                            <td
                                                class="px-4 py-3 text-center font-mono text-xs font-bold text-blue-500 text-muted-foreground"
                                            >
                                                {{ item.helpful_count }}
                                            </td>
                                        ...
                                        <tr
                                            ...
                                            "
                                            class="border-b border-border/30 bg-muted/10"
                                        >
                                            <td colspan="6" class="px-6 py-4">
                                                <div
                                                    class="grid gap-4 text-xs sm:grid-cols-2"
                                                >
                                                    <div>
                                                        <p
                                                        ...
                                                        <p
                                                            class="leading-relaxed font-semibold whitespace-pre-wrap text-slate-700 dark:text-slate-300"
                                                        >
                                                            {{ item.answer }}
                                                        </p>
                                                    </div>
                                                    ...
                                                    >
                                                        ...
                                                        >
                                                            ...
                                                            <ul
                                                                class="list-inside list-disc space-y-0.5 font-semibold text-muted-foreground"
                                                            >
                                                                <li
                                                                    v-for="(
                                                                        q, i
                        ...
                        <div
                            v-if="totalPages > 1"
                            class="flex flex-col items-center justify-between gap-4 border-t border-border/30 bg-muted/5 px-4 py-3 sm:flex-row"
                        >
                            <div
                            ...
                            >
                                ...
                                trong tổng số {{ items.length }} câu hỏi
                            </div>
                            <div class="flex items-center gap-1">
                                <Button
                                    variant="outline"
                                    size="icon"
                                    class="size-8 cursor-pointer rounded-lg hover:bg-muted"
                                ...
                                <Button
                                    ...
                                    "
                                    size="sm"
                                    class="h-8 min-w-8 cursor-pointer rounded-lg text-xs font-bold"
                                    :class="
                                        currentPage === page
                                            ? 'border-orange-500 bg-orange-500 text-white shadow-xs hover:bg-orange-600'
                    ...
                    <CardContent class="space-y-4 p-5">
                        ...
                        <div class="grid grid-cols-3 gap-2.5">
                            ...
                            >
                                <span
                                    class="block text-[9px] font-bold tracking-wider text-muted-foreground uppercase"
                                    >Phiếu bình chọn</span
                                >
                                <span
                                    class="mt-1 block font-mono text-xs font-black text-slate-800 dark:text-slate-200"
                                    >{{
                                        stats.total_helpful + totalUnhelpful
                                    }}</span
                                >
                            </div>
                            <div
                        ...
                        <div class="space-y-3.5 pt-2">
                            ...
                            >
                                ...
                                <div>
                                    <p
                                        class="text-xs font-bold text-slate-800 dark:text-slate-200"
                                    >
                                        Nâng cấp chất lượng trả lời
                                    </p>
                                    <p
                                        class="mt-0.5 text-[10px] leading-normal text-muted-foreground"
                                    >
                                        Tỷ lệ hữu ích đang ở mức thấp ({{
                                            helpfulRate
                                        }}%). Hãy rà soát lại các câu trả lời
                                        ngắn hoặc bổ sung thêm từ khóa để
                                        Chatbot khớp lệnh chính xác hơn.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div
                                v-if="stats.total - stats.active > 0"
                            ...
                            >
                                ...
                                <div>
                                    <p
                                        class="text-xs font-bold text-slate-800 dark:text-slate-200"
                                    >
                                        Câu hỏi chưa kích hoạt
                                    </p>
                                    <p
                                        class="mt-0.5 text-[10px] leading-normal text-muted-foreground"
                                    >
                                        Bạn đang tắt
                                        {{ stats.total - stats.active }} câu hỏi
                                        tri thức. Hãy rà soát nội dung và kích
                                        hoạt chúng để mở rộng khả năng giải đáp
                                        tự động.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div
                                v-if="helpfulRate >= 75 && stats.total >= 15"
                                class="flex items-start gap-2.5 rounded-xl border border-emerald-500/20 bg-emerald-500/[0.03] p-3"
                            >
                                <div
                                    class="mt-0.5 shrink-0 rounded-lg bg-emerald-500/10 p-1 text-emerald-500"
