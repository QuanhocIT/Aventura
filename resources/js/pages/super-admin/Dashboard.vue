...
<template>
    ...
    <div class="flex flex-col gap-6 p-6">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                ...
                <p class="text-sm text-muted-foreground">
                    Trung tam du lieu vi mo cho Super Admin
                </p>
            </div>
        ...
        <div class="grid gap-4 xl:grid-cols-[1.6fr_1fr]">
            <Card>
                <CardHeader
                    class="flex-row items-center justify-between gap-4 pb-2"
                >
                    <div>
                        <CardTitle class="text-base"
                            >Tenant growth & conversion</CardTitle
                        >
                        <p class="text-sm text-muted-foreground">
                            New tenant registrations vs Free to Pro conversion
                        </p>
                    </div>
                    <Badge variant="secondary" class="gap-1">
                        <TrendingUp class="size-3.5" />
                        {{ tenantGrowth.length }} mo
                    </Badge>
                </CardHeader>
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <div class="rounded-lg border p-4">
                            ...
                            >
                                ...
                                <span class="text-muted-foreground">{{
                                    tenantGrowth.at(-1)?.new_tenants ?? 0
                                }}</span>
                            </div>
                        ...
                        <div class="rounded-lg border p-4">
                            ...
                            >
                                ...
                                <span class="text-muted-foreground">{{
                                    tenantGrowth.at(-1)?.free_to_pro ?? 0
                                }}</span>
                            </div>
                            <svg viewBox="0 0 100 44" class="h-24 w-full">
                                <path
                    ...
                    <CardContent class="space-y-3">
                        <div class="flex items-center justify-between text-sm">
                            ...
                            ><span class="font-semibold">{{
                                saasMetrics.active_subscriptions
                            }}</span>
                        </div>
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-muted-foreground"
                                >Paid tenants</span
                            ><span class="font-semibold">{{
                                saasMetrics.paid_tenants
                            }}</span>
                        </div>
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-muted-foreground">Churn rate</span
                            ><span class="font-semibold"
                                >{{ saasMetrics.churn_rate }}%</span
                            >
                        </div>
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-muted-foreground"
                                >Churned this month</span
                            ><span class="font-semibold">{{
                                saasMetrics.churned_this_month
                            }}</span>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card>
                    ...
                    <CardContent class="space-y-3">
                        ...
                        <div class="flex items-center justify-between text-sm">
                            ...
                            ><span class="font-semibold">{{
                                formatCurrency(saasMetrics.mrr)
                            }}</span>
                        </div>
                        <div class="flex items-center justify-between text-sm">
                            <span class="text-muted-foreground">ARR</span
                            ><span class="font-semibold">{{
                                formatCurrency(saasMetrics.arr)
                            }}</span>
                        </div>
                    </CardContent>
                </Card>
        ...
        <div class="grid gap-4 xl:grid-cols-2">
            <Card>
                <CardHeader class="pb-2">
                    <CardTitle class="text-base"
                        >Top tenant by order volume</CardTitle
                    >
                </CardHeader>
                <CardContent class="space-y-3">
                    <div
                    ...
                    >
                        ...
                        <Badge variant="secondary"
                            >{{ item.orders_count }} orders</Badge
                        >
                    </div>
                    <p
            ...
            <Card>
                <CardHeader class="pb-2">
                    <CardTitle class="text-base"
                        >Top tenant by cloud storage</CardTitle
                    >
                </CardHeader>
                <CardContent class="space-y-3">
                    <div
                    ...
                    <CardContent class="space-y-3">
                        ...
                        >
                            <div class="flex items-center gap-2">
                                <Crown
                                    v-if="plan.code?.toLowerCase() === 'pro'"
                ...
                <Card>
                    ...
                    <CardContent class="space-y-2 text-sm">
                        ...
                        <div class="text-xs text-muted-foreground">
                            API error:
                            {{ supportOverview.monitoring.api_error_rate }}% |
                            Slow query:
                            {{ supportOverview.monitoring.slow_queries }}
                        </div>
                        <div class="text-xs text-muted-foreground">
                            Ticket open:
                            {{ supportOverview.stats.tickets_open }} | Alert
                            open: {{ supportOverview.stats.alerts_open }}
                        </div>
                    </CardContent>
                </Card>
            </div>
