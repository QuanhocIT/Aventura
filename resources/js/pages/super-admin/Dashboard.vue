...
const props = defineProps<{
    ...
    aiInsights: {
        ...
        data_source?: 'live' | 'estimated_fallback';
        is_degraded?: boolean;
    };
    supportOverview: {
        monitoring: {
...
<template>
    ...
    <div :class="['flex flex-col gap-5 px-6 py-5 mx-auto w-full', { 'compact-mode': compactMode }]">
        <!-- Header -->
        ...
        <div v-if="hasAiData && isSectionVisible('ai_insights')" class="space-y-4 rounded-3xl border border-violet-500/20 dark:border-violet-500/30 p-6 bg-gradient-to-br from-violet-500/[0.04] via-indigo-500/[0.03] to-sky-500/[0.04] backdrop-blur-xl shadow-xs">
            ...
            <!-- Degraded-mode notice: shown when the Python AI/analytics service is
                 unavailable and these numbers are a rule-based estimate, not live ML output -->
            <div v-if="aiInsights.is_degraded"
                class="flex items-center gap-2.5 rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-2.5 text-xs font-semibold text-amber-700 dark:text-amber-400">
                <AlertTriangle class="size-4 shrink-0" />
                <span>Dữ liệu ước tính — dịch vụ phân tích AI hiện không khả dụng. Các chỉ số bên dưới được tính bằng quy tắc dự phòng dựa trên dữ liệu thật, không phải từ mô hình AI trực tiếp.</span>
            </div>
