...
import { 
    Users, UserCheck, Star, ShoppingCart, Search, Crown,
    Brain, Sparkles, AlertTriangle, TrendingUp, Calendar
    ...
    <div class="flex flex-col gap-5 px-6 py-5">
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div class="flex items-center justify-between p-4 bg-card/40 border border-border/40 backdrop-blur-md rounded-2xl shadow-3xs transition-all duration-300 hover:shadow-xs">
                <div>
                    <p class="text-[9px] font-black text-muted-foreground uppercase tracking-wider">Tổng khách hàng</p>
            ...
            <div class="flex items-center justify-between p-4 bg-card/40 border border-border/40 backdrop-blur-md rounded-2xl shadow-3xs transition-all duration-300 hover:shadow-xs">
                <div>
                    <p class="text-[9px] font-black text-muted-foreground uppercase tracking-wider">Khách VIP</p>
                    <h3 class="text-2xl font-black font-mono tracking-tight mt-1 text-amber-500">{{ stats.vip }}</h3>
                </div>
            ...
            <div class="flex items-center justify-between p-4 bg-card/40 border border-border/40 backdrop-blur-md rounded-2xl shadow-3xs transition-all duration-300 hover:shadow-xs">
                <div>
                    <p class="text-[9px] font-black text-muted-foreground uppercase tracking-wider">Mới tháng này</p>
                    <h3 class="text-2xl font-black font-mono tracking-tight mt-1 text-emerald-500">{{ stats.new_this_month }}</h3>
                </div>
            ...
            <div class="flex items-center justify-between p-4 bg-card/40 border border-border/40 backdrop-blur-md rounded-2xl shadow-3xs transition-all duration-300 hover:shadow-xs">
                <div>
                    <p class="text-[9px] font-black text-muted-foreground uppercase tracking-wider">Đã chi tiêu</p>
                    <h3 class="text-2xl font-black font-mono tracking-tight mt-1 text-violet-500">{{ stats.has_spent }}</h3>
                </div>
                ...
                <CardHeader class="pb-2">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-[10px] text-muted-foreground mt-0.5">Biểu đồ biểu diễn tốc độ tích lũy tài khoản khách hàng đăng ký trong 6 tháng.</p>
                        </div>
                        <div class="text-[9px] font-black uppercase font-mono text-sky-500 flex items-center gap-1">
                            <span class="size-2 rounded-full bg-sky-500"></span> Khách hàng
                        </div>
                    </div>
                </CardHeader>
                    ...
                    <div class="relative w-full h-32 mt-4 bg-muted/10 rounded-xl border border-border/20 p-2 overflow-hidden flex flex-col justify-between">
                        ...
                        <div class="absolute inset-0 top-3 bottom-8 left-0 right-0">
                            <svg class="w-full h-full overflow-visible" viewBox="0 0 500 100" preserveAspectRatio="none">
                                ...
                                <path :d="growthAreaPath" fill="rgba(14, 165, 233, 0.06)" />
                                <path :d="growthLinePath" fill="none" stroke="#0ea5e9" stroke-width="2" />
                        ...
                        <div class="z-10 flex justify-between px-2 pt-24 text-[9px] font-black text-muted-foreground uppercase font-mono">
                            <span v-for="(p, i) in chartPoints" :key="i" class="text-center w-12 truncate">
                                {{ p.label }}
                                <div class="text-[8px] text-sky-500 font-extrabold mt-0.5">{{ p.val }} khách</div>
                            </span>
                        </div>
                    </div>
                </CardContent>
            ...
            <Card class="border border-border/40 bg-card/45 backdrop-blur-md shadow-2xs overflow-hidden rounded-2xl flex flex-col justify-between">
                ...
                <CardContent class="space-y-3 flex-grow flex flex-col justify-between pb-3">
                    <div class="bg-indigo-500/[0.03] border border-indigo-500/10 p-3 rounded-xl space-y-2">
                        ...
                        <p class="text-[11px] text-muted-foreground leading-relaxed font-semibold">
                            <span v-if="stats.vip === 0">
                                Hiện hệ thống ghi nhận 0 khách hàng VIP ({{ vipRatio }}%). Giải pháp: Thiết lập chính sách nâng hạng VIP tự động (ví dụ: chi tiêu &ge; 1,000,000đ) để thúc đẩy doanh thu bán lẻ trung bình trên mỗi khách.
                            </span>
                            <span v-else>
                                Tỷ lệ khách hàng VIP hiện đạt {{ vipRatio }}%. Hoạt động giữ chân đang phát huy hiệu quả. Gợi ý: Gửi tặng các mã coupon giảm giá 10% đặc quyền vào ngày sinh nhật để tối ưu hóa sự gắn kết.
                            </span>
                        </p>
                    </div>
                    ...
                    <div class="flex items-center justify-between text-[10px] font-bold text-muted-foreground bg-muted/20 px-3 py-2 rounded-xl border border-border/30">
                        <span class="flex items-center gap-1"><AlertTriangle class="size-3 text-amber-500" /> Tỷ lệ kích hoạt chi tiêu</span>
                        <span class="font-mono text-emerald-500 font-extrabold">{{ spentRatio }}% có giao dịch</span>
                    </div>
                </CardContent>
            </Card>
        </div>

        <FilterBar class="border border-border/40 bg-card/45 backdrop-blur-md shadow-3xs rounded-2xl p-4 gap-4 flex-wrap">
            <div class="relative min-w-48 flex-1">
                <Search class="absolute left-3 top-2.5 size-4 text-muted-foreground" />
                <Input v-model="search" placeholder="Tìm kiếm tên, số điện thoại, email..." class="pl-9 rounded-xl border-border focus-visible:ring-orange-500/20 focus-visible:border-orange-500 h-9 text-xs" />
            </div>
            <Select v-model="restaurantId" @update:model-value="applyFilter">
                <SelectTrigger class="w-[200px] h-9 text-xs rounded-xl border-border focus:ring-orange-500/20 focus:border-orange-500">
            ...
            <Select v-model="vip" @update:model-value="applyFilter">
                <SelectTrigger class="w-[130px] h-9 text-xs rounded-xl border-border focus:ring-orange-500/20 focus:border-orange-500">
        ...
        >
            ...
            <template #cell-vip="{ row }">
                <span 
                    v-if="row.is_vip"
                    class="inline-flex items-center gap-1 text-[9px] font-black uppercase rounded-full px-2 py-0.5 border border-amber-500/25 bg-amber-500/10 text-amber-600 dark:text-amber-400 shadow-3xs"
                >
            ...
            <template #cell-total_spent="{ row }">
                <span class="font-mono font-black tabular-nums text-emerald-500 text-xs">{{ formatVND(row.total_spent) }}₫</span>
            </template>

            <template #cell-loyalty_points="{ row }">
                <span class="font-mono font-bold text-xs tabular-nums text-amber-500 bg-amber-500/5 px-2 py-0.5 rounded-md border border-amber-500/10">{{ row.loyalty_points }}</span>
            </template>
