...
import {
    Activity, Settings, MessageSquare, RefreshCw, AlertTriangle,
    CheckCircle, XCircle, Server, Hammer, Clock, Terminal,
...
import { PageHeader, StatCard, LedIndicator, AlertBanner, ProgressBar } from '@/components/super-admin';
...
async function checkLiveStatuses() {
    ...
    promise.finally(() => { isPinging.value = false; });
}
...
async function toggleMaintenance(service: Service) {
    ...
    try {
        const response = await fetch(`/super-admin/service-monitor/${service.service_key}/toggle-maintenance`, {
            ...
            body: JSON.stringify({ is_maintenance: service.is_maintenance, message: service.maintenance_message }),
        });
        const data = await response.json();
        if (!data.success) throw new Error('Lỗi cập nhật');
        toast.success(`Đã ${service.is_maintenance ? 'BẬT' : 'TẮT'} chế độ bảo trì cho ${service.name}`);
    } catch {
        service.is_maintenance = originalValue;
...
async function saveMaintenanceMessage() {
    ...
    try {
        ...
        if (data.success) {
            ...
            if (target) target.maintenance_message = maintenanceMessageText.value;
...
function getServiceLedStatus(service: Service): 'online' | 'offline' | 'warning' | 'maintenance' {
...
<template>
    ...
    <div class="flex flex-col gap-5 px-6 py-5">
        ...
        >
            <template #actions>
                <Button @click="checkLiveStatuses" :disabled="isPinging" class="gap-2">
        ...
        <AlertBanner
            v-if="localServices.some(s => s.last_status === 'offline' && !s.is_maintenance)"
            severity="critical"
        ...
        <div class="grid gap-4 sm:grid-cols-3">
            ...
            <StatCard
                ...
                :value="localServices.filter(s => s.last_status === 'offline' && !s.is_maintenance).length"
                :icon="XCircle"
                color="rose"
                class=""
            />
        ...
        <div class="grid gap-6 lg:grid-cols-2">
            ...
            >
                    ...
                    <div class="flex items-start justify-between gap-4 border-b p-5">
                        <div class="space-y-1">
                            <div class="flex items-center gap-2">
                                <h3 class="text-base font-bold text-foreground">{{ service.name }}</h3>
                                <span class="rounded border bg-muted px-1.5 py-0.5 font-mono text-[10px] text-muted-foreground">
                                    {{ service.service_key }}
                                </span>
                            </div>
                        ...
                        <LedIndicator
                            ...
                            :label="service.is_maintenance ? 'Bảo trì' : service.last_status === 'online' ? 'Online' : 'Offline'"
                            size="md"
                        />
                    </div>
                    ...
                    <div class="space-y-4 p-5 text-sm">
                        <div class="grid grid-cols-2 gap-4">
                            <div class="rounded-xl border bg-muted/30 p-3">
                                <div class="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Độ trễ (Latency)</div>
                            ...
                            <div class="rounded-xl border bg-muted/30 p-3">
                                <div class="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Lần cuối kiểm tra</div>
                                <div class="mt-2 flex items-center gap-1 text-xs font-semibold text-foreground">
                        ...
                        <div class="flex items-center justify-between border-t pt-4">
                            <div class="space-y-0.5">
                                <span class="text-xs font-bold text-foreground">Chế độ bảo trì</span>
                        ...
                        <!-- Message area -->
                        <div class="space-y-2.5 rounded-xl border bg-muted/20 p-4">
                ...
                >
                    <div class="mb-1 flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-rose-600 dark:text-rose-400">
                        <Terminal class="size-3 animate-pulse text-rose-500" />
                        Nhật ký lỗi hệ thống
                    </div>
                    <div class="break-words font-mono text-[11px] leading-relaxed text-rose-700 dark:text-rose-300/80">
    ...
    <Dialog v-model:open="showEditMessageDialog">
        <DialogContent class="max-w-md">
            <DialogHeader>
                ...
                <DialogDescription>
                    Thông báo này sẽ được hiển thị cho khách hàng khi phân hệ bảo trì.
                </DialogDescription>
            </DialogHeader>
            <div class="space-y-4 py-4">
                <div class="space-y-1.5">
                    <Label>Tên phân hệ</Label>
                    <Input :value="editingService?.name" disabled class="cursor-not-allowed bg-muted text-muted-foreground" />
                </div>
                <div class="space-y-1.5">
                    ...
                    <textarea
                        ...
                        rows="4"
                        class="w-full rounded-lg border bg-background p-3 text-sm text-foreground outline-none transition-colors focus:border-primary focus:ring-1 focus:ring-primary"
            ...
            <DialogFooter class="gap-2">
                <Button variant="ghost" @click="showEditMessageDialog = false">Hủy</Button>
                <Button @click="saveMaintenanceMessage" :disabled="isSavingMessage">
