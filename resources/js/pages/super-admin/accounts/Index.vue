...
function resetPassword(account: { id: number; name: string }) {
    if (
        !confirm(
            `Reset mật khẩu cho "${account.name}"? Mật khẩu mới sẽ hiển thị ngay sau thao tác này.`,
        )
    ) {
        return;
    }
...
function impersonateUser(account: any) {
    if (
        !confirm(
            `Bạn có chắc chắn muốn đăng nhập sắm vai dưới danh nghĩa "${account.name}" không?`,
        )
    ) {
        return;
    }
    ...
    <div class="flex flex-col gap-6 p-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-muted-foreground">
                    Tổng cộng {{ accounts.meta?.total ?? 0 }} tài khoản chủ
                    doanh nghiệp
                </p>
            </div>
            <a
        ...
        <Card>
            <CardContent class="p-0">
                            ...
                            <td class="px-4 py-4">
                                ...
                                >
                                    ...
                                    <Button
                                        variant="ghost"
                                        size="sm"
                                        class="gap-1.5 text-xs font-semibold text-indigo-600 hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300"
                ...
                <!-- Pagination -->
                <div
                    v-if="accounts.meta?.last_page > 1"
    ...
    <Dialog v-model:open="showTempPassword">
        <DialogContent class="max-w-sm">
            ...
            <div class="space-y-3 py-2">
                <p class="text-sm text-muted-foreground">
                    Cung cấp mật khẩu này cho người dùng. Hệ thống không lưu mật
                    khẩu dạng văn bản — đây là lần duy nhất bạn thấy nó.
                </p>
                <div class="flex items-center gap-2">
                    <code
                        class="flex-1 rounded bg-muted px-4 py-3 font-mono text-lg font-bold tracking-widest select-all"
                    >
                        {{ tempPassword }}
                    </code>
                </div>
    ...
    <Dialog v-model:open="showStatusDialog">
        <DialogContent class="max-w-sm">
            ...
            <p class="py-2 text-sm text-muted-foreground">
                Bạn có chắc muốn
                <strong>{{
                    statusForm.status === 'suspended' ? 'khoá' : 'mở khoá'
                }}</strong>
