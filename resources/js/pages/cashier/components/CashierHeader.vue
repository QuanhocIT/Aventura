...
<template>
    <div class="flex flex-col items-start justify-between gap-4 rounded-3xl border border-slate-200/50 bg-white/70 p-6 shadow-sm backdrop-blur-md md:flex-row md:items-center dark:border-slate-800/80 dark:bg-slate-900/80">
        <div>
            <h1 class="flex items-center gap-2.5 text-2xl font-black text-slate-800 dark:text-slate-100">
            ...
            <p class="mt-1 flex flex-wrap items-center gap-2 text-xs font-medium text-muted-foreground">
                ...
                <span v-if="shiftInfo?.active_shift" class="text-slate-300 dark:text-slate-700">|</span>
                <span v-if="shiftInfo?.active_shift">Ca làm việc:
                    <span class="font-bold text-indigo-600 dark:text-indigo-400">
                        {{ shiftInfo.active_shift.shift_name }}
                    </span>
                </span>
                <span class="text-slate-300 dark:text-slate-700">|</span>
                <span>Doanh thu ca này:
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="flex items-center gap-3 rounded-2xl bg-slate-100 px-4 py-2.5 dark:bg-slate-800">
                <Clock class="size-4 text-slate-500" />
                <div class="text-left font-mono">
                    <span class="text-sm font-bold text-slate-800 dark:text-slate-200">{{ currentTime }}</span>
                    <span class="ml-2 text-[10px] text-muted-foreground">{{ currentDate }}</span>
                </div>
            </div>
        </div>
    </div>
