...
const filteredProducts = computed(() => {
    return props.products.filter((p) => {
        const matchesCategory =
            selectedCategoryId.value === 'all' || p.category_id === selectedCategoryId.value;
        const matchesSearch =
            !searchQuery.value.trim() ||
            p.name.toLowerCase().includes(searchQuery.value.toLowerCase().trim());
    ...
    <div class="flex flex-col gap-4">
        ...
        <div class="grid grid-cols-2 gap-3.5 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            <div
                ...
                :key="product.id"
                @click="!product.is_paused && !product.is_out_of_stock && emit('clickProduct', product)"
                ...
                :class="{
                    'opacity-60 cursor-not-allowed': product.is_paused || product.is_out_of_stock,
                    'cursor-pointer': !product.is_paused && !product.is_out_of_stock,
                }"
            >
                <div class="flex flex-col text-left">
                    <h5 class="line-clamp-2 text-xs font-bold text-slate-800 dark:text-slate-100">
                        {{ product.name }}
                    </h5>
                    <span class="mt-1 font-mono text-sm font-black text-indigo-600 dark:text-indigo-400">
                ...
                >
                    ...
                    {{ product.is_out_of_stock ? 'Tạm hết món' : 'Tạm ngưng bán' }}
                </div>
                ...
                <div v-else class="mt-3 flex items-center justify-between">
                    <template v-if="getCartItemQty(product.id) > 0">
                        <div class="flex items-center gap-1.5 rounded-xl bg-indigo-50 p-1 dark:bg-indigo-950/40">
                            <button
                            ...
                            <span class="w-4 text-center font-mono text-xs font-bold text-indigo-600 dark:text-indigo-400">
