...
const editingItems = ref<
    Array<{ product_id: number; name: string; quantity: number; notes: string }>
>([]);
...
function addProduct() {
    ...
    const product = props.products.find(
        (item) => item.id === selectedProductId.value,
    );
    ...
    const existing = editingItems.value.find(
...
async function saveRevision() {
    ...
    try {
        const response = await axios.patch(
            `/api/temporary-orders/${editingOrder.value.id}/revise`,
            {
        ...
        emit('refreshQrOrders', {
            message: response.data.message || 'Đã gửi thay đổi tới khách.',
            type: 'success',
        });
    } catch (error: any) {
        emit('refreshQrOrders', {
            message: error.response?.data?.message || 'Không thể chỉnh đơn.',
            type: 'error',
        });
    } finally {
...
async function submitRejection() {
    ...
    try {
        const response = await axios.post(
            `/api/temporary-orders/${rejectingOrder.value.id}/cancel`,
            {
        ...
        emit('refreshQrOrders', {
            message:
                ...
                'Đã từ chối đơn và thông báo tới khách.',
            type: 'success',
        });
    } catch (error: any) {
        emit('refreshQrOrders', {
        ...
        <Teleport to="body">
            ...
            >
                ...
                >
                    ...
                    <div
                        class="scrollbar-hidden max-h-64 space-y-3 overflow-y-auto pr-1"
                    >
                        <div
                ...
                >
                    <div class="mb-5 flex items-start justify-between gap-4">
                        <div>
                            ...
                            <p class="mt-1 text-xs text-slate-500">
                                Khách tại
                                {{ rejectingOrder.table?.name || 'bàn này' }} sẽ
                                được thông báo.
                            </p>
                        </div>
                        <button
                            class="rounded-full p-1 text-slate-400 hover:bg-slate-100"
                            @click="rejectingOrder = null"
                        >
                            <X class="size-5" />
                        </button>
                    </div>
