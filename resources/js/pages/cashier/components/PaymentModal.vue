...
import { DollarSign, X, CheckCircle2 as CheckIcon, Search } from 'lucide-vue-next';
...
<template>
    ...
    >
        <div class="animate-fade-in flex w-full max-w-md flex-col gap-6 overflow-hidden rounded-3xl border bg-white p-6 shadow-2xl dark:bg-slate-900">
            ...
            <div class="flex flex-col gap-4 text-left">
                <div class="rounded-2xl border bg-slate-50 p-4 dark:bg-slate-950">
                    ...
                    <template v-if="activeTable?.active_order?.discount_amount && activeTable.active_order.discount_amount > 0">
                        <div class="mb-1 flex justify-between text-xs">
                            ...
                            <span class="font-mono">{{ numberFormat(activeTable.active_order.subtotal) }}đ</span>
                        </div>
                        <div class="mb-2 flex justify-between text-xs">
                            ...
                            <span class="font-mono font-bold text-emerald-600">
                                -{{ numberFormat(activeTable.active_order.discount_amount) }}đ
                            </span>
                        </div>
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <span class="font-mono text-lg font-black text-indigo-600">
                            {{ numberFormat(activeTable?.active_order?.total_amount ?? 0) }}đ
                        </span>
                    </div>
                </div>
                ...
                <div class="mt-1 flex flex-col gap-2 border-t pt-3 text-left">
                    ...
                    <div v-if="!foundCustomer" class="flex gap-2">
                        <Input
                            ...
                            @input="emit('update:searchCustomerPhone', ($event.target as HTMLInputElement).value)"
                    ...
                    <div
                        v-else
                        class="flex items-center justify-between rounded-xl border border-indigo-150 bg-indigo-50/50 p-3 dark:bg-indigo-950/20"
                    >
                        <div class="flex flex-col text-left text-xs">
                            <span class="font-bold text-slate-800 dark:text-slate-200">
                                👤 {{ foundCustomer.full_name }}
                            </span>
                            <span class="mt-0.5 text-[10px] font-bold text-indigo-600 dark:text-indigo-400">
                                SĐT: {{ foundCustomer.phone }} • Hạng:
                                {{
                                    foundCustomer.membership_level === 'diamond'
                                        ? '💎 Kim Cương (-10%)'
                                        : foundCustomer.membership_level === 'gold'
                                          ? '⭐ Vàng (-5%)'
                                          : '🥈 Bạc'
                                }}
                ...
                <div
                    v-if="foundCustomer && (foundCustomer.loyalty_points ?? 0) > 0"
                ...
                >
                    <span class="flex items-center gap-1.5 text-xs font-bold text-amber-700 dark:text-amber-400">
                        🎁 Đổi điểm thưởng giảm giá
                        <span class="ml-auto font-mono text-[10px] text-amber-600">
                    ...
                    <div class="flex items-center gap-2">
                        <Input
                            ...
                            @input="emit('update:loyaltyPointsToRedeem', Number(($event.target as HTMLInputElement).value))"
                            :min="0"
                        ...
                        <Button
                            ...
                            @click="emit('update:loyaltyPointsToRedeem', foundCustomer.loyalty_points ?? 0)"
                        >
                            Đổi tất cả
                        </Button>
                    </div>
                    <p v-if="loyaltyPointsToRedeem > 0" class="text-[10px] text-amber-600 dark:text-amber-400">
                        ✓ Đổi {{ loyaltyPointsToRedeem }} điểm → Backend tự động tính giảm giá tương ứng
                    </p>
                </div>
                ...
                <div v-if="paymentMethod === 'cash'" class="flex flex-col gap-2">
                    ...
                    <Input
                        type="number"
                        :value="cashReceived"
                        @input="emit('update:cashReceived', Number(($event.target as HTMLInputElement).value))"
                    ...
                    <div class="flex flex-wrap gap-1">
                        <Button
                            ...
                            size="sm"
                            variant="outline"
                            class="h-7 border-slate-200 px-2.5 text-[10px] font-bold rounded-lg dark:border-slate-800"
                    ...
                    <div class="mt-1 flex justify-between text-xs font-bold text-emerald-600">
                        <span>Tiền thối lại:</span>
                        <span class="font-mono">{{ numberFormat(changeAmount) }}đ</span>
                    </div>
                </div>

                <!-- Thông tin ghi nợ VIP/B2B -->
                <div v-if="paymentMethod === 'debt'" class="flex flex-col gap-2 rounded-xl p-1 text-left text-xs">
                    <div v-if="!foundCustomer" class="rounded-lg border border-rose-100 bg-rose-50 p-2.5 font-bold text-rose-500">
                        ⚠️ Giao dịch ghi nợ yêu cầu chọn khách hàng trước.
                    </div>
                    <div v-else-if="!foundCustomer.is_vip && !foundCustomer.is_b2b" class="rounded-lg border border-rose-100 bg-rose-50 p-2.5 font-bold text-rose-500">
                        ⚠️ Khách hàng này không được cấp quyền mua nợ (Không phải VIP/B2B).
                    </div>
                    <div v-else class="flex flex-col gap-1.5 rounded-xl border bg-slate-50 p-3 dark:bg-slate-900/20">
                        <div class="flex justify-between">
                            <span class="text-slate-500">Hạn mức nợ tối đa:</span>
                            <span class="font-mono font-bold">{{ numberFormat(foundCustomer.credit_limit ?? 0) }}đ</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-slate-500">Dư nợ hiện tại:</span>
                            <span class="font-mono font-bold text-rose-500">{{ numberFormat(foundCustomer.current_debt ?? 0) }}đ</span>
                        </div>
                        <div class="flex justify-between border-t pt-1.5 dark:border-slate-800">
                            <span class="font-bold text-slate-500">Khả năng nợ còn lại:</span>
                            <span class="font-mono font-bold text-slate-800 dark:text-slate-200">
                                {{ numberFormat((foundCustomer.credit_limit ?? 0) - (foundCustomer.current_debt ?? 0)) }}đ
                            </span>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex gap-2">
                ...
                <Button
                    ...
                    :disabled="isPaying || (paymentMethod === 'debt' && (!foundCustomer || (!foundCustomer.is_vip && !foundCustomer.is_b2b)))"
