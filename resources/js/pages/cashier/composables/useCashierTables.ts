...
export function useCashierTables(
    ...
    const openSplitOrder = () => {
        ...
        const validItems = activeTable.value.active_order.items.filter(
            (item) => item.status !== 'cancelled',
        );

        const totalOriginalQty = validItems.reduce(
        ...
        if (totalOriginalQty <= 1) {
            toast(
                'Đơn hàng chỉ có 1 món, không thể tách đơn. Đơn gốc phải còn lại tối thiểu 1 món.',
                'error',
            );
    ...
    const processMergeTable = () => {
        ...
        router.post(
            ...
            { target_order_id: selectedMergeTargetOrderId.value },
            {
                preserveScroll: true,
                onSuccess: () =>
                    finishTableAction('Đã gộp hai bàn thành công.'),
                onError: (errors: Record<string, string | string[]>) => {
                    const message = Object.values(errors)[0];
                    toast(
                        Array.isArray(message)
    ...
    const processSplit = () => {
        ...
        if (totalRemainingQty < 1) {
            toast(
                'Không thể tách toàn bộ đơn — Đơn gốc phải còn lại tối thiểu 1 món!',
                'error',
            );
    ...
    const callPayment = async () => {
        ...
        try {
            ...
            if (res.data?.success) {
                ...
                toast(
                    res.data.message ||
                        'Đã gửi thông báo gọi thanh toán cho Thu ngân!',
                );
