...
import {
    ...
    ChevronDown, ChevronUp, Sparkles, Upload, Search
...
const submitRecipe = () => {
    recipeForm.post('/inventory/recipes', {
        onSuccess: () => {
 recipeForm.reset('quantity', 'waste_rate'); showAddRecipe.value = false; 
},
...
const submitWaste = () => {
    wasteForm.post('/inventory/waste', {
        onSuccess: () => {
 toast.success(flashSuccess() ?? 'Đã ghi nhận hao hụt!'); wasteForm.reset(); 
},
...
<template>
    ...
    <div class="flex flex-col gap-6 p-4 lg:p-6 max-w-7xl mx-auto w-full">
        ...
        >
            ...
            <div>
                <span class="font-bold">{{ lowStockIngredients.length }} nguyên liệu SẮP HẾT HÀNG (&lt;5 đơn vị):</span>
                {{ lowStockIngredients.map(i => `${i.name} (còn ${i.stock} ${i.unit?.symbol ?? ''})`).join(' · ') }}.
                    ...
                    <Card class="shadow-sm h-full">
                        <CardHeader class="pb-3 border-b border-border">
                            <div class="flex items-center justify-between gap-2">
                                <div>
                                    <CardTitle class="text-sm font-bold">Tồn Kho Nguyên Liệu ({{ filteredIngredients.length }}/{{ ingredients.length }})</CardTitle>
                                    <CardDescription class="text-[11px]">
                                        <span class="text-rose-500 font-semibold">{{ lowStockIngredients.length }}</span> sắp hết ·
                            ...
                            <div class="relative mt-2">
                                ...
                                <input
                                    ...
                                    class="w-full pl-7 pr-3 py-1.5 text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 focus:outline-none focus:ring-1 focus:ring-indigo-400"
                                />
                            </div>
                        </CardHeader>
