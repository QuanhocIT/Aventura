...
const openComplete = (row: Row) => {
 const disposition = row.source_type === 'supply_request'
     ? 'central_quarantine'
     : (row.source_type === 'stock_transfer' ? 'sender_quarantine' : (row.supplier_id ? 'supplier_confirmed' : 'central_quarantine'));
 Object.assign(completeForm, { row, disposition, notes: '', evidence: null }); completeModal.value = true;
};
...
<template>
    ...
    <div v-if="completeModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" @click.self="completeModal = false"><Card class="w-full max-w-xl"><CardHeader><div class="flex items-center justify-between"><CardTitle>Chốt giao nhận phiếu hoàn</CardTitle><Button variant="ghost" size="icon" @click="completeModal = false"><X class="size-4" /></Button></div></CardHeader><CardContent class="space-y-4"><label class="block text-sm font-medium">Kết quả<select v-model="completeForm.disposition" class="mt-1 w-full rounded-md border bg-background p-2"><option v-if="completeForm.row?.source_type !== 'supply_request' && completeForm.row?.source_type !== 'stock_transfer'" value="supplier_confirmed">Đã giao và bên nhận xác nhận</option><option v-if="completeForm.row?.source_type !== 'stock_transfer' && completeForm.row?.source_type !== 'supply_request'" value="central_quarantine">Chuyển sang cách ly Kho Tổng</option><option v-if="completeForm.row?.source_type === 'supply_request'" value="central_quarantine">Hoàn về cách ly Kho Tổng</option><option v-if="completeForm.row?.source_type === 'stock_transfer'" value="sender_quarantine">Hoàn vào cách ly kho gửi</option><option value="destroyed">Tiêu hủy</option></select></label><p v-if="completeForm.disposition === 'central_quarantine'" class="text-xs text-amber-600">Hệ thống đã tự xác định Kho Tổng là nơi nhận lại hàng hỏng.</p><p v-if="completeForm.disposition === 'sender_quarantine'" class="text-xs text-amber-600">Hệ thống đã tự xác định kho gửi là nơi nhận lại hàng hỏng.</p><label class="block text-sm font-medium">Biên bản đối chiếu<input v-model="completeForm.notes" class="mt-1 w-full rounded-md border bg-background p-2" placeholder="Mã biên bản, người nhận, thời điểm..." /></label><label class="block text-sm font-medium">Bằng chứng <span v-if="completeForm.disposition === 'destroyed'">(bắt buộc khi tiêu hủy)</span><input type="file" accept="image/*,.pdf" class="mt-1 block w-full text-sm" @change="setFile(completeForm, $event)" /></label><div class="flex justify-end gap-2"><Button variant="outline" @click="completeModal = false">Hủy</Button><Button @click="submitComplete">Chốt phiếu</Button></div></CardContent></Card></div>
