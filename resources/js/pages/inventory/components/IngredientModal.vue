...
watch(
    ...
    ([open, ing]) => {
        if (open) {
            if (ing) {
                form.name = ing.name || '';
                form.unit_id = ing.unit?.id
                    ? String(ing.unit.id)
                    : props.units[0]?.id
                      ? String(props.units[0].id)
                      : '';
                form.category = ing.category_name || '';
                form.storage_type = ing.storage_type || 'dry';
                form.default_shelf_life_days =
                    ing.default_shelf_life_days !== null &&
                    ing.default_shelf_life_days !== undefined
                        ? String(ing.default_shelf_life_days)
                        : '';
                form.storage_location = ing.storage_location || '';
                form.expiry_warning_days = ing.expiry_warning_days
                    ? String(ing.expiry_warning_days)
                    : '3';
                form.min_stock_level = ing.min_stock_level
                    ? String(ing.min_stock_level)
                    : '0';
                form.reorder_level = ing.reorder_level
                    ? String(ing.reorder_level)
                    : '0';
                form.supplier_id = ing.supplier_id
                ...
                form.safety_stock_quantity = ing.safety_stock_quantity
                    ? String(ing.safety_stock_quantity)
                    : '0';
                form.lead_time_days =
                    ing.lead_time_days !== null &&
                    ing.lead_time_days !== undefined
                        ? String(ing.lead_time_days)
                        : '0';
                form.batch_tracking_required = Boolean(
                    ing.batch_tracking_required,
                );
                form.storage_temperature_min_c =
                    ing.storage_temperature_min_c !== null &&
                    ing.storage_temperature_min_c !== undefined
                        ? String(ing.storage_temperature_min_c)
                        : '';
                form.storage_temperature_max_c =
                    ing.storage_temperature_max_c !== null &&
                    ing.storage_temperature_max_c !== undefined
                        ? String(ing.storage_temperature_max_c)
                        : '';
...
const submitForm = () => {
    if (props.ingredient) {
        form.put(`/inventory/ingredients/${props.ingredient.id}`, {
            ...
            onError: () =>
                toast.error('Vui lòng kiểm tra lại thông tin nguyên liệu.'),
        });
    } else {
...
<template>
    <Teleport to="body">
        <div
            v-if="isOpen"
            class="fixed inset-0 z-50 overflow-y-auto bg-black/60 p-3 backdrop-blur-xs sm:p-4"
        ...
        >
            ...
            >
                <Card
                    class="flex max-h-[calc(100vh-3rem)] w-full max-w-xl animate-in flex-col overflow-hidden border bg-card text-card-foreground shadow-2xl duration-150 zoom-in-95 fade-in"
                >
                    <CardHeader
                        class="sticky top-0 z-10 flex shrink-0 flex-row items-center justify-between gap-4 border-b bg-background/95 px-6 pt-4 pb-3 backdrop-blur-xs"
                    >
                        <div>
                            <CardTitle
                            ...
                            >
                                ...
                                <span>{{
                                    ingredient
                                        ? 'Chỉnh Sửa Nguyên Liệu Master'
                                        : 'Thêm Nguyên Liệu & Phân Loại Bảo Quản'
                                }}</span>
                            </CardTitle>
                            <CardDescription
                                >Khai báo loại bảo quản (Tươi sống, Đồ khô, Bán
                                trong ngày), thời hạn sử dụng & định mức tồn
                                kho.</CardDescription
                            >
                        </div>
                        <button
                            type="button"
                            @click="emit('close')"
                            class="shrink-0 cursor-pointer rounded-lg p-1.5 text-muted-foreground hover:bg-muted hover:text-foreground"
                        >
                            <X class="size-4" />
                        </button>
                    </CardHeader>
                    ...
                    <CardContent class="flex-1 space-y-4 overflow-y-auto p-6">
                        <form @submit.prevent="submitForm" class="space-y-4">
                            ...
                            <div
                                class="space-y-3 rounded-xl border border-indigo-100 bg-indigo-50/40 p-3.5 dark:border-indigo-900/40 dark:bg-indigo-950/20"
                            >
                                <div
                                ...
                                >
                                    <div class="space-y-1.5">
                                        ...
                                        >
                                            <option value="fresh">
                                                🥬 Hàng tươi sống (Hạn 1-3 ngày)
                                            </option>
                                            <option value="daily">
                                                🥖 Bán trong ngày (Hết ca / Cuối
                                                ngày)
                                            </option>
                                            <option value="short_shelf">
                                                🥛 Hạn ngắn (5-15 ngày)
                                            </option>
                                            <option value="canned_packaged">
                                                🥫 Đồ đóng hộp / Đóng gói
                                            </option>
                                            <option value="dry">
                                                🌾 Đồ khô & Gia vị (Hạn dài)
                                            </option>
                                        </select>
                                    </div>
                                ...
                                <div
                                    v-if="form.storage_type === 'daily'"
                                    class="flex items-center gap-2 pt-1 text-xs"
                                >
                                    <input
                                        id="auto_waste_chk"
                                        type="checkbox"
                                    ...
                                    <Label
                                        for="auto_waste_chk"
                                        class="cursor-pointer font-medium text-amber-700 dark:text-amber-400"
                                    >
                                        Tự động gợi ý xuất hủy / chuyển dùng hết
                                        vào cuối ngày
                                    </Label>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div class="space-y-1.5">
                                    <Label class="text-xs font-semibold"
                                        >Tồn kho tối thiểu (Min Stock)</Label
                                    >
                                    <Input
                                ...
                                <div class="space-y-1.5">
                                    <Label class="text-xs font-semibold"
                                        >Điểm đặt hàng lại (Reorder)</Label
                                    >
                                    <Input
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div class="space-y-1.5">
                                    ...
                                    >
                                    <Input
                                        v-model="form.safety_stock_quantity"
                                        type="number"
                                        step="any"
                                        min="0"
                                        placeholder="0"
                                ...
                                <div class="space-y-1.5">
                                    <Label class="text-xs font-semibold"
                                        >Lead time nhà cung cấp (ngày)</Label
                                    >
                                    <Input
                            ...
                            <div
                                class="space-y-3 rounded-xl border border-sky-200 bg-sky-50/50 p-3.5 dark:border-sky-900/40 dark:bg-sky-950/20"
                            >
                                <div
                                ...
                                >
                                    <div>
                                        <Label class="text-xs font-semibold"
                                            >Kiểm soát truy xuất & nhiệt
                                            độ</Label
                                        >
                                        <p
                                            class="mt-1 text-[11px] text-muted-foreground"
                                        >
                                            Dùng cho nguyên liệu tươi, kho lạnh
                                            hoặc mặt hàng cần quản lý theo lô.
                                        </p>
                                    </div>
                                    <label
