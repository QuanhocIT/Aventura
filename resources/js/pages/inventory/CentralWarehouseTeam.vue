...
function openSupervisorModal(staff: StaffMember) {
    ...
    supervisorForm.supervisor_user_id = staff.supervisor_id
        ? String(staff.supervisor_id)
        : '';
...
function toggleStaffStatus(staff: StaffMember) {
    const newStatus =
        staff.warehouse_staff_status === 'active' ? 'paused' : 'active';
...
function openReassignModal(task: TaskAssignment) {
    ...
    reassignForm.new_assigned_to = task.assignee
    ...
    <div class="mx-auto flex w-full max-w-7xl flex-col gap-6 p-4 sm:p-6">
        ...
        >
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400">
                        {{ centralBranch?.name || 'Kho Tổng' }} • Quản lý nhân
                        sự, Trưởng kho trực tiếp, xếp ca và chỉ số KPI.
                    </p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                <Button
                    ...
                    class="gap-2 bg-indigo-600 font-semibold text-white hover:bg-indigo-700 dark:bg-indigo-500"
                >
                    <Plus class="size-4" />
                    Giao Nhiệm Vụ Mới
                </Button>
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Card class="border-indigo-100 shadow-sm dark:border-indigo-950/30">
                ...
                <CardContent>
                    ...
                    <p class="mt-0.5 text-xs text-muted-foreground">
                        thuộc phạm vi Kho Tổng
                    </p>
                </CardContent>
            </Card>

            <Card
                class="border-emerald-100 shadow-sm dark:border-emerald-950/30"
            >
                <CardHeader
                    class="flex flex-row items-center justify-between pb-2"
                >
                    <CardDescription
                        ...
                        >Nhiệm Vụ Đang Làm</CardDescription
                    >
            ...
            <Card class="border-amber-100 shadow-sm dark:border-amber-950/30">
                ...
                >
                    <CardDescription
                        ...
                        >Đơn Xin Nghỉ Phép</CardDescription
                    >
            ...
            <Card class="border-purple-100 shadow-sm dark:border-purple-950/30">
                ...
                >
                    <CardDescription
                        class="text-xs font-bold tracking-wider text-purple-500 uppercase"
                        >KPI Đội Ngũ</CardDescription
                    >
                    <Award class="size-4 text-purple-500" />
                </CardHeader>
                <CardContent>
                    <p
                        class="text-2xl font-black text-purple-600 tabular-nums dark:text-purple-400"
                    >
                        {{
                            teamKpi.length
                                ? (
        ...
        <div
            class="flex items-center border-b border-slate-200 dark:border-slate-800"
        >
            <button
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                <Card
                    ...
                    :class="[
                        ...
                        staff.warehouse_staff_status === 'paused'
                            ? 'border-amber-300 opacity-80 dark:border-amber-900/50'
                            : '',
                    ]"
                >
                    <CardHeader
                        class="border-b bg-slate-50/50 pb-3 dark:bg-slate-900/50"
                    >
                            ...
                            <span
                                :class="[
                                    ...
                                    staff.warehouse_staff_status === 'active'
                                        ? 'border-emerald-200 bg-emerald-500/10 text-emerald-600 dark:border-emerald-900'
                    ...
                    <CardContent class="flex flex-col gap-3 p-4 text-xs">
                        <div
                            class="flex items-center justify-between rounded-lg bg-slate-50 p-2 dark:bg-slate-800/40"
                        >
                            <span class="text-slate-500"
                                >Trưởng Kho trực tiếp:</span
                            >
                            <span
                                class="font-semibold text-slate-800 dark:text-slate-200"
                                >{{ staff.supervisor_name }}</span
                            >
                        </div>

                        <div class="grid grid-cols-2 gap-2">
                            <div
                                class="rounded-lg border p-2 dark:border-slate-800"
                            >
                                <p
                        ...
                        >
                            ...
                            <Button
                                @click="openSupervisorModal(staff)"
                                variant="outline"
                                size="sm"
                                class="h-8 gap-1 text-[11px]"
                            >
                            ...
                            <Button
                                @click="openAssignTaskModal(staff)"
                                size="sm"
                                class="h-8 gap-1 bg-indigo-600 text-[11px] text-white hover:bg-indigo-700"
                            >
                                <Plus class="size-3.5" />
                                Giao Việc
                            </Button>
                ...
                >
                    <div>
                        <CardTitle class="text-base font-bold"
                            >Danh Sách Nhiệm Vụ Kho Tổng</CardTitle
                        >
                        <CardDescription
                            >Theo dõi tiến độ, người phụ trách và điều chuyển
                            công việc.</CardDescription
                        >
                    </div>
                    <Button
                        @click="openAssignTaskModal()"
                        size="sm"
                        class="gap-1.5 bg-indigo-600 text-xs text-white hover:bg-indigo-700"
                    >
                        <Plus class="size-4" />
                        Giao Việc Mới
                    </Button>
                    ...
                    >
                        ...
                        >
                            <div class="flex items-start gap-3">
                                <div
                                    class="mt-0.5 flex size-9 items-center justify-center rounded-xl bg-indigo-50 text-indigo-600 dark:bg-indigo-950 dark:text-indigo-400"
                                >
                                    <ClipboardList class="size-4" />
                                </div>
        ...
        <div v-if="activeTab === 'shifts'" class="flex flex-col gap-4">
            <Card class="overflow-hidden shadow-sm">
                <CardHeader
                    class="border-b bg-slate-50/50 pb-4 dark:bg-slate-900/50"
                >
                    <CardTitle class="text-base font-bold"
                        >Duyệt Đơn Nghỉ Phép & Đổi Ca</CardTitle
                    >
                    <CardDescription
                        >Trưởng kho xem xét và phê duyệt nghỉ phép cho nhân viên
                        thuộc kho của mình.</CardDescription
                    >
                </CardHeader>
                        ...
                        >
                            <div class="flex flex-col gap-1">
                                <div class="flex items-center gap-2">
                                    <span
                                        class="font-bold text-slate-800 dark:text-slate-200"
                                        >{{ leave.user.name }}</span
                                    >
                                    <span
                                        class="rounded bg-slate-100 px-2 py-0.5 text-[10px] font-semibold text-slate-600 dark:bg-slate-800 dark:text-slate-300"
                                    >
                                        {{ leave.leave_type }}
                                    </span>
                                    <span
                                ...
                                <p class="text-xs text-slate-500">
                                    Thời gian: {{ leave.start_date }} đến
                                    {{ leave.end_date }}
                                </p>
                                <p
                                    v-if="leave.reason"
                            ...
                            >
                                <Button
                                    @click="
                                        handleLeaveProcess(leave.id, 'rejected')
                                    "
                                    variant="outline"
                                    size="sm"
                                    class="h-8 border-rose-200 text-xs text-rose-600 hover:bg-rose-50"
                                >
                                    Từ Chối
                                </Button>
                                <Button
                                    @click="
                                        handleLeaveProcess(leave.id, 'approved')
                                    "
                                    size="sm"
        ...
        <div v-if="activeTab === 'kpi'" class="flex flex-col gap-4">
            <Card class="overflow-hidden shadow-sm">
                ...
                >
                    <CardTitle class="text-base font-bold"
                        >Bảng Xếp Hạng & Báo Cáo KPI Đội Ngũ</CardTitle
                    >
                    <CardDescription
                        >Đánh giá tổng hợp dựa trên tỷ lệ hoàn thành, đúng hạn,
                        sai lệch nhận hàng và quy trình bàn
                ...
                <CardContent class="p-0">
                    <div class="overflow-x-auto">
                        <table class="w-full text-left text-xs">
                            <thead
                                class="bg-slate-50 text-[11px] text-slate-500 uppercase dark:bg-slate-900"
                            >
                                <tr>
                            ...
                            >
                                <tr
                                    v-for="item in teamKpi"
                                    :key="item.id"
                                    class="hover:bg-slate-50/50 dark:hover:bg-slate-900/30"
                                >
                                    <td
                                        class="px-4 py-3 font-black text-slate-700 dark:text-slate-300"
                                    >
                                        #{{ item.rank }}
                                    </td>
                                    <td
                                        class="px-4 py-3 font-semibold text-slate-800 dark:text-slate-100"
                                    >
                                        {{ item.name }}
                                    </td>
                                    <td
                                    ...
                                    >
                                        {{ item.avg_duration_minutes }} phút
                                    </td>
                                    <td
                                        class="px-4 py-3 text-center font-bold"
                                        :class="
                                            item.discrepancy_rate > 0
                                                ? 'text-rose-600'
                                                : 'text-slate-600'
                                        "
                                    >
                                    ...
                                    <td
                                        class="px-4 py-3 text-right font-black text-purple-600 dark:text-purple-400"
                                    >
        ...
        <Teleport to="body">
            ...
            >
                ...
                >
                    ...
                    >
                        Bổ Nhiệm Trưởng Kho Trực Tiếp
                    </h3>
                    <p class="mt-1 text-xs text-slate-500">
                        Gán Trưởng kho quản lý trực tiếp cho nhân viên
                        <strong>{{ selectedStaffForSupervisor?.name }}</strong
                        >.
                    </p>

                    <form
                        @submit.prevent="submitSupervisorAssignment"
                        class="mt-4 flex flex-col gap-4"
                    >
                        <div class="flex flex-col gap-1.5">
                            <Label class="text-xs font-bold"
                                >Chọn Trưởng kho quản lý</Label
                            >
                            <select
                        ...
                        <div class="flex flex-col gap-1.5">
                            <Label class="text-xs font-bold"
                                >Ghi chú bổ nhiệm</Label
                            >
                            <Input
                                v-model="supervisorForm.notes"
                                placeholder="Lý do bổ nhiệm / phạm vi phụ trách..."
                                class="text-xs"
                            />
                        </div>
            ...
            >
                ...
                >
                    <h3
                        class="text-base font-bold text-slate-800 dark:text-slate-100"
                    >
                        Điều Chuyển Nhiệm Vụ #{{ selectedTaskForReassign?.id }}
                    </h3>
                    ...
                    >
                        <div class="flex flex-col gap-1.5">
                            <Label class="text-xs font-bold"
                                >Chuyển cho nhân viên khác</Label
                            >
                            <select
                        ...
                        <div class="flex flex-col gap-1.5">
                            <Label class="text-xs font-bold"
                                >Lý do điều chuyển</Label
                            >
                            <Input
                                v-model="reassignForm.reason"
                                placeholder="Quá tải, bận ca khác, hỗ trợ gấp..."
                                class="text-xs"
                            />
                        </div>
