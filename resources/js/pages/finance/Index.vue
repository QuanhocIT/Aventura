...
type CloseCheck = { ok: boolean; blocking: boolean; count: number; message: string };
...
return;
    ...
    const blocking = closeChecks.value.filter((check) => check.blocking && !check.ok);
    if (blocking.length) {
...
function reopenPeriod() {
    ...
    router.patch(`/finance/periods/${props.period.id}/reopen`, { reason }, { preserveScroll: true });
}
...
function removeEntryLine(index: number) {
    if (entryForm.value.lines.length <= 2) return;
    ...
    router.post('/finance/entries', entryForm.value, {
        ...
        onSuccess: () => {
            ...
            entryForm.value = {
                ...
                description: '',
                idempotency_key: '',
                lines: [{ account: '6271', debit: 0, credit: 0 }, { account: '1111', debit: 0, credit: 0 }],
            };
        },
    });
}
...
<template>
    ...
    <div class="space-y-6 p-4 sm:p-6">
        <FinancePageHeader title="Sổ tài chính" description="Bút toán, bảng cân đối phát sinh và khóa kỳ." :icon="BookOpen">
            <template #actions>
                <Button v-if="canManageAccounts" type="button" variant="outline" class="gap-2" @click="showEntryForm = !showEntryForm">
                ...
                <Button v-if="period.status !== 'closed' && canClose" type="button" class="gap-2" @click="closePeriod">
                    <LockKeyhole class="size-4" />
                    Khóa kỳ
                </Button>
                <Button v-if="period.status === 'closed' && canReopen" type="button" variant="outline" class="gap-2" @click="reopenPeriod">
                    <LockKeyhole class="size-4" /> Mở lại kỳ
                </Button>
                <span v-if="period.status === 'closed'" class="inline-flex h-9 items-center gap-2 rounded-md bg-emerald-500/10 px-3 text-sm font-medium text-emerald-600 ring-1 ring-emerald-500/20">
        ...
        <section class="rounded-2xl border border-border/60 bg-card/80 p-5 shadow-sm">
            <div class="mb-3 flex items-center gap-2"><ClipboardCheck class="size-4 text-blue-600" /><h2 class="font-semibold">Checklist đóng kỳ</h2></div>
            <div class="grid gap-2 md:grid-cols-2">
                <div v-for="(check, key) in closeChecklist" :key="key" class="flex items-center justify-between rounded-lg border px-3 py-2 text-sm" :class="check.ok ? 'border-emerald-200 bg-emerald-50/60' : check.blocking ? 'border-rose-200 bg-rose-50/60' : 'border-amber-200 bg-amber-50/60'"><span>{{ check.message }}</span><span class="text-xs font-semibold">{{ check.ok ? 'OK' : check.blocking ? 'CẦN XỬ LÝ' : 'THEO DÕI' }}</span></div>
            </div>
        </section>

        <section v-if="showEntryForm" class="rounded-2xl border border-blue-200 bg-blue-50/40 p-5 shadow-sm">
            <form class="space-y-4" @submit.prevent="saveEntry">
                <div class="grid gap-3 md:grid-cols-3"><input v-model="entryForm.entry_date" required type="date" class="rounded-lg border-slate-300 text-sm" /><input v-model="entryForm.description" required minlength="3" placeholder="Nội dung bút toán" class="rounded-lg border-slate-300 text-sm md:col-span-2" /></div>
                <div v-for="(line, index) in entryForm.lines" :key="index" class="grid gap-2 md:grid-cols-[1fr_180px_180px_auto]"><select v-model="line.account" required class="rounded-lg border-slate-300 text-sm"><option value="" disabled>Chọn tài khoản</option><option v-for="account in accounts" :key="account.code" :value="account.code">{{ account.code }} — {{ account.name }}</option></select><input v-model.number="line.debit" type="number" min="0" step="0.01" placeholder="Nợ" class="rounded-lg border-slate-300 text-sm" /><input v-model.number="line.credit" type="number" min="0" step="0.01" placeholder="Có" class="rounded-lg border-slate-300 text-sm" /><Button type="button" variant="outline" @click="removeEntryLine(index)">Xóa</Button></div>
                <div class="flex gap-2"><Button type="button" variant="outline" @click="addEntryLine">Thêm dòng</Button><Button type="submit">Ghi sổ</Button></div>
            </form>
        </section>
