...
const policyForm = useForm({
    ...
    require_employee_acceptance:
        props.policySettings.require_employee_acceptance,
});
...
const selectedEmployee = computed(() =>
    props.canManage
        ? (props.employees.find(
              (employee) => employee.id === Number(form.employee_id),
          ) ?? null)
...
const selectedPolicy = computed(
    () =>
        props.policy.types.find((item) => item.value === form.overtime_type) ??
        props.policy.types[0],
);
...
const filteredRequests = computed(() => {
    ...
    return props.requests.filter((item) => {
        ...
        const haystack =
            `${item.employee_name ?? ''} ${item.employee_code ?? ''} ${item.reason ?? ''}`.toLocaleLowerCase(
                'vi-VN',
            );
...
const reconcile = (item: OvertimeRequest) => {
    const hours = window.prompt(
        'Số giờ OT thực tế:',
        String(item.worked_hours || item.hours_approved),
    );
    ...
    const reason = window.prompt(
        'Lý do đối soát / điều chỉnh:',
        'Đã kiểm tra chấm công OT',
    );
    ...
    router.patch(
        `/overtime-requests/${item.id}/reconcile`,
        { worked_hours: Number(hours), adjustment_reason: reason },
        { preserveScroll: true },
    );
};
...
const saveHoliday = () => {
    holidayForm.post('/overtime-holidays', {
        preserveScroll: true,
...
<template>
    ...
    <div class="mx-auto flex w-full max-w-7xl flex-col gap-6 p-6">
        <header
            class="flex flex-col gap-4 border-b border-border/70 pb-6 sm:flex-row sm:items-center sm:justify-between"
        >
            <div class="flex items-center gap-3">
                <div
                    class="flex size-12 items-center justify-center rounded-2xl bg-indigo-500/10 text-indigo-500"
                >
                    <Clock3 class="size-6" />
                </div>
                <div>
                    ...
                    <p class="text-sm text-muted-foreground">
                        Đăng ký, duyệt, chấm công và tính tiền OT theo đúng giờ
                        thực tế.
                    </p>
                </div>
            </div>
        ...
        <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
            <div class="rounded-2xl border bg-card p-5 shadow-sm">
                ...
                <p class="mt-1 text-xs text-muted-foreground">
                    Trong phạm vi dữ liệu hiện tại
                </p>
            </div>
            <div
            ...
            >
                ...
                <p class="mt-1 text-xs text-muted-foreground">
                    Cần quản lý hoặc nhân viên phản hồi
                </p>
            </div>
            <div
            ...
            >
                ...
                <p class="mt-2 text-3xl font-black text-emerald-600">
                    {{ summary.approvedHours.toFixed(2) }}h
                </p>
                <p class="mt-1 text-xs text-muted-foreground">
                    Sẽ tính vào bảng lương khi đủ chấm công
                </p>
            </div>
            <div
                class="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-5"
            >
                ...
                <p class="mt-2 text-2xl font-black text-emerald-600">
                    {{ vnd(summary.approvedAmount) }}
                </p>
                <p class="mt-1 text-xs text-muted-foreground">
                    Theo đơn giá và hệ số đã snapshot
                </p>
            </div>
            <div
                class="rounded-2xl border border-indigo-500/20 bg-indigo-500/5 p-5"
            >
                <p
                    class="text-xs font-semibold tracking-wide text-indigo-700 uppercase dark:text-indigo-300"
                >
                    Chờ nhân viên xác nhận
                </p>
                <p class="mt-2 text-3xl font-black text-indigo-600">
                    {{ summary.waitingEmployee }}
                </p>
                <p class="mt-1 text-xs text-muted-foreground">
                    Yêu cầu đột xuất đã gửi email
                </p>
            </div>
        </div>

        <div
            class="flex flex-col gap-3 rounded-2xl border border-indigo-500/20 bg-indigo-500/5 p-4 text-sm sm:flex-row sm:items-center"
        >
            <ShieldCheck class="size-5 shrink-0 text-indigo-500" />
            <p class="text-muted-foreground">
                <span class="font-semibold text-foreground">Cách tính:</span>
                đơn giá giờ quy đổi × hệ số OT × giờ thực tế (không vượt giờ
                được duyệt).
            </p>
            <span
                v-if="props.canManage"
                class="inline-flex items-center gap-1 text-xs font-semibold text-indigo-600 sm:ml-auto"
                ><Mail class="size-3.5" /> Yêu cầu đột xuất sẽ gửi Gmail</span
            >
            <span
                v-else
                class="text-xs font-semibold text-indigo-600 sm:ml-auto"
                >Giới hạn: {{ props.policy.max_daily_hours }}h/ngày ·
                {{ props.policy.max_monthly_hours }}h/tháng</span
            >
        </div>

        <section
            v-if="props.canManage && props.report"
        ...
        >
            <div class="rounded-2xl border bg-card p-5 shadow-sm">
                ...
                >
                    <div>
                        <h2 class="font-bold">
                            Báo cáo chi phí OT · {{ props.report.period }}
                        </h2>
                        <p class="text-xs text-muted-foreground">
                            Dùng để đối soát trước khi chốt bảng lương.
                        </p>
                    </div>
                    <Button size="sm" variant="outline" @click="exportReport"
                        >Xuất CSV</Button
                    >
                </div>
                <div class="grid gap-3 sm:grid-cols-4">
                    <div class="rounded-xl bg-muted/40 p-3">
                        <p class="text-xs text-muted-foreground">Đơn OT</p>
                        <p class="mt-1 text-xl font-black">
                            {{ props.report.total_requests }}
                        </p>
                    </div>
                    <div class="rounded-xl bg-muted/40 p-3">
                        <p class="text-xs text-muted-foreground">Giờ thực tế</p>
                        <p class="mt-1 text-xl font-black">
                            {{ props.report.worked_hours.toFixed(2) }}h
                        </p>
                    </div>
                    <div class="rounded-xl bg-emerald-500/10 p-3">
                        <p class="text-xs text-muted-foreground">Tiền OT</p>
                        <p class="mt-1 text-lg font-black text-emerald-600">
                            {{ vnd(props.report.actual_amount) }}
                        </p>
                    </div>
                    <div class="rounded-xl bg-amber-500/10 p-3">
                        ...
                        <p class="mt-1 text-xl font-black text-amber-600">
                            {{ props.report.pending_reconciliation }}
                        </p>
                    </div>
                </div>
                <div
                    v-if="props.report.by_employee.length"
                    class="mt-4 divide-y rounded-xl border"
                >
                    <div
                        v-for="row in props.report.by_employee"
                        :key="
                            (row.employee_code ?? row.employee_name) as string
                        "
            ...
            <div class="rounded-2xl border bg-card p-5 shadow-sm">
                <div class="mb-4 flex items-center gap-2">
                    <Settings2 class="size-4 text-indigo-500" />
                    <h2 class="font-bold">Chính sách OT áp dụng từ</h2>
                </div>
                <form
                ...
                >
                    <label class="flex items-center gap-2 text-xs sm:col-span-2"
                        ><input
                            v-model="policyForm.require_qr"
                            type="checkbox"
                        />
                        Bắt buộc QR khi chấm công OT</label
                    >
                    ...
                    <div
                        v-for="field in [
                            { key: 'normal_multiplier', label: 'Ngày thường' },
                            { key: 'night_multiplier', label: 'Ban đêm' },
                            { key: 'rest_day_multiplier', label: 'Ngày nghỉ' },
                            { key: 'holiday_multiplier', label: 'Ngày lễ' },
                        ]"
                        :key="field.key"
                        class="grid gap-1.5"
                    >
                        <Label>{{ field.label }} (x)</Label
                        ><Input
                            v-model.number="
                                policyForm[field.key as keyof typeof policyForm]
                            "
                            type="number"
                            min="1"
                            max="10"
                    ...
                    <div class="grid gap-1.5">
                        ...
                        ><Input
                            v-model.number="policyForm.max_daily_hours"
                            type="number"
                            min="0.25"
                            step="0.25"
                        />
                    ...
                    <div class="grid gap-1.5">
                        <Label>Tối đa/tuần</Label
                        ><Input
                            v-model.number="policyForm.max_weekly_hours"
                            type="number"
                            min="0.25"
                            step="0.25"
                        />
                    ...
                    <div class="grid gap-1.5">
                        <Label>Tối đa/tháng</Label
                        ><Input
                            v-model.number="policyForm.max_monthly_hours"
                            type="number"
                            min="0.25"
                            step="0.25"
                        />
                    ...
                    <div class="grid gap-1.5">
                        <Label>Nghỉ tối thiểu (giờ)</Label
                        ><Input
                            v-model.number="policyForm.minimum_rest_hours"
                            type="number"
                            min="0"
                            step="0.5"
                        />
                    ...
                    <label class="flex items-center gap-2 text-xs sm:col-span-2"
                        ><input
                            v-model="policyForm.require_gps"
                            type="checkbox"
                        />
                        Bắt buộc GPS khi chấm công OT</label
                    >
                    <label class="flex items-center gap-2 text-xs sm:col-span-2"
                        ><input
                            v-model="policyForm.require_photo"
                            type="checkbox"
                        />
                        Bắt buộc ảnh xác nhận khi chấm công OT</label
                    >
                    <Button
                        type="submit"
                        class="bg-indigo-600 text-white hover:bg-indigo-700 sm:col-span-2"
                        :disabled="policyForm.processing"
                        >Lưu chính sách</Button
                    >
                </form>
                <div class="mt-5 border-t pt-4">
                    <p class="mb-2 text-xs font-semibold text-muted-foreground">
                        Lịch ngày lễ / ngày đặc biệt
                    </p>
                    <form
                        class="grid gap-2 sm:grid-cols-[1fr_1.5fr_0.7fr_auto]"
                    ...
                    >
                        <div
                            v-for="holiday in props.holidays.slice(0, 5)"
                            :key="holiday.id"
                            class="flex items-center justify-between rounded-lg bg-muted/40 px-2 py-1.5"
                        >
                            <span
        ...
        <section class="overflow-hidden rounded-2xl border bg-card shadow-sm">
            <div
                class="flex flex-col gap-3 border-b border-border/70 p-4 lg:flex-row lg:items-center lg:justify-between"
            >
                <div>
                    <h2 class="font-bold">Danh sách yêu cầu</h2>
                    <p class="text-xs text-muted-foreground">
                        Duyệt, từ chối hoặc theo dõi phản hồi của nhân viên.
                    </p>
                </div>
                <div class="flex flex-col gap-2 sm:flex-row">
                    ...
                    >
                        ...
                        <button
                            ...
                            "
                            @click="
                                statusFilter = filter.value as 'all' | Status
                            "
                        >
                            {{ filter.label }}
                        </button>
            ...
            <div v-if="filteredRequests.length === 0" class="p-12 text-center">
                ...
                <p class="mt-1 text-sm text-muted-foreground">
                    Các đơn tăng ca mới sẽ xuất hiện tại đây.
                </p>
            </div>
            ...
            <div v-else class="divide-y divide-border/70">
                <article
                    ...
                    :key="item.id"
                    class="flex flex-col gap-4 p-5 transition-colors hover:bg-muted/20 lg:flex-row lg:items-center lg:justify-between"
                >
                    <div class="flex min-w-0 items-start gap-3">
                        ...
                        <div class="min-w-0">
                            ...
                            >
                                ...
                                <span>·</span>
                                <span>{{
                                    sourceLabel(item.request_source)
                                }}</span>
                                <span v-if="item.requester_name"
                                    >· Gửi bởi {{ item.requester_name }}</span
                                >
                            </div>
                            <p
                                v-if="item.reason"
                            ...
                            <p
                                v-if="item.check_in_at"
                                class="mt-2 text-xs font-medium text-emerald-600"
                            >
                                <CheckCircle2 class="mr-1 inline size-3.5" />
                                Đã chấm {{ item.worked_hours.toFixed(2) }}h thực
                                tế{{
                                    item.payroll_status === 'ready'
                                        ? ', đủ điều kiện tính lương'
                                        : ''
                                }}
                            </p>
                        </div>
                    ...
                    <div class="flex shrink-0 flex-col gap-3 lg:items-end">
                        <div class="text-left lg:text-right">
                            ...
                            <p class="text-xs text-muted-foreground">
                                {{
                                    item.status === 'approved'
                                        ? `${item.hours_approved} giờ được duyệt`
                                        : 'Dự kiến tăng ca'
                                }}
                            </p>
                            <p
                                v-if="item.estimated_amount"
                            ...
                            >
                                {{
                                    item.actual_amount
                                        ? 'Tiền theo giờ thực tế'
                                        : 'Tiền dự kiến nếu làm đủ'
                                }}
                            </p>
                        </div>
                        <div class="flex flex-wrap gap-2 lg:justify-end">
                            <template v-if="canCheckIn(item)">
                                <Button
                                    ...
                                    @click="attendance(item.id, 'check-in')"
                                    >Check-in OT</Button
                                >
                            </template>
                            <template v-if="canCheckOut(item)">
                                <Button
                                    ...
                                    @click="attendance(item.id, 'check-out')"
                                    >Check-out OT</Button
                                >
                            </template>
                            <template
                                v-if="
                                    props.canManage &&
                                    item.status === 'pending' &&
                                    (item.request_source === 'employee' ||
                                        item.employee_response === 'accepted')
                                "
                            >
                                <Button
                                    size="sm"
    ...
    <Teleport to="body">
        ...
        >
            <div
                class="w-full max-w-xl rounded-2xl border bg-card p-6 shadow-2xl"
            >
                <div
                    class="mb-5 flex items-start justify-between border-b border-border/70 pb-4"
                >
                    <div>
                        ...
                        <p class="mt-1 text-xs text-muted-foreground">
                            {{
                                props.canManage
                                    ? 'Nhân viên sẽ nhận thông báo và Gmail để xác nhận.'
                                    : 'Đơn sẽ được chuyển tới quản lý để duyệt.'
                            }}
                        </p>
                    </div>
                    <button
                ...
                >
                    <div
                        v-if="!props.canManage && props.currentEmployee"
                        class="flex items-center gap-3 rounded-xl border border-indigo-500/20 bg-indigo-500/5 p-3 sm:col-span-2"
                    >
                        <UserRound class="size-5 text-indigo-500" />
                        <div>
                            <p class="text-sm font-semibold">
                                {{ props.currentEmployee.full_name }} ({{
                                    props.currentEmployee.employee_code
                                }})
                            </p>
                            <p class="text-xs text-muted-foreground">
                                Đơn giá OT quy đổi:
                    ...
                    <div class="grid gap-1.5">
                        ...
                        >
                            <option
                                v-for="type in props.policy.types"
                                :key="type.value"
                                :value="type.value"
                            >
                    ...
                    <div class="grid gap-1.5">
                        ...
                        <Input
                            v-model="form.hours_requested"
                            type="number"
                            min="0.25"
                            :max="props.policy.max_daily_hours"
                            step="0.25"
                            required
                        />
                        <p class="text-[11px] text-muted-foreground">
                            Tối đa {{ props.policy.max_daily_hours }}h/ngày và
                            {{ props.policy.max_monthly_hours }}h/tháng.
                        </p>
                    </div>
                    <div
                        class="flex items-center gap-3 rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-3 sm:col-span-2"
                    >
                        ...
                        >
                            ...
                            <p class="text-right text-xs text-muted-foreground">
                                {{
                                    vnd(
                                        Number(
                                            selectedEmployee?.overtime_hourly_rate ||
                                                0,
                                        ),
                                    )
                                }}/giờ × {{ selectedPolicy?.multiplier ?? 0 }}x
                                × {{ Number(form.hours_requested || 0) }}h
                            </p>
                        </div>
                    </div>
                    <div class="grid gap-1.5 sm:col-span-2">
                        ...
                        <textarea
                            v-model="form.reason"
                            rows="4"
                            class="w-full rounded-md border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-500"
                            placeholder="Thiếu nhân sự, đơn hàng tăng đột biến..."
                        />
                    </div>
                    <div
                        class="flex justify-end gap-2 border-t border-border/70 pt-4 sm:col-span-2"
                    >
                        <Button
                            type="button"
                            variant="outline"
