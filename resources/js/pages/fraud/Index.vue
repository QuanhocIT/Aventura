...
function applyPeriod() {
    router.get(
        '/fraud',
        { period: activePeriod.value, tab: props.activeTab },
        { preserveScroll: true },
    );
}
...
const tabs = computed(() => [
    {
        ...
        label: 'AI Cảnh báo',
        icon: ShieldAlert,
        count: props.summary.ai_flagged_alerts ?? 0,
        color: 'rose',
    },
    {
        key: 'audit' as TabKey,
        label: 'Kiểm toán',
        icon: List,
        count: props.summary.audit_logs_count ?? 0,
        color: 'blue',
    },
    {
        key: 'cash' as TabKey,
        label: 'Thiếu quỹ',
        icon: Banknote,
        count: props.summary.cash_shortfall_cashiers,
        color: 'rose',
    },
    {
        key: 'discount' as TabKey,
        label: 'Chiết khấu',
        icon: Percent,
        count: props.summary.discount_flagged_days,
        color: 'amber',
    },
    {
        ...
        label: 'Hủy đơn',
        icon: XCircle,
        count: props.summary.cancellation_cashiers,
        color: 'rose',
    },
    {
        key: 'waste' as TabKey,
        label: 'Hao hụt kho',
        icon: Package,
        count: props.summary.waste_flagged_entries,
        color: 'amber',
    },
    {
        key: 'revenue' as TabKey,
        label: 'Doanh thu',
        icon: BarChart3,
        count: props.summary.revenue_flagged_days,
        color: 'orange',
    },
]);
...
const severityConfig: Record<Severity, { label: string; cls: string }> = {
    ...
    critical: {
        label: 'Nghiêm trọng',
        cls: 'bg-red-200 text-red-800 dark:bg-red-900/60 dark:text-red-200',
    },
    warning: {
        label: 'Cảnh báo',
        cls: 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300',
...
async function overrideSplit(orderId: number) {
    if (
        !(await confirmDialog({
            ...
            description:
                'Bạn có chắc chắn muốn phê duyệt đối soát và gỡ bỏ khoản phạt âm tiền cho đơn hàng này?',
            variant: 'default',
        }))
    ) {
        return;
    ...
    router.patch(
        ...
        {
            onSuccess: () =>
                toast.success('Đã phê duyệt đối soát đơn tách thành công!'),
...
function openViolation(target: ViolationTarget) {
    ...
    violationForm.penalty_amount =
        target.penalty_amount > 0 ? String(target.penalty_amount) : '';
    ...
    <div class="mx-auto max-w-7xl space-y-6 p-4 lg:p-6">
        ...
        >
            <div class="flex items-center gap-3">
                <div
                    class="flex h-10 w-10 items-center justify-center rounded-xl bg-rose-500/10 text-rose-500"
                >
            ...
            <input
                ...
                type="month"
                @change="applyPeriod"
                class="rounded-xl border border-border bg-card px-3 py-2 text-sm focus:border-rose-400 focus:ring-2 focus:ring-rose-500/20 focus:outline-none"
            />
        </div>
        ...
        <div class="grid grid-cols-2 gap-3 lg:grid-cols-7">
            <button
                ...
                class="cursor-pointer rounded-2xl border border-border bg-card p-4 text-left shadow-sm transition hover:shadow-md active:scale-95"
                :class="
                    activeTab === tab.key
            ...
            >
                ...
                <p
                    ...
                    :class="
                        tab.count > 0 ? 'text-rose-600 dark:text-rose-400' : ''
                    "
                >
                    {{ tab.count }}
                </p>
            ...
            >
                ...
                >
                    ...
                    >
                        <div class="flex items-center gap-3">
                            <div
                                class="flex h-9 w-9 items-center justify-center rounded-xl bg-rose-500/10 text-rose-600"
                            >
                                <ShieldAlert class="size-5" />
                            </div>
                            <div>
                                ...
                                <p class="text-xs text-muted-foreground">
                                    {{ violationTarget.employee_name }}
                                </p>
                            </div>
                        </div>
                        <button
                            @click="violationTarget = null"
                    ...
                    <div class="space-y-4 px-6 py-5">
                        <div class="space-y-1.5">
                            ...
                            >
                            <input
                                v-model="violationForm.violation_type"
                                type="text"
                                maxlength="100"
                        ...
                        <div class="space-y-1.5">
                            ...
                            <select
                                v-model="violationForm.severity"
                                class="w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm focus:border-rose-400 focus:ring-2 focus:ring-rose-500/20 focus:outline-none"
                            >
                        ...
                        <div class="space-y-1.5">
                            ...
                            <textarea
                                ...
                                rows="3"
                                maxlength="2000"
                                class="w-full resize-none rounded-xl border border-border bg-background px-3 py-2.5 text-sm focus:border-rose-400 focus:ring-2 focus:ring-rose-500/20 focus:outline-none"
                            />
                        </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            >
                            <input
                                v-model="violationForm.occurred_at"
                                type="date"
                                class="w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm focus:border-rose-400 focus:ring-2 focus:ring-rose-500/20 focus:outline-none"
                            />
                        </div>
                        ...
                        <div class="space-y-1.5">
                            <label class="text-sm font-medium"
                                >Số tiền phạt (nếu có)</label
                            >
                            <div class="relative">
                                <span
                                    class="absolute top-1/2 left-3 -translate-y-1/2 text-sm font-bold text-muted-foreground"
                                    >₫</span
                                >
                                <input
                                    v-model="violationForm.penalty_amount"
                                    type="number"
                                    min="0"
                                    step="10000"
                                    class="w-full rounded-xl border border-border bg-background py-2.5 pr-3 pl-8 text-sm focus:border-rose-400 focus:ring-2 focus:ring-rose-500/20 focus:outline-none"
                                />
                            </div>
                        </div>
                        ...
                        >
                            <input
                                v-model="violationForm.apply_deduction"
                                type="checkbox"
                    ...
                    >
                        ...
                        <button
                            ...
                            :disabled="violationForm.processing"
