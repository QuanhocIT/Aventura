...
import { BarChart3, CheckCircle2, DollarSign, TrendingDown, TrendingUp, Users, XCircle } from 'lucide-vue-next';
import { ref } from 'vue';
...
function retentionColor(rate: number): string {
    if (rate >= 40) return 'bg-green-500 text-white';
    if (rate >= 20) return 'bg-green-300 text-green-900';
    if (rate >= 10) return 'bg-amber-200 text-amber-900';
...
<template>
    ...
    <div class="flex flex-col gap-6 p-6 max-w-7xl mx-auto">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-2xl font-bold">Business Intelligence Dashboard</h1>
        ...
        <div class="flex gap-2 border-b pb-2 overflow-x-auto">
            <button v-for="tab in [
                ...
                :class="['px-4 py-2 rounded-lg text-sm font-semibold whitespace-nowrap', activeTab === tab.key ? 'bg-indigo-600 text-white' : 'hover:bg-muted']"
            >{{ tab.label }}</button>
        </div>
        ...
        <div v-if="activeTab === 'overview'" class="space-y-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
                <Card><CardContent class="p-3"><p class="text-[10px] text-muted-foreground">Doanh thu</p><p class="text-lg font-bold">{{ ue.revenue.toLocaleString() }}đ</p></CardContent></Card>
                <Card><CardContent class="p-3"><p class="text-[10px] text-muted-foreground">Gross Margin</p><p class="text-lg font-bold" :class="ue.gross_margin >= 55 ? 'text-green-600' : 'text-red-500'">{{ ue.gross_margin }}%</p></CardContent></Card>
                <Card><CardContent class="p-3"><p class="text-[10px] text-muted-foreground">LTV (Giá trị vòng đời)</p><p class="text-lg font-bold text-blue-600">{{ ue.ltv.toLocaleString() }}đ</p></CardContent></Card>
                <Card><CardContent class="p-3"><p class="text-[10px] text-muted-foreground">CAC (Chi phí/KH)</p><p class="text-lg font-bold">{{ ue.cac.toLocaleString() }}đ</p></CardContent></Card>
                <Card><CardContent class="p-3"><p class="text-[10px] text-muted-foreground">LTV/CAC</p><p class="text-lg font-bold" :class="ue.ltv_cac_ratio >= 3 ? 'text-green-600' : 'text-amber-500'">{{ ue.ltv_cac_ratio }}x</p></CardContent></Card>
                <Card><CardContent class="p-3"><p class="text-[10px] text-muted-foreground">Đơn TB/KH</p><p class="text-lg font-bold">{{ ue.avg_orders_per_customer }}</p></CardContent></Card>
            </div>

            <!-- Revenue trend bar chart -->
            <Card>
                <CardHeader>
                    <CardTitle class="text-sm">Doanh thu 12 tháng & YoY</CardTitle>
                </CardHeader>
                <CardContent>
        ...
        <div v-if="activeTab === 'cohort'">
            <Card>
                <CardHeader>
                    <CardTitle class="text-base">Customer Cohort Retention (6 tháng)</CardTitle>
                    <CardDescription>% khách hàng quay lại mua theo tháng sau khi đăng ký.</CardDescription>
                </CardHeader>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle class="text-base flex items-center gap-2">
                            <component :is="breakEven.is_profitable ? CheckCircle2 : XCircle" :class="breakEven.is_profitable ? 'text-green-500' : 'text-red-500'" class="size-5" />
                            {{ breakEven.is_profitable ? 'Đã vượt điểm hòa vốn!' : 'Chưa đạt điểm hòa vốn' }}
                        </CardTitle>
                    </CardHeader>
                    <CardContent class="space-y-3 text-sm">
                        ...
                        <div class="border-t pt-2 flex justify-between"><span>Contribution margin/đơn</span><span class="font-bold text-blue-600">{{ breakEven.contribution_margin.toLocaleString() }}đ</span></div>
                    </CardContent>
                </Card>

                <Card>
                    <CardHeader><CardTitle class="text-base">Điểm hòa vốn</CardTitle></CardHeader>
                    <CardContent class="space-y-3 text-sm">
                        <div class="flex justify-between"><span>Cần tối thiểu</span><span class="font-bold text-2xl">{{ breakEven.break_even_orders }} đơn</span></div>
                        ...
                        <div class="h-3 rounded-full bg-muted overflow-hidden">
                            <div :class="['h-full rounded-full', breakEven.is_profitable ? 'bg-green-500' : 'bg-amber-500']"
        ...
        <div v-if="activeTab === 'benchmark'">
            <Card>
                <CardHeader>
                    <CardTitle class="text-base">Benchmark so với ngành F&B Việt Nam</CardTitle>
                    <CardDescription>So sánh các chỉ số chính với trung bình ngành nhà hàng.</CardDescription>
                </CardHeader>
