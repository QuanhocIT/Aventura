...
const filteredGoals = computed(() => {
    ...
    if (goalFilter.value === 'on_track') {
        return props.activeGoals.filter(
            (goal) =>
                getPace(goal) === 'on_track' || getPace(goal) === 'completed',
        );
    }
...
const overdueActionCount = computed(() => {
    return props.activeGoals.reduce((total, goal) => {
        const count =
            goal.actions?.filter((action) => isActionOverdue(action)).length ||
            0;
    ...
    <div class="mx-auto w-full max-w-[1600px] space-y-5 p-4 lg:p-7">
        ...
        <section v-if="activeTab === 'active'" class="space-y-5">
            <div v-if="filteredGoals.length" class="grid gap-4 xl:grid-cols-2">
                ...
                <div
                    ...
                    class="flex flex-col justify-between overflow-hidden rounded-2xl border border-indigo-200/90 bg-gradient-to-br from-indigo-50/80 via-white to-amber-50/60 p-5 shadow-sm lg:p-6 dark:border-indigo-500/25 dark:from-[#111625] dark:via-[#141a2c] dark:to-[#0f1422] dark:text-white"
                >
                    <div>
                        <!-- Header Strip -->
                        <div
                            class="flex items-center justify-between border-b border-indigo-100/80 pb-4 dark:border-white/10"
                        >
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    >
                                        Dự báo vận tốc & gợi ý hành động chiến
                                        lược
                                    </p>
                                </div>
                            </div>
                            <span
                                class="rounded-full border border-amber-300 bg-amber-100 px-2.5 py-0.5 text-[10px] font-extrabold text-amber-800 dark:border-amber-400/30 dark:bg-amber-500/20 dark:text-amber-300"
                            >
                                Gợi ý tối ưu
                            </span>
                        </div>
                        ...
                        <div
                            class="mt-5 rounded-2xl border border-slate-200/80 bg-white/90 p-4 shadow-2xs dark:border-white/10 dark:bg-black/30"
                        >
                            <div
                            ...
                            >
                                <span
                                    ...
                                    >Dự báo hoàn thành mục tiêu:</span
                                >
                                <span
                                    class="font-extrabold text-emerald-600 dark:text-emerald-400"
                                    >92.4% Đạt chỉ tiêu</span
                                >
                            </div>
                            <div
                                class="mt-2 text-xs leading-relaxed font-semibold text-slate-800 dark:text-slate-200"
                            >
                                🎯 Khoảng bù cần tăng trưởng:
                                <strong
                                    class="text-indigo-600 dark:text-indigo-400"
                                    >{{ formatCurrency(32891800) }}</strong
                                >
                                trong 34 ngày tới (~967.406 ₫/ngày).
                            </div>
                        </div>
                        ...
                        <div class="mt-5 space-y-2.5">
                            ...
                            >
                                Khuyến nghị gia tăng tốc độ:
                            </p>
                            <div
                                class="flex items-start gap-2.5 rounded-xl border border-emerald-200/80 bg-emerald-50/70 p-3 text-xs font-medium text-slate-800 dark:border-emerald-500/20 dark:bg-emerald-500/10 dark:text-slate-200"
                            >
                                <CheckCircle2
                                    class="mt-0.5 size-4 shrink-0 text-emerald-600 dark:text-emerald-400"
                                />
                                <span
                                    >Đẩy mạnh quảng cáo Zalo OA & gửi Voucher
                                    tri ân cho 350 khách hàng cũ.</span
                                >
                            </div>
                            <div
                                class="flex items-start gap-2.5 rounded-xl border border-indigo-200/80 bg-indigo-50/70 p-3 text-xs font-medium text-slate-800 dark:border-indigo-500/20 dark:bg-indigo-500/10 dark:text-slate-200"
                            >
                                <Sparkles
                                    class="mt-0.5 size-4 shrink-0 text-indigo-600 dark:text-indigo-400"
                                />
                                <span
                                    >Triển khai Menu Combo Gia Đình giảm 15% vào
                                    cuối tuần để kích cầu lượt khách nhóm.</span
                                >
                            </div>
                        </div>
                    </div>
                    ...
                    <div
                        class="mt-6 flex flex-wrap items-center justify-between gap-3 border-t border-slate-200/80 pt-4 dark:border-white/10"
                    >
                        <span
                            class="text-xs font-medium text-slate-500 dark:text-slate-400"
                            >Muốn thiết lập thêm OKR mới?</span
                        >
                        <Button
                            size="sm"
                            variant="brand"
        ...
        >
            ...
            >
                ...
                <Badge
                    variant="outline"
                    class="border-slate-200 text-xs font-bold dark:border-white/10"
                >
        ...
        >
            <DialogContent class="max-w-md">
                <DialogHeader>
                    <DialogTitle class="text-lg font-bold"
                        >Thêm kế hoạch hành động</DialogTitle
                    >
                </DialogHeader>
                <form
                    class="space-y-4 py-2"
                    @submit.prevent="submitCreateAction"
                >
                    <div>
