...
<template>
    ...
    <div class="min-h-dvh grid-cols-1 flex flex-col lg:grid lg:grid-cols-[1.1fr_2fr]">
        ...
        <div class="relative flex min-h-dvh flex-col overflow-hidden bg-gradient-to-b from-zinc-50 via-white to-zinc-50 dark:from-zinc-950 dark:via-zinc-900 dark:to-zinc-950 px-6 py-10 sm:px-10 md:px-12">
            ...
            <div class="pointer-events-none absolute -bottom-32 -left-32 h-[450px] w-[450px] rounded-full bg-primary/15 blur-[120px] float-glow" />
            <div class="pointer-events-none absolute -right-20 top-1/4 h-[350px] w-[350px] rounded-full bg-violet-500/10 blur-[100px] float-glow" style="animation-delay: 4s;" />
            <div class="pointer-events-none absolute top-3/4 right-1/3 h-[200px] w-[200px] rounded-full bg-rose-500/8 blur-[80px] float-glow" style="animation-delay: 6s;" />
            ...
            <div class="flex flex-1 flex-col justify-start pt-6 sm:pt-8 relative z-10">
                <div class="animate-in fade-in slide-in-from-bottom-6 duration-500 fill-mode-both w-full max-w-md mx-auto backdrop-blur-2xl bg-white/75 dark:bg-zinc-950/65 border border-white/40 dark:border-white/[0.08] p-8 sm:p-10 rounded-3xl shadow-[0_24px_64px_rgba(0,0,0,0.06),0_8px_20px_rgba(0,0,0,0.03)] dark:shadow-[0_24px_64px_rgba(0,0,0,0.35),0_8px_20px_rgba(0,0,0,0.2)]">

                    <div class="mb-4 inline-flex items-center gap-1.5 rounded-full border border-primary/20 bg-primary/5 dark:bg-primary/10 px-4 py-1 text-xs font-bold text-primary dark:text-primary">
                        <span class="h-1.5 w-1.5 animate-pulse rounded-full bg-primary/50/80" />
                        Đăng nhập an toàn
                    </div>
                    <h1 class="bg-gradient-to-r from-zinc-950 via-zinc-800 to-primary dark:from-white dark:via-zinc-200 dark:to-primary bg-clip-text text-3xl font-black tracking-tight text-transparent leading-none">Chào mừng trở lại</h1>
                    <p class="mt-2.5 text-sm text-muted-foreground">
                        Nhập thông tin để đăng nhập vào hệ thống.
                    </p>

                    <div v-if="status" class="mt-4 rounded-xl border border-primary/20 bg-primary/5 px-4 py-3 text-sm font-semibold text-primary dark:border-primary/20 dark:bg-primary/10 dark:text-primary shadow-sm animate-in fade-in slide-in-from-top-2 duration-300">
                        {{ status }}
                    </div>
                    ...
                    >
                        ...
                        <div class="grid gap-1.5">
                            <div class="flex items-center justify-between">
                                ...
                                <TextLink v-if="canResetPassword" :href="request()" class="text-xs text-muted-foreground hover:text-primary dark:hover:text-primary transition-colors duration-250 font-medium" :tabindex="5">
                                    Quên mật khẩu?
                                </TextLink>
                            </div>
                        ...
                        <div class="flex items-center gap-2.5 py-1">
                            <Checkbox id="remember" name="remember" :tabindex="3" class="rounded border-zinc-300 dark:border-zinc-700 text-primary focus:ring-primary/10" />
                        ...
                        <Button type="submit" class="w-full rounded-xl py-6 font-bold uppercase tracking-wider text-xs bg-gradient-to-r from-primary via-amber-500 to-rose-500 hover:from-primary/90 hover:via-amber-400 hover:to-rose-400 text-white transition-all duration-300 shadow-[0_4px_24px_rgba(245,158,11,0.3)] hover:shadow-[0_8px_32px_rgba(245,158,11,0.4)] hover:-translate-y-0.5 border-none cursor-pointer" :tabindex="4" :disabled="processing">
            ...
            <p v-if="canRegister" class="text-center text-sm text-muted-foreground mt-6 relative z-10">
                Chưa có tài khoản?
                <TextLink :href="register.url({ query: { plan: selectedPlan, cycle: selectedCycle } })" :tabindex="6" class="font-bold text-primary dark:text-primary underline underline-offset-4 hover:text-primary/80 transition-colors">
                    Đăng ký miễn phí
                </TextLink>
            </p>
        </div>
        ...
        <div class="relative hidden flex-col overflow-hidden border-l border-zinc-900 bg-zinc-950 px-12 py-10 lg:flex lg:min-h-dvh justify-between">
            ...
            <div class="pointer-events-none absolute top-1/4 left-1/3 h-[500px] w-[500px] rounded-full bg-primary/50/[0.12] blur-[80px] z-0" />
            <div class="pointer-events-none absolute bottom-1/4 right-1/4 h-[450px] w-[450px] rounded-full bg-violet-500/[0.10] blur-[80px] z-0" />
            ...
            <div class="relative z-10 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
                <div class="max-w-xl">
                    <div class="mb-3.5 inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-3.5 py-1 text-xs text-zinc-400 shadow-sm backdrop-blur-md">
                        <svg class="size-3 text-primary animate-pulse" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
                        Chọn gói phù hợp
                    </div>
                ...
                <div class="flex items-center gap-1 rounded-xl border border-white/10 bg-white/5 p-1 text-xs shrink-0 self-start sm:self-auto backdrop-blur-md">
                    ...
                    >
                        Năm
                        <span class="text-[9px] font-bold bg-primary/50 text-white px-1.5 py-0.5 rounded-full shadow-xs">-{{ maxDiscountPercent }}%</span>
                    </button>
                </div>
            </div>
                ...
                <div class="animate-in fade-in slide-in-from-bottom-4 duration-700 fill-mode-both w-full">
                    ...
                    <div class="mt-5 rounded-2xl border border-white/[0.08] bg-white/[0.04] backdrop-blur-md px-5 py-4 transition-all duration-300 shadow-md">
                        <p class="text-xs text-zinc-400 flex flex-wrap items-center gap-1.5 leading-relaxed">
                            <span class="inline-flex h-2 w-2 shrink-0 items-center justify-center rounded-full bg-primary/50 animate-pulse" />
