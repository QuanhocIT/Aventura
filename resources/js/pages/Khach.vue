...
function buildDisplayPlan(db: DbPlan) {
    ...
    for (const { key, label } of conditionals) {
        ...
        seen.add(label);
        if (db.features[key]) features.push(label);
        else unsupportedFeatures.push(label);
    }
...
<template>
    <AppTopbarLayout>
        ...
        <!-- ── Hero Slideshow (2.9) ────────────────────────────────── -->
        <section v-if="heroBanners.length > 0" class="relative w-full overflow-hidden bg-black" style="height: clamp(120px, 20vw, 280px);">
            <!-- Slides -->
            <TransitionGroup :name="heroDir === 'next' ? 'slide-next' : 'slide-prev'" tag="div" class="relative h-full w-full">
                <a
                    v-for="(b, i) in heroBanners"
                    v-show="i === heroIndex"
                    :key="b.id"
                    :href="b.link_url ?? undefined"
                    :target="b.link_url ? '_self' : undefined"
                    class="absolute inset-0 block h-full w-full"
                    :class="b.link_url ? 'cursor-pointer' : 'cursor-default'"
                >
                    <img
                        :src="b.image_url"
                        :alt="b.title ?? ''"
                    ...
                    <!-- Title overlay -->
                    <div v-if="b.title || b.subtitle" class="absolute bottom-10 left-8 right-8 sm:left-14">
                        <p v-if="b.title" class="text-xl font-bold text-white drop-shadow sm:text-3xl">{{ b.title }}</p>
                        <p v-if="b.subtitle" class="mt-1 text-sm text-white/80 drop-shadow sm:text-base">{{ b.subtitle }}</p>
                    </div>
                </a>
            </TransitionGroup>
            ...
            <template v-if="heroBanners.length > 1">
                <button
                    @click="navHero('prev')"
                    class="absolute left-3 top-1/2 z-10 -translate-y-1/2 rounded-full bg-black/40 p-2 text-white backdrop-blur-sm transition hover:bg-black/70 focus:outline-none"
                ...
                <button
                    @click="navHero('next')"
                    class="absolute right-3 top-1/2 z-10 -translate-y-1/2 rounded-full bg-black/40 p-2 text-white backdrop-blur-sm transition hover:bg-black/70 focus:outline-none"
                ...
                <div class="absolute bottom-4 left-1/2 z-10 flex -translate-x-1/2 gap-2">
                    <button
                        ...
                        :class="i === heroIndex ? 'w-7 bg-white' : 'w-2 bg-white/50 hover:bg-white/80'"
        ...
        <section id="hero-section" class="px-4 pt-16 lg:px-8 lg:pt-20">
            <div
            ...
            >
                <div>
                    ...
                    <div class="mt-5 flex items-center gap-2 text-sm text-muted-foreground">
                        ...
                        <span class="italic">"Đã dùng thử và rất thực tế"</span>
                        <span class="hidden sm:inline">— Nguyễn Minh, chủ quán phở 3 chi nhánh</span>
                    </div>
                    <div
