...
const openQuickCombo = (rule: ComboRule) => {
    ...
    const priceA = rule.item_a.includes('Lẩu')
        ? 350000
        : rule.item_a.includes('Nướng')
          ? 250000
          : 85000;
    const priceB =
        rule.item_b.includes('Sấu') ||
        rule.item_b.includes('Trà') ||
        rule.item_b.includes('Nước')
            ? 35000
            : 55000;
...
const createQuickCombo = () => {
    ...
    alert(
        `Đã tạo thành công Combo "${comboForm.name}" trong thực đơn và giảm giá ${comboForm.discount_percent}% cho khách hàng!`,
    );
