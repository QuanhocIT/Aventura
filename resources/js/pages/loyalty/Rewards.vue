...
function submit() {
    if (editing.value) {
        form.patch(`/loyalty/rewards/${editing.value.id}`, {
            onSuccess: () => {
                showDialog.value = false;
            },
        });
    } else {
        form.post('/loyalty/rewards', {
            onSuccess: () => {
                showDialog.value = false;
                form.reset();
            },
...
async function deleteReward(r: any) {
    if (
        !(await confirmDialog({
            ...
            description: `Xóa phần thưởng "${r.name}"?`,
        }))
    ) {
        return;
    }
...
<template>
                ...
                <CardContent class="space-y-2">
                    ...
                    <div class="flex items-center justify-between text-xs">
                        ...
                        <span class="font-bold text-amber-600"
                            >{{ r.points_cost }} điểm</span
                        >
                    </div>
                    <div
                        v-if="r.max_redemptions"
                        class="text-xs text-muted-foreground"
                    >
                        Đã đổi: {{ r.current_redemptions }}/{{
                            r.max_redemptions
                        }}
                    </div>
    ...
    <Dialog v-model:open="showDialog">
        <DialogContent class="max-w-md">
            <DialogHeader>
                <DialogTitle>{{
                    editing ? 'Sửa phần thưởng' : 'Thêm phần thưởng'
                }}</DialogTitle>
            </DialogHeader>
            <form @submit.prevent="submit" class="space-y-4">
                <div class="grid gap-1.5">
                    ...
                    <Input
                        v-model="form.name"
                        placeholder="Ví dụ: Giảm 20% đơn tiếp theo"
                        required
                    />
                </div>
                <div class="grid gap-1.5">
