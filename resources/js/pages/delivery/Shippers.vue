    ...
    try {
        if (editId.value) {
            const { data } = await axios.patch(
                `/delivery/shippers/${editId.value}`,
                {
            ...
            employees.value = employees.value.filter(
                (e) => e.id !== Number(form.value.employee_id),
            );
...
async function toggleActive(s: ShipperRow) {
    try {
        await axios.patch(`/delivery/shippers/${s.id}`, {
            is_active: !s.is_active,
        });
        ...
        toast.success(
            s.is_active ? 'Đã tạm ngưng shipper' : 'Đã kích hoạt shipper',
        );
    } catch {
...
async function deleteShipper(s: ShipperRow) {
    if (
        !(await confirmDialog({
            ...
            description: `Xác nhận vô hiệu hóa shipper "${s.name}"?`,
        }))
    ) {
        return;
    }
...
<template>
    ...
    <div class="mx-auto max-w-5xl space-y-6 p-6">
            ...
            <Card>
                <CardContent class="pt-4">
                    ...
                    <p class="text-2xl font-bold text-green-600">
                        {{
                            shippers.filter((s) => s.is_active && !s.deleted_at)
                                .length
                        }}
                    </p>
                </CardContent>
        ...
        <div class="flex gap-2 border-b">
            <button
                ...
                :key="f[0]"
                class="border-b-2 px-1 pb-2 text-sm transition-colors"
                :class="
                    filterActive === f[0]
            ...
            <CardContent class="p-0">
                ...
                <table v-else class="w-full text-sm">
                    <thead class="border-b bg-muted/40">
                        <tr>
                            <th
                                class="py-3 pl-4 text-left font-medium text-muted-foreground"
                            >
                                Nhân viên
                            </th>
                            <th
                            ...
                            <th
                                class="py-3 pr-4 text-right font-medium text-muted-foreground"
                            >
                                Thao tác
                            </th>
                        </tr>
                        ...
                        >
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-2">
                                    <component
                                        :is="VEHICLE_ICONS[s.vehicle_type]"
                                        class="h-4 w-4 text-muted-foreground"
                                    />
                                    <span>{{
                                        VEHICLE_LABELS[s.vehicle_type]
                                    }}</span>
                                </div>
                                <div
                            ...
                            <td class="px-4 py-3">
                                <Badge
                                    :variant="
                                        shipperStatus(s) === 'active'
                                            ? 'default'
                                            : 'secondary'
                                    "
                                    :class="
                                        shipperStatus(s) === 'deleted'
    ...
    <Teleport to="body">
        ...
        >
            <div
                class="mx-4 w-full max-w-md space-y-5 rounded-xl bg-background p-6 shadow-2xl"
            >
                <div class="flex items-center justify-between">
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        <button
                            ...
                            :key="v[0]"
                            type="button"
                            class="rounded-lg border py-2 text-center text-sm transition-colors"
                            :class="
                                form.vehicle_type === v[0]
                ...
                <div class="flex gap-3 pt-2">
                    <Button variant="outline" class="flex-1" @click="closeModal"
                        >Hủy</Button
                    >
                    <Button
                        class="flex-1"
