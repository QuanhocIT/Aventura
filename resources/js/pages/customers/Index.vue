...
const props = defineProps<{
    ...
    tierCounts: {
        gold: number; silver: number; bronze: number; normal: number;
        goldPct: number; silverPct: number; bronzePct: number; normalPct: number;
    };
    isOwner: boolean;
...
function applyFilters() {
    router.get('/customers', {
        ...
        segment: segmentFilter.value !== 'all' ? segmentFilter.value : undefined,
    ...
    <div class="flex flex-col gap-6 p-6 max-w-7xl mx-auto w-full">
            ...
            <div class="sm:col-span-1 flex flex-col items-center justify-center rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 gap-2 relative group cursor-pointer">
                <svg width="80" height="80" viewBox="0 0 80 80" class="overflow-visible">
                    ...
                    <circle v-if="tierCounts.gold > 0" cx="40" cy="40" r="28" fill="none" stroke="#d97706" stroke-width="12"
                        stroke-dasharray="175.9"
                        :stroke-dashoffset="175.9 * (1 - tierCounts.gold / (segmentCounts.all || 1))"
                        transform="rotate(-90 40 40)" />
                        
                    <!-- Silver segment -->
                    <circle v-if="tierCounts.silver > 0" cx="40" cy="40" r="28" fill="none" stroke="#94a3b8" stroke-width="12"
                        stroke-dasharray="175.9"
                        :stroke-dashoffset="175.9 * (1 - tierCounts.silver / (segmentCounts.all || 1))"
                    ...
                    <circle v-if="tierCounts.bronze > 0" cx="40" cy="40" r="28" fill="none" stroke="#ea580c" stroke-width="12"
                        stroke-dasharray="175.9"
                        :stroke-dashoffset="175.9 * (1 - tierCounts.bronze / (segmentCounts.all || 1))"
                    ...
                    <circle v-if="tierCounts.normal > 0" cx="40" cy="40" r="28" fill="none" stroke="#cbd5e1" stroke-width="12"
                        stroke-dasharray="175.9"
                        :stroke-dashoffset="175.9 * (1 - tierCounts.normal / (segmentCounts.all || 1))"
                        :transform="`rotate(${-90 + ((tierCounts.gold + tierCounts.silver + tierCounts.bronze) / (segmentCounts.all || 1)) * 360} 40 40)`" class="dark:stroke-slate-700" />
                </svg>
        ...
        <Card class="shadow-sm overflow-hidden">
            ...
            <CardContent class="p-0">
                <div v-if="customers.data.length === 0" class="flex flex-col items-center gap-3 py-20 text-center text-muted-foreground">
                    ...
                    <table class="w-full text-xs text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-100 dark:bg-slate-950 border-b text-[10px] uppercase font-bold tracking-wider text-slate-500">
                                ...
                                <th class="p-3.5 cursor-pointer select-none" @click="setSort(sortBy === 'points_desc' ? 'points_asc' : 'points_desc')">
                                    <span class="flex items-center gap-1">
                                        Điểm tích lũy
                                        <ArrowUpDown class="size-3 text-slate-400" />
                                    </span>
                                </th>
                                <th class="p-3.5 cursor-pointer select-none" @click="setSort('recent')">
                                    <span class="flex items-center gap-1">
                                        Đơn cuối lúc
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                            <tr v-for="c in customers.data" :key="c.id" class="hover:bg-slate-50/50 dark:hover:bg-slate-900/30 transition-colors">
                ...
                <!-- Pagination -->
                <div v-if="customers.last_page > 1" class="flex items-center justify-between border-t p-4 bg-slate-50/50 dark:bg-slate-900/30">
                    <div class="text-xs text-muted-foreground">
                        Hiển thị {{ customers.from ?? 0 }} - {{ customers.to ?? 0 }} trong tổng số {{ customers.total }} khách hàng
                    </div>
