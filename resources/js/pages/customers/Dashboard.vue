                ...
                <div v-else class="grid grid-cols-1 gap-3.5">
                    <div v-for="reward in rewards" :key="reward.id" class="p-4 bg-white border border-slate-200 rounded-2xl flex gap-3.5 items-center shadow-sm relative">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-[10px] text-slate-400 font-bold mt-0.5">Trị giá: {{ formatCurrency(reward.price) }}</p>
                            
                            <div class="flex items-center justify-between mt-2">
                                <span class="text-[11px] font-black text-amber-600 bg-amber-50 px-2 py-0.5 rounded-lg border border-amber-100">{{ reward.points_required }} pt</span>
                                
                                <button 
                                    @click="redeem(reward)"
                                    :disabled="currentPoints < reward.points_required || isRedeeming !== null"
