...
<template>
    ...
    <div class="min-h-full bg-slate-50/60 px-4 py-6 dark:bg-slate-950 sm:px-6 lg:px-8">
        <div class="mx-auto max-w-7xl space-y-6">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div class="flex items-center gap-3">
                    <div class="flex size-12 items-center justify-center rounded-2xl bg-indigo-100 text-indigo-600 dark:bg-indigo-950/40 dark:text-indigo-400">
                        <UtensilsCrossed class="size-6" />
                    </div>
                    <div>
                        <p class="text-xs font-bold tracking-wide text-indigo-600 uppercase dark:text-indigo-400">
                            {{ branchName }}
                        </p>
                        ...
                        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                            Chọn các món bếp tạm thời không thể phục vụ để POS và QR không lên nhầm đơn.
                        </p>
                    </div>
                </div>

                <div class="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs font-semibold text-amber-800 dark:border-amber-900/50 dark:bg-amber-950/30 dark:text-amber-300">
                    <div class="flex items-center gap-2">
                        <AlertTriangle class="size-4 shrink-0" />
                        <span>Trạng thái kích hoạt có hiệu lực đến hết ngày hôm nay.</span>
                    </div>
                </div>
            </div>

            <div v-if="step === 'select'" class="space-y-5">
                <div class="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-5">
                    <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                        <div class="relative min-w-0 flex-1">
                            <Search class="absolute top-1/2 left-3.5 size-4 -translate-y-1/2 text-slate-400" />
                            <input
                                v-model="searchQuery"
                                type="search"
                                placeholder="Tìm kiếm món ăn..."
                                class="h-11 w-full rounded-2xl border border-slate-200 bg-slate-50 pr-4 pl-10 text-sm text-slate-900 outline-none transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 dark:border-slate-700 dark:bg-slate-950 dark:text-white"
                            />
                        </div>
                        <div class="flex gap-2 overflow-x-auto pb-1">
                            <button
                                ...
                                :class="selectedCategoryId === 'all' ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300'"
                            ...
                            <button
                                ...
                                type="button"
                                class="shrink-0 rounded-xl px-3.5 py-2 text-xs font-bold transition"
                                :class="selectedCategoryId === category.id ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300'"
                ...
                <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                    ...
                    >
                        <span class="text-[10px] font-bold tracking-wide text-slate-400 uppercase">
                            {{ product.category_name }}
                        </span>
                        <span class="mt-3 line-clamp-2 text-sm font-black text-slate-900 dark:text-white">
                            {{ product.name }}
                        </span>
                        <span class="mt-2 text-xs font-bold text-indigo-600 dark:text-indigo-400">
                        ...
                        <span
                            v-else-if="product.is_paused || !product.is_available"
                ...
                <div v-if="filteredProducts.length === 0" class="rounded-3xl border border-dashed border-slate-300 py-20 text-center text-sm text-slate-500 dark:border-slate-700">
                    Không tìm thấy món phù hợp.
                </div>

                <div class="sticky bottom-4 z-10 flex flex-col gap-3 rounded-2xl border border-indigo-200 bg-white/95 p-4 shadow-xl backdrop-blur sm:flex-row sm:items-center sm:justify-between dark:border-indigo-900/50 dark:bg-slate-900/95">
                    <div>
            ...
            <div v-else class="mx-auto max-w-3xl space-y-5">
                ...
                <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-7">
                    <div class="flex items-start gap-3 border-b border-slate-100 pb-5 dark:border-slate-800">
                        <div class="flex size-11 shrink-0 items-center justify-center rounded-2xl bg-rose-100 text-rose-600 dark:bg-rose-950/40 dark:text-rose-400">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                                Các món dưới đây sẽ bị làm mờ và không thể lên đơn tại {{ branchName }} đến hết ngày {{ today }}.
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="mt-5 grid gap-2 sm:grid-cols-2">
                        ...
                        >
                            <span class="flex size-8 items-center justify-center rounded-xl bg-rose-600 text-xs font-black text-white">
                                <Check class="size-4" />
                            </span>
                            <div class="min-w-0">
                                ...
                                <p class="text-[11px] text-slate-500 dark:text-slate-400">
                                    {{ product.category_name }} · {{ numberFormat(product.price) }}đ
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-6">
                        <label class="mb-2 block text-sm font-black text-slate-800 dark:text-slate-200">
                            Lý do tạm ngưng món <span class="text-rose-500">*</span>
                        </label>
                        <textarea
                            v-model="reason"
                            rows="5"
                            ...
                            class="w-full rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-900 outline-none transition focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 dark:border-slate-700 dark:bg-slate-950 dark:text-white"
                        ></textarea>
                    ...
                    <div class="mt-5 flex flex-col-reverse gap-2 border-t border-slate-100 pt-5 sm:flex-row sm:justify-end dark:border-slate-800">
                        <Button
                            variant="outline"
                        ...
                        >
                            {{ isSubmitting ? 'Đang kích hoạt...' : 'Kích hoạt đến hết ngày' }}
                        </Button>
                    </div>
                </div>
            </div>
