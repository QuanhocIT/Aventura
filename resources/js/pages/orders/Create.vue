...
import {
    ...
    Truck, MapPin, UtensilsCrossed as DineIn
...
const channel = ref<'dine_in' | 'takeaway' | 'delivery'>('dine_in');
const deliveryInfo = ref({ customer_name: '', phone: '', address: '', fee: 0 });
...
const submitOrder = () => {
    ...
    if (channel.value === 'delivery') {
        if (!deliveryInfo.value.customer_name || !deliveryInfo.value.phone || !deliveryInfo.value.address) {
    ...
    const form = useForm({
        ...
        table_id: channel.value === 'dine_in' ? selectedTableId.value : null,
        note: '',
        ...
        ...(channel.value === 'delivery' ? {
            delivery_customer_name: deliveryInfo.value.customer_name,
            delivery_phone: deliveryInfo.value.phone,
            delivery_address: deliveryInfo.value.address,
            delivery_fee: deliveryInfo.value.fee,
        } : {}),
    });
    ...
    form.post('/orders', {
        onSuccess: () => {
            ...
            deliveryInfo.value = { customer_name: '', phone: '', address: '', fee: 0 };
        }
    });
};
...
<template>
    ...
    <div class="flex flex-col h-[calc(100vh-4rem)] p-4 max-w-[1600px] mx-auto w-full gap-4 overflow-hidden">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 border-b pb-3 shrink-0">
            ...
            <!-- Channel selector + table/delivery controls -->
            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-2 w-full sm:w-auto">
                <!-- Channel pills -->
                <div class="flex rounded-lg border border-input bg-background p-0.5 gap-0.5 shrink-0">
                    <button
                        v-for="ch in ([{val:'dine_in',label:'Tại bàn'},{val:'takeaway',label:'Mang về'},{val:'delivery',label:'Giao hàng'}] as const)"
                        :key="ch.val"
                        @click="channel = ch.val"
                        class="px-3 py-1.5 rounded-md text-xs font-semibold transition-all"
                        :class="channel === ch.val
                ...
                <!-- Delivery badge -->
                <div v-if="channel === 'delivery'" class="flex items-center gap-1.5 text-xs text-orange-600 dark:text-orange-400 font-medium bg-orange-50 dark:bg-orange-950/30 px-3 py-1.5 rounded-md border border-orange-200 dark:border-orange-800">
                    <Truck class="size-3.5" /> Điền địa chỉ trong giỏ hàng
                </div>
            </div>
        </div>
        ...
        <div class="flex flex-col lg:flex-row gap-4 flex-1 min-h-0 overflow-hidden">
            ...
            <div class="w-full lg:w-96 flex flex-col shrink-0 min-h-0 bg-white dark:bg-slate-950 rounded-2xl border overflow-hidden">
                ...
                <div v-if="channel === 'delivery'" class="shrink-0 p-4 border-t bg-orange-50/60 dark:bg-orange-950/10">
                    <div class="flex items-center gap-2 text-orange-700 dark:text-orange-400 font-semibold text-xs mb-3">
                        <MapPin class="size-3.5" /> Thông tin giao hàng
                    </div>
                    <div class="flex flex-col gap-2.5">
                        <div class="grid grid-cols-2 gap-2">
                            <div>
                                <label class="text-[10px] font-medium text-slate-600 dark:text-slate-400 mb-1 block">Tên người nhận <span class="text-red-500">*</span></label>
                                <input v-model="deliveryInfo.customer_name" type="text" placeholder="Nguyễn Văn A"
                                    class="w-full h-8 rounded border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-2.5 text-xs focus:outline-none focus:ring-1 focus:ring-orange-400" />
                            </div>
                            <div>
                                <label class="text-[10px] font-medium text-slate-600 dark:text-slate-400 mb-1 block">Số điện thoại <span class="text-red-500">*</span></label>
                                <input v-model="deliveryInfo.phone" type="tel" placeholder="0901234567"
                                    class="w-full h-8 rounded border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-2.5 text-xs focus:outline-none focus:ring-1 focus:ring-orange-400" />
                            </div>
                        </div>
                        <div>
                            <label class="text-[10px] font-medium text-slate-600 dark:text-slate-400 mb-1 block">Địa chỉ giao hàng <span class="text-red-500">*</span></label>
                            <textarea v-model="deliveryInfo.address" rows="2" placeholder="123 Nguyễn Huệ, Quận 1, TP.HCM"
                                class="w-full rounded border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-2.5 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-orange-400 resize-none" />
                        </div>
                        <div>
                            <label class="text-[10px] font-medium text-slate-600 dark:text-slate-400 mb-1 block">Phí giao hàng (₫)</label>
                            <input v-model.number="deliveryInfo.fee" type="number" min="0" step="1000" placeholder="0"
                                class="w-full h-8 rounded border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-2.5 text-xs focus:outline-none focus:ring-1 focus:ring-orange-400" />
                        </div>
                    </div>
                </div>
                ...
                <div class="p-4 border-t bg-slate-50 dark:bg-slate-900/40 shrink-0 flex flex-col gap-3">
                    ...
                    >
                        <Truck v-if="channel === 'delivery'" class="size-4" />
                        <ChefHat v-else class="size-4" />
                        {{ channel === 'delivery' ? 'Xác nhận & Tạo đơn Giao hàng' : 'Xác nhận & Chuyển bếp' }}
                    </Button>
                </div>
            </div>
        </div>
