...
import {
    ...
    CheckCircle2, AlertCircle, Pencil, Trash2, ChevronDown, ChevronUp,
...
<template>
    ...
    <FormModal
        ...
        @update:open="showAddCategory = $event"
        title="Thêm Nhóm Món Ăn Mới"
        :processing="categoryForm.processing"
        submit-label="Tạo nhóm món"
    ...
    <FormModal
        :open="showAddProduct"
        @update:open="showAddProduct = $event"
        title="Thêm Món Ăn Mới Vào Thực Đơn"
        description="Nhập thông tin chi tiết về sản phẩm để phục vụ bán hàng."
        :processing="productForm.processing"
        submit-label="Thêm món ăn"
    ...
    <FormModal
        :open="!!editingProduct"
        @update:open="(v: boolean) => { if (!v) editingProduct = null }"
    ...
    <FormModal
        :open="showComboModal"
        @update:open="showComboModal = $event"
        title="Thiết lập Combo đề xuất"
        description="Tạo nhanh sản phẩm Combo từ gợi ý phân tích giỏ hàng của AI."
        :processing="comboForm.processing"
        submit-label="Tạo Combo"
        processing-label="Đang tạo..."
        @submit="submitCombo"
    >
        <div class="grid gap-1.5">
