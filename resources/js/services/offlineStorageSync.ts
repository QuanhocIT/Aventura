...
function savePendingOrders(orders: PendingOfflineOrder[]): void {
    try {
        localStorage.setItem(
            STORAGE_KEY_PENDING_ORDERS,
            JSON.stringify(orders),
        );
        pendingOrderCount.value = orders.filter(
...
// Initialize count
pendingOrderCount.value = loadPendingOrders().filter(
    (o) => o.status === 'pending_sync',
).length;
...
export function storeOfflineOrder(
    orderData: Omit<PendingOfflineOrder, 'id' | 'created_at' | 'status'>,
): PendingOfflineOrder {
