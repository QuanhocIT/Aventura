...
function buildPoints(
    ...
    return series.map((point, index) => {
        const x =
            series.length <= 1 ? 0 : (index / (series.length - 1)) * VIEW_WIDTH;
...
const points = computed(() =>
    buildPoints(props.series, scale.value.min, scale.value.span),
);
const comparePoints = computed(() =>
    props.compareSeries
        ? buildPoints(props.compareSeries, scale.value.min, scale.value.span)
        : [],
);
...
function handleMouseMove(e: MouseEvent) {
    ...
    hoveredIdx.value = Math.min(
        ...
        Math.max(0, Math.round(percent * (points.value.length - 1))),
    );
}
...
const hoveredPoint = computed(() =>
    hoveredIdx.value !== null ? points.value[hoveredIdx.value] : null,
);
const hoveredComparePoint = computed(() =>
    hoveredIdx.value !== null
        ? (comparePoints.value[hoveredIdx.value] ?? null)
        : null,
);
</script>
...
<template>
    ...
    >
        ...
        >
            ...
            <circle
                v-if="hoveredComparePoint"
                :cx="hoveredComparePoint.x"
                :cy="hoveredComparePoint.y"
                r="1.2"
                :fill="compareColor"
                stroke="#fff"
                stroke-width="0.5"
        ...
        >
            <slot
                name="tooltip"
                :point="hoveredPoint.raw"
                :compare-point="hoveredComparePoint?.raw ?? null"
                :index="hoveredIdx"
            >
                <span
                ...
                >
                <span class="font-extrabold" :style="{ color }">{{
                    hoveredPoint.value
                }}</span>
            </slot>
        </div>
