...
<template>
    <Link
        :href="`/tin-tuc/${slug}`"
        class="group flex flex-col overflow-hidden rounded-2xl border border-border bg-background shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
    >
        <!-- Image -->
        <div class="relative aspect-video overflow-hidden bg-muted">
            ...
            <span
                v-if="is_featured"
                class="absolute top-3 left-3 rounded-full bg-primary px-2 py-0.5 text-xs font-semibold text-primary-foreground"
                >Nổi bật</span
            >
        </div>
        ...
        <div class="flex flex-1 flex-col gap-2 p-4">
            ...
            >
                ...
                <span v-if="reading_time" class="flex items-center gap-1">
                    <Clock class="size-3" />
                    {{ reading_time }} phút đọc
                </span>
                <span
                    v-if="view_count !== undefined"
