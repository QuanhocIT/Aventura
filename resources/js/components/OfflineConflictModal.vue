...
<template>
    ...
    >
        ...
        >
            <button
                @click="emit('close')"
                class="absolute right-4 top-4 rounded-full p-1 text-muted-foreground hover:bg-muted"
            >
                <X class="size-5" />
            </button>

            <div class="flex items-center gap-3">
                <div class="flex h-12 w-12 items-center justify-center rounded-2xl bg-rose-500/10 text-rose-600 dark:bg-rose-500/20">
                ...
                <div>
                    ...
                    <p class="text-xs text-muted-foreground">
                        Một số món ăn không đủ số lượng trong kho tại thời điểm đồng bộ lại.
                    </p>
                </div>
            </div>
            ...
            <div class="mt-5 space-y-3">
                ...
                >
                    <div>
                        <p class="font-bold text-foreground">{{ c.product_name }}</p>
                        <p class="mt-0.5 text-muted-foreground">
                            Đã đặt: <strong class="text-rose-600">{{ c.requested }}</strong> · Còn khả dụng: <strong class="text-emerald-600">{{ c.available_stock }}</strong>
                        </p>
                    </div>
                </div>
            </div>
