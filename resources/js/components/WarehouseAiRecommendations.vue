...
const signals = computed(() => {
    ...
    return allSignals
        .filter(
            (signal: any) =>
                !sourceFilter.length || sourceFilter.includes(signal.source),
        )
        .slice(0, props.max);
});
...
<template>
    <Card
        class="border-indigo-500/25 bg-gradient-to-br from-indigo-500/5 via-background to-violet-500/5 shadow-sm"
    >
        ...
        <CardContent class="space-y-3 p-4">
            ...
            >
                <LoaderCircle class="size-4 animate-spin" /> AI đang tổng hợp dữ
                liệu Kho Tổng...
            </div>
            <template v-else>
                <div
                ...
                >
                    <span>{{
                        ai?.summary || 'Chưa có đủ dữ liệu để đưa ra gợi ý.'
                    }}</span>
                    <span v-if="ai?.confidence" class="shrink-0"
                        >Độ tin cậy
                        {{ Math.round(Number(ai.confidence) * 100) }}%</span
                    >
                </div>

                <div
                    v-if="!signals.length"
                    class="flex items-center gap-2 rounded-xl border border-dashed border-emerald-500/25 bg-emerald-500/5 p-3 text-xs text-emerald-700 dark:text-emerald-300"
                >
                    <CheckCircle2 class="size-4 shrink-0" /> Chưa có ngoại lệ
                    cần ưu tiên ở chức năng này.
                </div>
                <div
                    v-for="signal in signals"
                    :key="`${signal.source}-${signal.metric}-${signal.title}`"
                    class="rounded-xl border p-3"
                    :class="signalClass(signal.severity)"
                >
                    <div class="flex items-start gap-3">
                        <div
                            class="flex size-8 shrink-0 items-center justify-center rounded-lg"
                            :class="iconClass(signal.severity)"
                        >
                            <AlertTriangle
                                v-if="
                            ...
                            <Clock3
                                v-else-if="
                                    signal.source === 'warehouse_workload' ||
                                    signal.source === 'supply_request_sla'
                                "
                                class="size-4"
                            />
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p
                                class="mt-1.5 text-[11px] leading-relaxed font-semibold text-foreground"
                            >
                                {{ signal.advice }}
                            </p>
                            <a
                                ...
                                class="mt-2 inline-flex items-center gap-1 text-[11px] font-bold text-indigo-600 hover:underline dark:text-indigo-300"
                            >
                                {{ signal.action_label || 'Xem chi tiết' }}
            ...
            <p class="text-[10px] text-muted-foreground">
                Gợi ý chỉ hỗ trợ quyết định; Trưởng kho xác nhận trước khi thực
                hiện thao tác.
            </p>
        </CardContent>
    </Card>
