...
<template>
    <Card
        class="relative overflow-hidden border border-slate-200 bg-white text-slate-900 shadow-sm dark:border-slate-800 dark:bg-slate-900/60 dark:text-slate-100"
    >
        <div
            class="absolute -top-10 -right-10 h-32 w-32 rounded-full bg-amber-500/10 blur-2xl"
        ></div>

        <CardHeader
            class="flex flex-row items-center justify-between gap-3 border-b border-slate-100 bg-slate-50/50 pb-3 dark:border-slate-800 dark:bg-slate-900/40"
        >
            <div>
                <CardTitle
        ...
        <CardContent class="space-y-4 pt-4 text-xs">
            ...
            <div v-else-if="forecastData.length > 0" class="space-y-4">
                    ...
                    <button
                        ...
                        :class="[
                            ...
                            selectedDayIndex === idx
                                ? 'border-amber-500 bg-amber-500/15 font-bold text-amber-600 shadow-xs ring-1 ring-amber-500/40 dark:text-amber-400'
                                : 'border-slate-200 bg-slate-50/70 text-slate-600 hover:border-slate-300 hover:bg-slate-100 hover:text-slate-900 dark:border-slate-800 dark:bg-slate-900/60 dark:text-slate-400 dark:hover:border-slate-700 dark:hover:bg-slate-800 dark:hover:text-slate-200',
                        ]"
                    >
                        <span
                            class="text-[10px] font-extrabold tracking-tight uppercase"
                            >{{ formatTabDate(day.date).dayName }}</span
                        >
                        <span class="my-0.5 text-xs leading-none font-black">{{
                ...
                <div
                    class="space-y-3 rounded-2xl border border-slate-200 bg-slate-50/80 p-3.5 dark:border-slate-800 dark:bg-slate-900/50"
                >
                    <div
                        class="flex items-center justify-between border-b border-slate-200/80 pb-2.5 dark:border-slate-800"
                    >
                        <div class="flex items-center gap-2.5">
                            ...
                            <div>
                                <h4
                                    class="text-xs font-bold text-slate-900 capitalize dark:text-white"
                                >
                                    {{
                                        forecastData[selectedDayIndex]
                                            ?.condition === 'sunny'
                        ...
                        <div
                            class="flex items-center gap-1 text-sm font-black text-slate-900 dark:text-white"
                        >
                    ...
                    <div class="space-y-2">
                        <h5
                            class="flex items-center gap-1.5 text-[10px] font-black tracking-wider text-slate-500 uppercase dark:text-slate-400"
                        >
                            ...
                            >
                                <div class="flex items-center justify-between">
                                    <div class="min-w-0 pr-2">
                                        <span
                                            class="block truncate text-xs font-bold text-slate-900 dark:text-white"
                                            >{{ rec.product_name }}</span
                                        >
                                        <span
                                            class="mt-0.5 inline-block rounded border border-slate-200 bg-slate-100 px-1.5 py-0.5 text-[9px] font-semibold text-slate-600 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400"
                                        >
                                            {{ rec.category_name }}
                                        </span>
                                    </div>
                                ...
                                <p
                                    class="text-[10px] leading-relaxed text-slate-600 dark:text-slate-300"
                                >
                                    {{ rec.reason }}
                                </p>
                                <div
                                    class="flex items-center justify-between border-t border-slate-100 pt-1.5 text-[9px] text-slate-500 dark:border-slate-800/80 dark:text-slate-400"
                                >
                                    <span
                                        >Bán ngày thường:
                                        <strong
                        ...
                        >
                            💡 Dự báo thời tiết ôn hòa, sức mua thực đơn dự kiến
