...
<template>
    <section class="space-y-4">
        <h2 class="text-base font-extrabold text-slate-800 dark:text-slate-100 flex items-center gap-2">
        ...
        <div class="grid grid-cols-2 gap-3.5 sm:grid-cols-3 lg:grid-cols-6">
            <Link
                ...
                :key="action.label"
                :href="action.href"
                class="group flex flex-col items-center gap-3 rounded-2xl border border-slate-100 dark:border-slate-800/80 bg-white dark:bg-slate-900/60 p-4 text-center transition-all duration-300 hover:shadow-lg hover:border-slate-200 dark:hover:border-slate-750 hover:translate-y-[-2px]"
            >
                <div
                    class="flex h-11 w-11 items-center justify-center rounded-xl border transition-all duration-300 group-hover:scale-110 group-hover:rotate-3 shadow-inner"
                    :class="{
                        ...
                        'border-rose-100    bg-rose-50    text-rose-600    dark:border-rose-900/30    dark:bg-rose-950/20    dark:text-rose-455':    action.color === 'rose',
                        ...
                        'border-slate-100   bg-slate-50   text-slate-600   dark:border-slate-900/30   dark:bg-slate-950/20   dark:text-slate-450':   action.color === 'slate',
                    }"
                >
                    <component :is="action.icon" class="size-5" />
                </div>
                <div>
                    <p class="text-sm font-bold text-slate-800 dark:text-slate-200 transition-colors group-hover:text-primary">{{ action.label }}</p>
                    <p class="mt-1 text-[11px] leading-snug font-medium text-slate-400 dark:text-slate-500">{{ action.description }}</p>
                </div>
            </Link>
        </div>
    </section>
