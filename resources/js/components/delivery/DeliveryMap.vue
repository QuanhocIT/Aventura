    ...
    props.shippers.forEach((s) => {
        ...
        if (shipperMarkers[s.id]) {
            ...
            shipperMarkers[s.id] = L.marker([s.lat, s.lng], { icon })
                .bindTooltip(
                    `<b>${s.name}</b><br>${VEHICLE_ICONS[s.vehicle_type] ?? ''} ${s.vehicle_type}${s.has_gps ? ' · 🟢 GPS' : ' · ⚫ offline'}`,
                    { direction: 'top' },
                )
...
function renderBatchRoute(s: ShipperMapData) {
    ...
    const color = getShipperColor(s.id);
    const items = [...s.active_batch.items].sort(
    ...
    validItems.forEach((item) => {
        ...
        const etaStr = item.eta
            ? formatEtaMs(new Date(item.eta).getTime() - Date.now())
            : '';
        const marker = L.marker([item.lat!, item.lng!], { icon: stopIcon })
            .bindTooltip(
                `<b>#${item.sequence_order} ${item.customer_name ?? ''}</b><br>${item.address ?? ''} ${etaStr ? `· ETA ${etaStr}` : ''}`,
                { direction: 'top' },
            )
            .addTo(map);
    ...
    props.orders.forEach((o) => {
        ...
        if (orderMarkers[o.id]) {
            ...
            orderMarkers[o.id] = L.marker([o.lat, o.lng], { icon })
                .bindTooltip(`<b>${o.customer_name}</b><br>${o.address}`, {
                    direction: 'top',
                })
