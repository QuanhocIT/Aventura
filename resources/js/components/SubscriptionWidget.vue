...
import {
    ArrowUpRight, Building2, Grid, Sparkles, Users2, Zap,
        ...
        <div class="relative overflow-hidden rounded-xl border border-border/60 bg-gradient-to-b from-card to-background/50 p-4 shadow-sm">
            ...
            <div class="space-y-1.5">
                ...
                <div class="flex items-center justify-between text-xs">
                    ...
                    <span class="font-semibold" :class="(tenant?.plan?.features?.ai_advisor || tenant?.plan?.features?.realtime) ? 'text-emerald-500' : 'text-muted-foreground'">
                        {{ (tenant?.plan?.features?.ai_advisor || tenant?.plan?.features?.realtime) ? 'Có' : 'Không' }}
                    </span>
                </div>
            </div>
